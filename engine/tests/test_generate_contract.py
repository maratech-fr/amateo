...
class TestGenerateContract:
    ...
    def test_build_schedule_fully_locked_team_does_not_fail(self) -> None:
        ...
        remaining=0. Without the all-zeros min_by_team, max_cap adds sum(vars)<=0
        while min_sessions adds sum(vars)>=1 → immediate INFEASIBLE.
        With the fix, min=0 for this team → no conflict.
        """
        input_data = ScheduleInputSchema.model_validate(
            {

...
3. **Score formula** — `SCORE_FORMULA_VERSION` and `LEVEL_2_OBJECTIVE_WEIGHTS` live in `app/solver/objective/weights.py` (a package, not a flat `objective.py`) — the constant is the sole source of truth, **never recopy a value into a doc**: a copied weight once survived a rebalancing and lied for weeks before it was caught. Changing any level-2 weight requires bumping `SCORE_FORMULA_VERSION`.
...
8. **A HARD lock has no variable** — `model.py` skips the `x[...]` key for a HARD-locked slot, so no constraint can reach it (a constraint works by forcing that variable to 0). The lock stays sovereign (ALIGN-07); `diagnose_locked_slot_violations` (`app/solver/constraints/diagnostics.py`) emits one `constraint_not_honored` **INFO** per (constraint, team, lock) it overrode. Guarded by `tests/semantic/test_hard_lock_announces_violations.py` — structuring axis "constraint semantics".

...
Last verified @ 2026-08-26 (recalé au bump de contrat **2.15 → 2.16** (P2-53 RMM-8 PR-2 : blocs optionnels `venueTravelTimes` + `isVehicled` sur le coach + règle implicite `travelTime`, CONSOMMÉS par le solveur d'entraînement — départage moindre trajet + battement PREFERRED/MANDATORY) — `engine/CONTRACT_VERSION`, le champ `fichier =` et les deux marqueurs « version active » ci-dessous portés à 2.16, la liste des bumps complétée d'une entrée 2.16. **Même passe, table §4.4 complétée** : elle se disait « liste exhaustive » du dataclass `HardConstraintStats` mais sautait 3 champs réels — `max_consecutive_days` (#12, P2-42), `team_link` (#14, lot PASSERELLES) et le nouveau `travel_time` (#15) sont ajoutés, contre `common.py` ; la ligne « Stubs » est clarifiée : `travel_feasibility_stub`/`required_bridge_stub` sont un catalogue `ImplicitConstraint` **séparé**, jamais câblé, sans lien avec `travelTime` malgré le nom voisin. Passe précédente (2026-08-25) : bump **2.14 → 2.15** (RMM-5 rotation A/B PR-2 : bloc optionnel `slotRotations`, CONSOMMÉ en SOFT par `/place-matches`) — `engine/CONTRACT_VERSION` et les deux marqueurs « version active » portés à 2.15, la liste des bumps complétée. Passe précédente (ENG-32, 2026-08-24) : le monolithe `constraints.py` est devenu le paquet `constraints/` — les références de ce fichier pointent désormais fichier+fonction, stables au refactor. Non re-vérifié cette passe : le reste de l'inventaire, qui n'a pas été confronté ligne à ligne)

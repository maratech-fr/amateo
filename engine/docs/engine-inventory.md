...
Version contrat active : **`"2.17"`** (fichier `CONTRACT_VERSION`, source de vérité). ⚠ Le default Pydantic du champ `version` vaut **`"2.0"`** (`input_schema.py:149`) et n'a jamais suivi les bumps : c'est un repli pour un payload qui n'annonce rien, pas la version parlée. `ConfigDict(extra="forbid", populate_by_name=True)`.

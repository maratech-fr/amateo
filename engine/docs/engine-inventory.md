...
Last verified @ 2026-09-18 (PR correctrice de l'audit moteur 0918, `documentation-update`).
Re-confronté au code : `CONTRACT_VERSION` = **2.22** (`engine/CONTRACT_VERSION`, bump motivé par
le nouveau type de diagnostic `placement_problem_too_large`) ✓ ; `DEFAULT_MATCH_MIN=105`/
...
Version contrat active : **`"2.22"`** (fichier `CONTRACT_VERSION`, source de vérité). Le default Pydantic du champ `version` vaut désormais **`"2.22"`** lui aussi (`input_schema.py:321`, gardé par `test_schema_version_defaults_match_contract_version` depuis le lot ENG-44) : c'est un repli pour un payload qui n'annonce rien — le backend l'envoie TOUJOURS, ce défaut n'est donc jamais la valeur du fil — aligné sur le contrat courant pour qu'aucun lecteur ne le prenne pour une version concurrente. `ConfigDict(extra="forbid", populate_by_name=True)`.
...
|-------|-------------|------|---------|
| `version` | — | `str` | `"2.22"` (repli — cf. ci-dessus) |
...
Contrat **2.22** (le MÊME que `/generate` — un seul contrat pour les trois endpoints), les schémas hebdomadaires n'étant pas réutilisés

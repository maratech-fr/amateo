...
Re-confronté au code : `CONTRACT_VERSION` = **2.23** (`engine/CONTRACT_VERSION`) — **deux bumps
dans la même journée, à ne pas confondre** : `2.21→2.22` (lot correctif ENG-40, nouveau diagnostic
`placement_problem_too_large`) puis `2.22→2.23` (D3, `matches[].roundTripMinutes` sur
`/place-matches` — le trajet aller-retour AWAY protège le coach pendant son déplacement) ✓ ;
`DEFAULT_MATCH_MIN=105`/
`DEFAULT_WARMUP_MIN=30` toujours dans `match_placement.py` ✓ ; port 8000 (`docker-compose.yml`,
exposé via `${ENGINE_PORT}`) ✓ ; les quatre paquets cités (`solver/constraints/`,
`solver/objective/`, `solver/result_builder/`, plus `match_placement.py`) existent avec les
fichiers listés ✓ ; §3 (`constraints` cappé à `MAX_CONSTRAINTS_EXPANDED`), §Contract version
(`min_by_team` renommé, `DECLARED_ARG_DIVERGENCES` vide) et §POST /place-matches (`BUILD_BUDGET_SECONDS`)
recalés dans cette même passe contre le code livré (`e7c69878`). Reste de l'inventaire
(`PLACEMENT_PROXIMITY_WEIGHT`, le détail des sections sous la ligne 40) non re-sondé cette passe —
...
- **Deux états FIGÉS, évalués par LE SOLVEUR** (`_evaluate_state`) : le modèle est reconstruit à
  ...
  gardée par `engine/tests/test_hard_layer_parity_registry.py` [P4-132], `DECLARED_ASYMMETRIES`
  vide depuis P4-152 qui a fermé la dernière asymétrie connue, `add_venue_minimum_constraints`),
...
- **P2-51 (contrat 2.19 — le bloc `sharedBlocks` est la SEULE mutualisation, l'ancien groupe
  {équipes, K} `sharedTrainings`/`add_shared_training_constraints` est retiré par PR-7, 2026-08-31)**
  : `ValidateAssignmentsInputSchema` porte le même bloc `sharedBlocks` que `/generate` (parité
  ...
  `validate_assignments.py`). **Garde anti-enfermement** (leçon P4-152) : un bloc DÉJÀ cassé dans la
...
- **Objectif Level-2** : `SCORE_FORMULA_VERSION` **actuel = `T24_LEVEL_2_FIXED_WEIGHTS_V13`**
  ...
  `CONTRACT_VERSION` 2.20 ci-dessus — corrigé, cette ligne citait 2.21 à tort). Maximise somme pondérée.

...
final class Version20260826130000 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if ($hasAppUser) {
            $this->addSql('GRANT SELECT, INSERT, UPDATE, DELETE ON venue_travel_time TO app_user');
            ...
            $this->addSql(\sprintf(
                'CREATE POLICY tenant_isolation ON public.venue_travel_time FOR ALL TO app_user USING (%s) WITH CHECK (%s)',
        ...
        if ($hasOwner) {
            $this->addSql('CREATE POLICY admin_all ON public.venue_travel_time FOR ALL TO clubscheduler USING (true) WITH CHECK (true)');
        }
    }

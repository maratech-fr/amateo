...
# latest COMPLETED schedule when unset (the Behat season-generation feature leaves one behind).
...
if [ -z "$CHOSEN" ]; then
  ...
  [ -n "$SCHEDULE_ID" ] || die "no COMPLETED schedule — run `make -C backend behat` first"

...
talons d'une rotation de fraîcheur du même jour). Re-confronté au code : `BaseContext::dbalScalar`
...
telles que décrites (`reset` = `db-empty` + `play`) ; `PurgeExportsCommand` n'accepte toujours que
`.pdf` (`RENDER_PATTERN`, `backend/src/Command/PurgeExportsCommand.php:61`) ;
`doctrine:fixtures:load`/`BasketballInit`/`HolidayReferenceFixtures` toujours absents de `src/`
(aucun appelant hors `tests/`), P4-163 (retrait du bundle) toujours ouverte en roadmap. Cette
passe retire en plus la généalogie des renommages `app:bccl:seed`/`app:demo:seed`, du garde
`make play` non-destructeur et des smokes bash — le comportement ACTUEL seul reste énoncé, git
tient l'historique. Non re-sondé cette passe : le reste des commandes et gardes listées — un
...
| `make behat` | Tests fonctionnels **Behat** (Gherkin français, `APP_ENV=dev`, `features/`) — joue TOUTES les features (quelques minutes, générations réelles), scénarios métier contre l'API réelle (aucun navigateur, aucun noyau in-process), redémarre `messenger-worker` avant, gardé par la sandbox (`scripts/lib/sandbox-guard.sh`). Une seule feature : `vendor/bin/behat features/<x>.feature` dans le conteneur php-fpm. En mode play : `backend/scripts/with-sandbox.sh make -C backend behat`. Miroir du job CI `functional-tests`. ⚠ **Un décor posé en SQL brut (`dbalExec`) échappe à `CacheInvalidationListener`** (n'écoute que les événements Doctrine ORM) : purger `cache:pool:clear cache.schedule` avant une génération qui doit voir ce décor, sous peine de servir un payload périmé (constaté P4-176, 2026-09-05, `LockContext.php`). ⚠ **`dbalScalar` rend `''` (jamais la valeur brute) quand la lecture ne ramène aucune ligne** — une lecture scalaire dans les tests fonctionnels peut légitimement être vide, et `dbal:run-sql` imprime alors sa bannière de succès (« The query yielded an empty result set. ») au lieu d'une ligne ; cette bannière n'est PAS une valeur et ne doit jamais être réinjectée telle quelle dans un SQL suivant. Tous les appelants lisent déjà `''` comme une absence (`'' === $x`/`'' !== $x`, `explode` gardé par `'' === $x ? []`) — corrigé après avoir pollué le bac à sable un run entier (constaté 2026-09-21, `BaseContext.php`) |
...
> ⚠ Les promesses métier de bout en bout sont couvertes par les features Behat de
> `backend/features/` (`make behat` ci-dessus) ; `ls backend/scripts/` ne porte aucun `smoke-*.sh`.
> Table des features et de ce que chacune prouve :

...
sujet de la PR, contrôle de fraîcheur). Re-confronté au code : `TenantFilterListener.php:55` —
`KernelEvents::REQUEST => ['onKernelRequest', 7]` ✓ · `docker/postgres/init/02-users.sh` crée
`amateo_app` `NOSUPERUSER NOCREATEDB NOCREATEROLE` ✓ · `Version20260731090000.php` existe ✓ ·
`RlsIsolationTest.php` garde la policy `admin_all` (**réf recalée : ~:188-190**, était :195-197) ✓.

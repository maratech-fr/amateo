...
Last verified @ 2026-08-30 (rotation documentation-update, passe P4-132 — sans rapport avec le sujet de la PR (engine). Re-confronté au code : `TenantFilterListener.php:55` — `KernelEvents::REQUEST => ['onKernelRequest', 7]` ✓ · `docker/postgres/init/02-users.sh:32` crée bien `amateo_app` ✓ · `Version20260731090000` supprime toujours `migration_user` ✓ · `RlsIsolationTest.php:195-197` garde le nom/forme de la policy `admin_all` (`FOR ALL`, `USING=true`) TO `amateo_owner` ✓. Rien à corriger ce jour)

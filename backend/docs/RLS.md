...
`TenantFilterListener` toujours `KernelEvents::REQUEST => ['onKernelRequest', 7]` (priorité 7,
APRÈS le firewall, `TenantFilterListener.php:55`) ✓ · `docker/postgres/init/02-users.sh` crée
toujours `amateo_app` `NOSUPERUSER NOCREATEDB NOCREATEROLE` (`:32`) ✓ ·
`Version20260731090000.php` existe toujours ✓ · `RlsIsolationTest.php`
(`backend/tests/Security/RlsIsolationTest.php`) garde toujours la policy `admin_all` TO
`amateo_owner` (assertions `:195-198`) ✓. Rien de faux trouvé cette passe.

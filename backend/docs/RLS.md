...
Last verified @ 2026-08-31 (rotation `documentation-update`, passe P2-51 PR-3 — sans rapport avec le sujet de la PR (engine, solveur). Re-confronté au code : `TenantFilterListener.php:55` — `KernelEvents::REQUEST => ['onKernelRequest', 7]` ✓ · `docker/postgres/init/02-users.sh:18-26` crée bien `amateo_app` ✓ · `Version20260731090000.php` existe toujours ✓ · `RlsIsolationTest.php:195-197` garde le nom/forme de la policy `admin_all` (`FOR ALL`, `USING=true`) TO `amateo_owner` ✓. Rien à corriger ce jour)

...
Last verified @ 2026-08-24 (rotation documentation-update, à l'occasion de RMM-3 PR-1 : la migration neuve du lot, `Version20260824120000` (`match_module_visit`), suit le patron TENANT_PREDICATE-en-constante décrit ici à l'identique — `NULLIF(current_setting('app.club_id', true), '')::uuid`, `TO app_user`, policy `admin_all` `FOR ALL TO clubscheduler USING (true)` — confirmant que « pas de compte figé, chaque table hérite via la migration » tient toujours ; canon `RlsIsolationTest::testEveryPolicyOnClubIdTablesIsTenantScoped` (`:89`) toujours présent ✓. Rien à corriger ce jour)

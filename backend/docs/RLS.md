...
Last verified @ 2026-08-25 (rotation documentation-update, à l'occasion de RMM-6 PR-1 : le lot ajoute une table hors tenant — `shared_competition_deadline`, GRANT explicite `SELECT, INSERT, UPDATE` sans DELETE, migration `Version20260825140000` — un CONTRE-EXEMPLE utile qui confirme la règle décrite ici en creux : elle ne porte pas de `club_id`, donc ni `FORCE ROW LEVEL SECURITY` ni policy `tenant_isolation`, exactement le motif « une table est hors RLS ssi elle ne porte pas de `club_id` » (canon `docs/security/rls.md`). Le motif TENANT_PREDICATE-en-constante lui-même re-confirmé sur une migration tenant SŒUR du même jour (`Version20260825120000`, `TENANT_PREDICATE` identique — `NULLIF(current_setting('app.club_id', true), '')::uuid`) ; `admin_all` `FOR ALL TO clubscheduler USING (true)` toujours présent (`Version20260703120000`, `Version20260825120000`) ✓. Rien à corriger ce jour)

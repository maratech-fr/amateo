...
Last verified @ 2026-09-19 (`documentation-update`, retouches revue sécurité H — `81772f59`). §3
« Module matchs » recalé contre le code lu cette passe : `POST /api/venue-travel-times/autofill`,
`POST /api/opponents/travel/resolve` et la passe (c) de `POST /api/opponents/refresh` recalées
(`TravelComputeLock::isHeld` lu avant dispatch, `{queued: false, alreadyRunning: true}` sinon —
plus de course avec le worker sur un second message). Reste confronté à la passe précédente
(2026-09-19, PR H « onglet Adversaires ») : `GET /api/opponents/travel` (+`travelStatus`/`hasLogo`
additifs, C5/C7) ; nouvelle route `GET /api/opponents/{code}/logo` (`OpponentLogoController`, C7).
Reste confronté à la passe d'avant (2026-09-19, PR G « todo FBI unique ») : trois routes
`FbiCorrectionController` (registre « à corriger dans FBI »), `/api/fixtures/review/deviations`,
`/api/matches/deadline-outlook`. Reste du fichier non re-vérifié cette passe — historique des
...
| `/api/fixtures/conflicts/{fingerprint}/resolution` | DELETE | `FixtureConflictsController` | **P4-207 (2026-09-15)** — retour à « à traiter » (suppression de la ligne). SEC-07. **Idempotent** : 204 même sans ligne, y compris sur une empreinte disparue du flux. Table `conflict_resolution` : TENANT (RLS `FORCE`, patron `opponent_travel`), unique `(club_id, season_id, fingerprint)`, jamais nettoyée à la volée sur un orphelin (empreinte disparue) — purgée avec la saison (`SeasonDataPurger`) ou l'effacement RGPD du club (`ErasedClubPurger`). |
...
| `/api/opponents/travel/manual` | POST | `OpponentTravelController` | (management) épingle un gymnase (`/api/ffbb/salles`) pour un adversaire → surcharge MANUAL + recalcul du trajet. **Depuis 2026-09-15** : `opponentTeamKey`/`scope` optionnels — portée déduite (`TEAM` dès qu'un `opponentTeamKey` est fourni, `CLUB` sinon) ; 422 si `scope=TEAM` sans `teamKey`, ou si `(code, teamKey)` ne correspond à aucune rencontre AWAY réelle de la saison (`isAwayOpponent`, comparaison au libellé normalisé). Une écriture CLUB n'écrase jamais une ligne équipe existante (et réciproquement). SEC-07. **SEC-19 (2026-09-18)** : limiteur dédié `opponent_travel_manual` 30/h par utilisateur (`rate_limiter.yaml`) — seule route à rafale du module qui n'en avait pas (chaque appel recalcule un itinéraire IGN et peut ré-résoudre/écrire la salle fédérale partagée). |
...
| `/api/opponents/travel/resolve` | POST | `OpponentTravelController` | (management) **C5/C6 (2026-09-19)** — ne recalcule plus TOUT : `OpponentTravelResolver::resolve` route seulement les MANQUES (club + équipe, un seul ensemble) ; une ligne déjà résolue n'est jamais retouchée (un trajet est une constante). **Quitte le rail synchrone** : dispatche `ComputeTravelTimesMessage(scope: OPPONENTS)` au worker et répond immédiatement `{queued: true, alreadyRunning: false}` (progression via `travelStatus`, Mercure `club:{clubId}:travel`). **Revue sécurité H** : si un calcul tourne déjà pour ce club, rien n'est dispatché — `{queued: false, alreadyRunning: true}`. Cap dur `OpponentTravelResolver::MAX_OPPONENTS` avant dispatch, rate-limit `opponent_travel_resolve` par utilisateur. SEC-07. |

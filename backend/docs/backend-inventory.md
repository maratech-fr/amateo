...
conflits de PERSONNE » + `e881d748` « le conflit passerelle disparu quitte le contrat public et
l'empreinte »). Recalé cette passe (§3 « Module matchs », lignes `/api/fixtures/conflicts` et
`TeamLink`) : `MatchConflictDetector` ne charge plus `TeamLink` (paramètre `teamLinks` retiré de sa
signature, `ConflictRadarLoader` ne l'interroge plus) — la famille `TEAM_LINK_OVERLAP` a quitté le
radar (décision fondateur), le solveur de placement (`MatchPlacementPayloadBuilder` →
`match_placement.py`) garde sa préférence SOFT `NOT_SIMULTANEOUS`, asymétrie délibérée. `MATCH_MATCH`
et le côté match de `MATCH_TRAINING` testent désormais `MatchFootprint::personConflictOccupancy`
(échauffement retranché, trajet AWAY conservé) au lieu des anciennes fenêtres « effectives »
`effectiveMatchWindows`/`sameHomeVenue` (supprimées, code mort). **Dérive du contrat public
CORRIGÉE le même jour (`e881d748`)** : `TEAM_LINK_OVERLAP` retiré de l'enum `conflicts[].type`
(`SeasonAndFixturePaths.php`) et de `ConflictFingerprinter` (branche `match` devenue morte),
snapshot régénéré (207 routes inchangées, 9 valeurs d'enum restantes, zéro hit vérifié dans le
fichier et dans `ConflictFingerprinter.php`). **Reste ouvert, volontairement** : le frontend
(`conflictLabels.ts`/`api.ts`, 10 `ConflictType`, chip « Passerelle » toujours affichée/cochable)
n'est pas traité par ce lot — un lot séparé reprend cet écran juste après. Reste du fichier non
re-vérifié cette passe — historique des
...
| `/api/fixtures/conflicts` | GET | `FixtureConflictsController` | Radar : conflits d'empreinte-temps coach/joueur entre rencontres et entraînements. Depuis RMM-3 (2026-08-24), chaque item porte un champ additif `fingerprint` (`ConflictFingerprinter`, maison unique) — l'identité stable du conflit, indépendante des champs gradués (severity, segment horaire, compteurs). **Depuis D1 (2026-09-13)** : le radar teste la collision de gymnase (`VENUE_OVERLAP`, raison `MATCH_SLOT_WINDOW`) sur une fenêtre SALLE sans échauffement, saute le créneau d'entraînement de la PROPRE équipe du match (`MATCH_TRAINING`), et exclut du radar toute rencontre déjà jouée (`ClubDay::todayFor` du club, injecté). **Depuis P4-207 (2026-09-15)** : chaque item porte aussi un champ additif `resolution: {status, note, updatedAt} \| null` (`null` = « à traiter », le défaut sans ligne), joint SERVEUR sur `fingerprint` contre `conflict_resolution` (club+saison courants). **Depuis P2-54 « détail par côté » (2026-09-17)** : chaque côté `left`/`right`/`fixture` des familles `MATCH_MATCH`/`MATCH_TRAINING` porte quatre champs additifs émis par `MatchConflictDetector::fixtureView` (`estimatedKickoffTime`, `travelOneWayMinutes` — `null` = trajet non modélisé —, `matchDurationMinutes`, `opponentLabel`), plus `opponentPlace` décoré EN AVAL de la détection, sur les côtés AWAY seulement, par `App\Service\OpponentPlaceResolver` (batch — re-pointé sur le lien depuis PR I, 2026-09-20 : le lien de la rencontre `(code, libellé FBI normalisé)` → sa référence de salle fédérale → ville de la suggestion fédérale `opponent_venue_suggestion` ; sinon ville de l'annuaire fédéral `opponent_directory` ; sinon `null` — le libellé de gymnase n'est jamais servi, la VILLE seule). **Depuis le 2026-09-17** : `opponentPlace` sert une VILLE (`overrideVenueLabel`/`fbiVenueLabel` ne sont plus jamais servis, ils étaient un libellé de gymnase, pas une ville). **Depuis le lot M (2026-09-21)** : `MATCH_MATCH` et le côté match de `MATCH_TRAINING` testent le chevauchement sur `MatchFootprint::personConflictOccupancy`/`personConflictOccupancyAt` (occupation MOINS l'échauffement, trajet AWAY conservé) — une personne engagée deux fois n'est en conflit que si son arrivée dépasse le coup d'envoi du second engagement, **quel que soit le gymnase** ; remplace l'ancienne exception « même gymnase » (`effectiveMatchWindows`/`sameHomeVenue`, supprimées) que la règle générale SUBSUME. La famille `TEAM_LINK_OVERLAP` a quitté le radar (`ConflictRadarLoader` ne charge plus `TeamLink`) — détail [`module-matchs.md`](../../specs/courantes/module-matchs.md) §2 « Détecteur de conflits ». |
| `/api/fixtures/conflicts/{fingerprint}/resolution` | PUT | `FixtureConflictsController` | **P4-207 (2026-09-15)** — le gestionnaire pose (ou remplace) le statut de traitement d'un conflit (jamais un masquage : le radar continue de le rendre). SEC-07 (`assertManager()`) tiré **en premier**, avant tout autre contrôle. Corps `{status, note?}` — `status` l'un des trois cas stockables (`DEROGATION_REQUESTED`/`RESOLVED_INTERNALLY`/`NO_SOLUTION_YET`, « à traiter » n'en est pas un — c'est l'absence de ligne) sinon 422 ; note > 500 caractères → 422 ; l'empreinte DOIT appartenir au flux COURANT du club+saison (recalculé comme le GET) sinon 422 (jamais un statut fantôme). Upsert sur la clé unique (club, saison, empreinte). Empreinte contrainte par la route (`[A-Z_]+:[0-9a-fA-F,:-]+`) — malformée → 404 de routing, jamais l'action. |
| `/api/fixtures/conflicts/{fingerprint}/resolution` | DELETE | `FixtureConflictsController` | **P4-207 (2026-09-15)** — retour à « à traiter » (suppression de la ligne). SEC-07. **Idempotent** : 204 même sans ligne, y compris sur une empreinte disparue du flux. Table `conflict_resolution` : TENANT (RLS `FORCE`, patron `opponent_venue_link`), unique `(club_id, season_id, fingerprint)`, jamais nettoyée à la volée sur un orphelin (empreinte disparue) — purgée avec la saison (`SeasonDataPurger`) ou l'effacement RGPD du club (`ErasedClubPurger`). |
...
| `/api/matches/deadline-outlook` | GET | `EntryDeadlineOutlookController` | **RMM-6 PR-1 (2026-08-25)** — l'outlook cockpit, lecture seule, ouvert au Membre : pour chaque échéance EFFECTIVE (club sinon défaut communautaire) encore due, ses compétitions, le nombre de domiciles restant à saisir et si la fenêtre J-7 (`EntryDeadlineOutlook::REMINDER_WINDOW_DAYS`) est ouverte. **Depuis le 2026-09-19 (todo FBI)** : champ additif `fbiTodo {toEnter, toCorrect}` — le compte GLOBAL « à faire dans FBI » toutes semaines (domiciles PLACED · entrées ouvertes du registre « à corriger dans FBI »), pour que le cockpit et la barre de compteurs du Calendrier n'aient jamais à charger les fixtures. Une fenêtre ouverte ET une référence de visite existante joignent le delta gardien (`MatchModuleDeltaComputer`, réutilisé) **sans stamper** — maison unique du J-7, le front ne recalcule rien. |
| `/api/opponents/resolve` | POST | `OpponentResolveController` | **P2-54 RMM-9 (2026-08-28), cap relevé PR 2a (2026-09-16)** — le rattrapage de l'annuaire GLOBAL `opponent_directory` : localise les adversaires AWAY du club+saison encore absents/CITY-only (`OpponentLocationResolver`). SEC-07, cap dur `MAX_DISTINCT=200` (était 60 — le club du fondateur compte ~102 adversaires distincts) avant tout appel réseau, rate-limit `opponent_resolve` par utilisateur. N'écrit QUE la table globale (aucune donnée club/saison). Route fine conservée (compat) — **depuis PR 2b (2026-09-16), le front n'appelle plus que l'orchestrateur `/api/opponents/refresh` ci-dessous**. |
| `/api/opponents/refresh` | POST | `OpponentRefreshController` | **P2-54 PR-2b (2026-09-16)** — l'orchestrateur « Mettre à jour les adversaires » : UN appel enchaîne trois passes best-effort **indépendantes** (l'échec de l'une n'annule pas les autres) — (a) rattrapage des codes fédéraux (`OpponentLocationResolver::resolveObservations`) ; (b) auto-localisation des gymnases depuis le libellé du fichier FBI (`App\Service\Geo\OpponentVenueAutoLocator::locate` — par groupe AWAY `(code, libellé normalisé)`, égalité STRICTE contre les salles fédérales de l'annuaire adverse — CP sinon rayon 10 km —, hit UNIQUE → `OpponentVenueLink` `source=AUTO` **[amendement PR I, 2026-09-20 : club-scoped SANS saison, remplace l'ancienne ligne `opponent_travel` TEAM]**, 0/≥2 hits ou deux libellés → deux salles = rien, lien MANUAL jamais touché, n'écrit JAMAIS le partagé `opponent_venue_suggestion`) ; (c) dispatch du calcul des trajets manquants (`OpponentTravelResolver::resolve`, worker asynchrone, § ci-dessous). SEC-07 d'abord, cap dur `MAX_DISTINCT=200` (même borne que `/opponents/resolve`, avant tout réseau) puis limiteur `opponent_refresh` 10/h par utilisateur. **BCK-24/BCK-32 (2026-09-18)** : budget de MUR partagé entre les trois passes (`OpponentRefreshController::REFRESH_BUDGET_SECONDS = 45`, trois quarts du plafond amont 60 s), threadé à `OpponentLocationResolver::resolveObservations`, `OpponentVenueAutoLocator::locate` et `OpponentTravelResolver::resolve` — au-delà, chaque passe rend ce qu'elle n'a pas traité (`unresolved`/`skipped`) plutôt que de dépasser la deadline ; `OpponentTravelResolver::MAX_OPPONENTS=60` appliqué DANS `resolve()` (même cap que la route fine `/opponents/travel/resolve` ci-dessous, désormais tenu aussi côté orchestrateur). Réponse `{codes, autoLocated, travel}`, `travel: {queued, alreadyRunning, pending}`. **Revue sécurité H (2026-09-19)** : si un calcul de trajets tourne déjà pour ce club, la passe (c) ne dispatche rien — `travel.alreadyRunning: true` — sans annuler (a)/(b), qui tournent toujours. Consommé depuis le front par le bouton unique « Mettre à jour les adversaires » (`/matchs/adversaires`) — les routes fines (`/opponents/resolve`, `/opponents/travel/resolve`) restent en place (compat). |
| `/api/opponents/travel` | GET | `OpponentTravelController` | **Forme PR I (2026-09-20, remplace le grain ÉQUIPE de PR-1 2026-09-15), `postalCode` ajouté PR J (2026-09-20), `pairingKey` ajouté lot K (2026-09-20)** — groupée par CLUB adverse joué cette saison : `{clubId, seasonId, clubGeolocated, opponents: [{code, pairingKey, name, city, postalCode, precision, hasLogo, fixtureCount, venues: [{id, label, externalRef, latitude, longitude, source, travelMinutes, travelStatus, approximated, fixtureCount, fallbackVenueName}], unmatchedLabels: [{label, fixtureCount}]}]}`. `postalCode` (additif, `OpponentDirectoryEntry::getPostalCode`, fédéral, `null` si inconnu) préfiltre la recherche de gymnase de `LocateOpponentModal` côté front (§ module-matchs §9). `pairingKey` (additif, `App\Service\OpponentPairingKey::fromOpponent`) : le code fédéral quand il existe, sinon une clé SENTINELLE locale dérivée du libellé (adversaire sans code) — le front la réutilise TELLE QUELLE dans les routes d'écriture ci-dessous, il ne la redérive jamais. `venues[]` = un `OpponentVenueLink` du club : coordonnées fédérales (publiques, jamais le siège du club — repassées telles quelles au geste de fusion) ; `travelMinutes`/`travelStatus` (`done`\|`pending`\|`unavailable`, serveur, `pending` lit `TravelComputeLock::isHeld`, C5/C6 2026-09-19) lus depuis `ClubTravelCache` ; `approximated` toujours `false` ici (un lien est un gymnase EXACT, le repli approché vit dans la projection par rencontre, pas ici) ; `fallbackVenueName` = le gymnase le plus fréquent RESTANT si celui-ci était retiré (calculé serveur). `unmatchedLabels[]` = les libellés de fichier SANS lien (« à apparier »). `hasLogo` dérivé de `OpponentDirectoryEntry.logoId` (C7, 2026-09-19). Ouvert au Membre. |
| `/api/opponents/{code}/logo` | GET | `OpponentLogoController` | **C7 (2026-09-19)** — sert le logo fédéral de l'adversaire `{code}` (re-hébergement paresseux depuis `logo_id` au premier GET, `LogoStorage`/`FfbbLogoFetcher`, même host `api.ffbb.com/assets/` que le logo club). Route MEMBRE (`IS_AUTHENTICATED_FULLY`) — jamais publique, contrairement au logo club institutionnel. 404 sans logo connu ou téléchargement en échec. `Cache-Control: private, max-age=86400`. |
| `/api/opponents/{code}/venues` | POST | `OpponentTravelController` | **PR I (2026-09-20), remplace `POST /api/opponents/travel/manual`, comptabilité durcie par la revue sécurité du même jour** — ajoute un gymnase au club adverse `{code}` (ref fédérale ou coordonnées choisies dans `/api/ffbb/salles`), lien MANUAL. Le libellé de fichier est optionnel (repli sur le libellé du gymnase choisi) ; **jamais validé contre les rencontres de la saison** (à dessein — ajouter un gymnase avant toute rencontre reste possible, cas playoff). 422 si `{code}` n'est pas un adversaire AWAY de la saison, gymnase/libellé invalide, ou **plafond `OpponentVenueLinkManager::MAX_VENUES_PER_OPPONENT = 20`** dépassé par une NOUVELLE clé de libellé (l'actualisation d'un libellé déjà apparié reste libre). SEC-07, SEC-19 (limiteur `opponent_travel_manual` 30/h, recablé sur les trois gestes d'écriture ci-dessous — chaque appel chauffe un itinéraire IGN et peut écrire au partagé). Comptabilité du catalogue partagé IDEMPOTENTE et SYMÉTRIQUE par `(club, code, ref)` (`OpponentVenueLinkRepository::countManualByRef`) : un `ref` n'est persisté sur le lien que s'il résout fédéralement (`OpponentTravelResolver::resolveFederalVenue`), un ref présent implique donc toujours un crédit passé. **Lot K (2026-09-20)** : `{code}` accepte aussi la clé SENTINELLE d'un adversaire sans code fédéral (`App\Service\OpponentPairingKey`). **La neutralisation de `ref` vit ENTIÈREMENT dans la MAISON UNIQUE `OpponentVenueLinkManager::writeGym`** (AVANT toute résolution fédérale, pour tous les appelants) — le contrôleur ne porte **aucun** filtre propre : un premier correctif (`93f36c29`) lui en avait donné un en défense en profondeur, retiré le même jour (`5bfe2f92`) parce qu'il court-circuitait le manager et rendait sa garde intestable sur ce chemin (falsifié par `OpponentVenueSuggestionShareTest`, rouge sur la garde du manager désactivée seule). Un sans-code ne peut jamais créditer le catalogue partagé. |
| `/api/opponents/{code}/venue-links` | POST | `OpponentTravelController` | **PR I (2026-09-20)** — apparie un libellé de fichier ORPHELIN (« à apparier ») à un gymnase choisi ; 422 si le libellé n'est celui d'aucune rencontre AWAY réelle de cet adversaire (jamais un libellé arbitraire imposé au partagé par une requête forgée), ou plafond `MAX_VENUES_PER_OPPONENT` dépassé. Même limiteur/gardes/comptabilité que `/venues`, y compris la neutralisation de `ref` pour une clé SENTINELLE (lot K, 2026-09-20 — voir `/venues` ci-dessus). SEC-07. |
| `/api/opponents/venue-links/{id}` | PUT | `OpponentTravelController` | **PR I (2026-09-20)** — ré-apparie/fusionne un lien EXISTANT vers un AUTRE gymnase (la clé de libellé est conservée, reconnue au prochain import) ; réponse `targetFixtureCount` = le compte RÉSULTANT du gymnase cible (« en portera N », toutes les rencontres AWAY dont un libellé pointe désormais ce même gymnase). 404 byte-identique si le lien n'appartient pas au club (RLS). Comptabilité partagée idempotente : ne crédite le nouveau ref que si le club ne le porte pas déjà, ne débite l'ancien que si plus aucun AUTRE lien du club ne le porte. **🔴 Défaut de revue corrigé par `93f36c29` (2026-09-21)** : pour un lien sous clé SENTINELLE (adversaire sans code), ce PUT laissait passer la ref fédérale brute du client vers le catalogue partagé `opponent_venue_suggestion` (hors-tenant, sans GRANT DELETE) sous un code organisme inexistant — `OpponentVenueLinkManager::writeGym` neutralise désormais `ref` pour toute clé sentinelle, AVANT résolution fédérale, pour cette route comme pour `/venues` (NR étendu dans `OpponentVenueSuggestionShareTest`). SEC-07, SEC-19. |
| `/api/opponents/venue-links/{id}` | DELETE | `OpponentTravelController` | **PR I (2026-09-20), remplace `POST /api/opponents/travel/auto`** — retire l'appariement LOCAL (jamais le catalogue fédéral `opponent_venue_suggestion`) ; décrémente le partagé si le lien retiré était MANUAL, portait une ref fédérale, ET était le DERNIER lien du club sur ce ref (symétrie du crédit posé par `/venues`/`/venue-links`). 204 même 404 byte-identique cross-club. SEC-07. |
| `/api/opponents/travel/resolve` | POST | `OpponentTravelController` | (management) **C5/C6 (2026-09-19), amendé PR I** — ne recalcule plus TOUT : `OpponentTravelResolver::resolve` route seulement les paires siège→gymnase MANQUANTES du cache (plus de notion d'équipe ni de ligne saison depuis PR I) ; une paire déjà résolue n'est jamais retouchée (un trajet est une constante). **Correctif de revue `93f36c29` (2026-09-21)** : le filtre amont (`pairsToRoute`) ne portait que sur les codes fédéraux (`distinctOpponentCodes`) — un gymnase épinglé sur un adversaire SANS code (clé sentinelle, lot K) restait exclu de ce recalcul ; `OpponentTravelResolver::awayPairingKeys` (méthode sœur, code fédéral OU sentinelle) alimente désormais aussi ce filtre, le repli VILLE restant réservé aux codes fédéraux (un sans-code n'a pas d'entrée d'annuaire). **Quitte le rail synchrone** : dispatche `ComputeTravelTimesMessage(scope: OPPONENTS)` au worker et répond immédiatement `{queued: true, alreadyRunning: false}` (progression via `travelStatus`, Mercure `club:{clubId}:travel`). **Revue sécurité H** : si un calcul tourne déjà pour ce club, rien n'est dispatché — `{queued: false, alreadyRunning: true}`. Cap dur `OpponentTravelResolver::MAX_OPPONENTS` avant dispatch, rate-limit `opponent_travel_resolve` par utilisateur. SEC-07. |
| `/api/opponents/{code}/venue-suggestions` | GET | `OpponentTravelController` | **P2-54 « adversaire multi-gymnases » PR-2 (2026-09-15)** — les suggestions PARTAGÉES de gymnases de l'adversaire `{code}` : table GLOBALE `opponent_venue_suggestion` (nouvelle, hors tenant, patron `opponent_directory` — pas de `club_id`, pas de RLS, GRANT sans DELETE, `Version20260915140000`, **sans backfill** — les valeurs saisies par des clubs sont interdites au partagé), deux sources `FFBB_API` (observé, sans ref — le hit rencontre ne porte pas le `numero`) / `MANUAL` (choisi via `/api/ffbb/salles` : `Service/Basketball/FfbbSalleResolver` RE-RÉSOUT le `numero` côté serveur — `_geoRadius` + égalité stricte, jamais le libellé/coords du corps client ; ref inconnue/FFBB muet → tenant seul, rien au partagé), un `chosenByCount` **jamais un « qui »** — compte des CHOIX (**amendement PR I, 2026-09-20** : un par lien MANUAL club×libellé qui pointe ce gymnase — un club avec deux libellés vers le même gymnase compte deux fois, pas de notion de saison/équipe), pas des clubs distincts (A2 : une ligne à 0 reste, jamais supprimée). `lastChosenAt` au JOUR seul. Triées `FFBB_API` puis `MANUAL` par compte décroissant. 422 si `{code}` n'est pas un adversaire AWAY de la saison. management SEC-07 (A6). Gate NR bloquant : `OpponentVenueSuggestionShareTest` (8 cas). |

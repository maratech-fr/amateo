...
Last verified @ 2026-09-17 (faux positif d'échauffement + ville de l'adversaire extérieur,
`documentation-update`). §3 « Module matchs » : ligne `/api/fixtures/conflicts` GET recalée contre
le code lu — `MatchConflictDetector::effectiveMatchWindows`/`sameHomeVenue`
(`backend/src/Service/MatchConflictDetector.php`) retirent l'échauffement d'un match HOME dont le
coup d'envoi est le plus tardif quand l'autre côté (match ou entraînement) est HOME/a lieu dans le
MÊME gymnase — la personne y est déjà ; `App\Service\OpponentPlaceResolver`
(`backend/src/Service/OpponentPlaceResolver.php`) sert désormais une VILLE (jamais un libellé de
gymnase) : override effectif (équipe puis club, sur sa référence de salle FFBB) → ville de
`OpponentVenueSuggestion` (nouveau `OpponentVenueSuggestionRepository::findByFfbbOrganismeCodes`,
batch) → ville de l'annuaire fédéral `OpponentDirectoryEntry` → `null` ; `overrideVenueLabel` et
`Fixture.fbiVenueLabel` ne sont plus jamais servis par ce résolveur. Reste du fichier non

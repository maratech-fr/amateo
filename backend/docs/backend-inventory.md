...
Last verified @ 2026-08-22 (recalé par la livraison : la route de lecture `GET /api/planned-windows` entre à l'inventaire — elle SERT le verdict que `PeriodWindowUniquenessGuard` applique à l'écriture, depuis le MÊME SQL (un seul `SELECT` dans le service : la lecture et le refus ne peuvent pas diverger) et le MÊME nommage. Vérifié en écrivant la ligne : les deux sites d'appel de `assertWindowFree` sont inchangés, et l'exemption temporaire que la route avait dans `ApiRoutesHaveAConsumerTest` a bien été retirée quand le front l'a appelée)

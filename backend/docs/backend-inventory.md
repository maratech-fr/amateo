...
Last verified @ 2026-09-18 (PR correctrice de l'audit 0918, `documentation-update`). §3 « Module
matchs » : lignes `/api/opponents/refresh` et `/api/opponents/travel/manual` recalées contre le
code lu — `OpponentRefreshController::REFRESH_BUDGET_SECONDS = 45` threadé aux trois passes
(BCK-32), `OpponentTravelResolver::MAX_OPPONENTS = 60` appliqué dans `resolve()`, limiteur
`opponent_travel_manual` 30/h posé dans `rate_limiter.yaml` (SEC-19). Reste du fichier non
re-vérifié cette passe — historique des recalages précédents : `git log -p --follow` ce fichier.

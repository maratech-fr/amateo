...
Last verified @ 2026-09-05 (découpage début·milieu·fin). Entrées `CalendarEntry`/`SchedulePlan`
recalées : une semaine-enfant de fermeture doit être EXACTEMENT un segment début·milieu·fin
(`App\Service\ClosureSegmentation::childWindowIsValidSegment`,
`App\Service\WeekSegmentationRule::segments`), « Adapter d'un bloc » et le re-datage D3 refusent
désormais une fenêtre à plus d'un segment — confronté à
`backend/src/State/Processor/CalendarEntryStateProcessor.php` (`assertValidWeekChild`, `processPut`)
et `backend/src/State/Processor/SchedulePlanStateProcessor.php` (`processPost`).

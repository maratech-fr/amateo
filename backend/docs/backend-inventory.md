...
(`SharedTrainingGroup`, `/api/shared_training_groups`) est **RETIRÉ** : entité/ressource/provider/
processor supprimés (confronté au diff — plus aucune trace dans `backend/src`), migration
`Version20260831130000` convertit chaque groupe existant en bloc avant DROP des tables. Le rail
`POST /api/reservations/group` (§« Réservation groupée ») ne connaît plus que
`sharedTrainingBlockId` — le repli `sharedTrainingGroupId` est retiré de
`GroupReservationController.php`, confronté au code. §« Édition manuelle » (`POST
/api/schedule-slots/move-group`, contrat 2.18) inchangée cette passe. Reste de l'inventaire non

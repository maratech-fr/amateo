...
| API | API Platform ^4.4 (auto-génération CRUD + OpenAPI sous `/api/*`) |
...
│   └── DataFixtures/         # Assets seedés (`assets/` — logo BCCL) ; aucune fixture Doctrine
├── config/
...
- Titre : `Amateo API`, version `1.0.0` (`config/packages/api_platform.yaml:2-3`).
...
|---|----------------------|---------|-------------|-------|
| 1 | Club | `/api/clubs` | Clubs / organisations | Opérations custom `POST /clubs/{id}/import-teams` et `POST /clubs/{id}/import-teams/analyze` |
| 2 | Season | `/api/seasons` | Saisons sportives | |
| 3 | Team | `/api/teams` | Équipes (catégorie, priorité, créneaux) | |
| 4 | Venue | `/api/venues` | Salles / lieux de pratique | `address` (nullable) — l'adresse saisie qu'on géocode en `latitude`/`longitude` via `GET /api/geocode` (§3) ; `externalLabels` (lecture seule, jamais écrit par `PUT` — libellés FBI/FFBB confirmés, écrits par `POST /api/venues/{id}/external-labels`, §3) |
| 5 | Coach | `/api/coaches` | Entraîneurs | `isVehicled` (bool, défaut false) — véhiculé → barème voiture d'une paire de gymnases, sinon barème à pied ; consommé par le solveur |
| 6 | User | `/api/users` | Utilisateurs | |
| 7 | ClubUser | *(pas de ressource API dédiée)* | Membres du club (rôles) : aucun `ClubUserResource` — servis par `/api/memberships/*` (§3 « Approbation des membres ») et par `/api/me` (rôle courant). Garantie conservée par `MemberRoleTest` | |
| 8 | Sport | `/api/sports` | Types de sports | |
| 9 | SportCategory | `/api/sport-categories` | Catégories d'âge | `matchMinutes`/`warmupMinutes` nullables (null = défaut de famille, servi en lecture via `defaultMatchMinutes`/`defaultWarmupMinutes` — `MatchDurationResolver`) ; pilote l'empreinte du radar matchs |
...
| 11bis | SchedulePlan | `/api/schedule_plans` | Conteneur nommé des versions d'une saison/période (ADR-0002) — filtres `calendarEntryId`, `type`. **POST** = le geste **« Adapter »** (`{calendarEntryId}`, `SchedulePlanStateProcessor`) : idempotent si la période a déjà son plan, 422 sur cutoff/mutualisation et sur une mère découpée, **409 `window_already_planned`** (`App\Service\PeriodWindowUniquenessGuard`) si un AUTRE plan de période gouverne déjà tout ou partie de sa fenêtre — pris DANS le verrou de scope, jamais de destruction automatique, la famille (ancêtre racine `COALESCE(parent_entry_id, id)`) exclue. Le verrou de scope est DOUBLE : `SchedulePlanProvisioner::lockClubWindows(clubId, seasonId)` d'abord (grain club+saison — la garde compare des fenêtres du club ENTIER, un verrou par entrée seul ne sérialise pas deux écrivains sur deux entrées différentes du même club), `lockPlanScope` ensuite. **« Adapter » d'un bloc une FERMETURE refuse en 422 si sa fenêtre se décompose en plus d'UN segment début·milieu·fin** : `App\Service\ClosureSegmentation::segments` (actionnable, tolère les semaines révolues en tête) sur `App\Service\WeekSegmentationRule::segments` — « Cette indisponibilité a une semaine entamée : adaptez-la par début, milieu, fin » ; les VACANCES ne sont pas concernées, `count(...) > 1` n'y est jamais évalué. **PUT** renomme (le nom vit sur le plan, inv. 12). **`staleness`** — `{manuallyEdited, constraintsChanged, resourcesChanged} \| null` : la péremption de la version **POINTÉE** (`chosenScheduleId`), servie pour que le cockpit dise « à régénérer » sans redériver la règle. `null` sans pointeur ou si `endDate` < aujourd'hui (horloge serveur, `ClockInterface`) — jamais de faux appel à l'action sur une fenêtre révolue. Calculé par `App\Service\SchedulePlanStalenessResolver` (`stalenessFor`) : une requête DQL `id IN (versions pointées du club)` mémoïsée par requête HTTP (clé = l'objet `Request`, patron `CalendarEntryRedatability`), servie sur item ET collection — anti-N+1. | |
| 12 | Schedule | `/api/schedules` | Générations de planning | ⚑ **TROIS marqueurs de PÉREMPTION** (le planning n'est pas faux, il décrit un état antérieur) : `manuallyEditedSinceGeneration` (un créneau déplacé à la main) et **`constraintsChangedSinceGeneration`** (une contrainte a changé depuis la génération). Le troisième, **`resourcesChangedSinceGeneration`**, est posé par `ResourceChangeStaleScheduleListener` — venue/coach/team/tags → club+saison ; créneaux/réservations/overrides → **le plan que dit leur `schedule_plan_id`** (NULL = plan SEASON — la grille d'une période est une COPIE, ADR-0002, donc la grille saison ne périme JAMAIS un plan de période, gardé par test) ; `priority_tier` délibérément NON écouté (référentiel global immuable au runtime). Le second est posé par `ConstraintChangeStaleScheduleListener`, un **listener d'entité** sur `Constraint` (`postPersist`/`postUpdate`/`postRemove` + `postFlush`) : les contraintes s'écrivent depuis l'API, les entrées de calendrier datées et d'éventuelles commandes — **marquer par appelant garantissait d'en oublier un**. Portée : les plannings **COMPLETED** du club+saison de la contrainte, **plans validés INCLUS**. ⚠ **Le cas validé a été MESURÉ, pas supposé** (`ConstraintWriteOnValidatedPlanTest`) : valider → 200, puis écrire une contrainte → **201**, et le plan **reste validé** — rien ne lie l'écriture des contraintes à l'état du plan. Un planning validé périmé est le plus grave : c'est celui qu'on distribue aux coachs. Les deux marqueurs sont remis à `false` par tout import solveur (`ScheduleResultImporter`, foyer unique). NR `ConstraintChangeStaleScheduleTest`, **step de `blocking-tests`**.  `mercure: true` ; opérations custom `generate`, `export-pdf`, `export-xlsx` ; filtres `isActive` (booléen) et `seasonId` (exact). Les routes de cycle de vie (`validate`/`reopen`/`regenerate`/`regenerate-from`) sont des routes Symfony hors API Platform (§3). |
| 13 | ScheduleSlotTemplate | `/api/schedule_slot_templates` | Créneaux générés | `GET`/`GetCollection` **seulement** — pas de POST/PUT/DELETE, pas de processor ni de DTO d'entrée : le déplacement passe par `POST /api/schedule-slots/{id}/move` (sous verdict moteur), les verrous/contraintes par `manual-edit/*`. **`lockOrigin`** — `RESERVATION` \| `MANUAL` \| `UNKNOWN`, **nullable** (`NULL` = pas de verrou : les 3 valeurs ne portent que sur un verrou RÉEL). **Server-authoritative** : aucune route ne le pose directement. Écrit aux 3 points d'origine — import du résultat solveur (`ScheduleResultImporter`), épinglage work-loop (`ManualEditService::applyLock`), pseudo-créneaux de réservation côté front (affichage seul, jamais persisté). ⚠ **`UNKNOWN` dit une IGNORANCE, pas une absence de verrou** — un verrou HARD sans réservation appariée reste indécidable et n'est **jamais deviné** (gardé par `LockOriginProvenanceTest`, **step de `blocking-tests`**). Le backfill de migration respecte **quel plan chaque réservation alimente** (base `NULL` → SEASON, overlay → son plan) pour ne pas fabriquer de faux `RESERVATION`. |
...
| 19 | TeamTagAssignment | `/api/team-tag-assignments` | Assignations d'étiquettes | Sous RLS FORCE (`backend/migrations/Version20260807170000.php`) : colonne `club_id` (backfillée depuis `team.club_id`) + policy `tenant_isolation` — c'était la seule table liée à un tenant sans backstop base de données, l'isolation reposait sur le seul filtre Doctrine |
...
| — | CalendarEntry | `/api/calendar-entries` | Cockpit temporel : périodes/événements (kind PERIOD/EVENT ; `parentEntryId` = enfant-**SEGMENT** d'une mère découpée). L'entrée porte le **FAIT** ; la RÉPONSE vit sur le plan — pas de pointeur inverse sur l'entrée (ADR-0002 lot D-b), « période → version active » se dérive de `SchedulePlanProvisioner::chosenOfPeriodPlan`. **Un enfant-SEGMENT naît AVEC son plan** — un SEGMENT est un bloc de semaines calendaires PLEINES lun→dim CONTIGUËS (clamp saison admis aux deux bords), inclus dans les semaines qui couvrent la mère ; la semaine simple est le segment de taille 1 (`CalendarEntryStateProcessor::assertValidWeekChild`). **Une semaine-enfant d'une mère VACANCES doit COUVRIR son lundi→vendredi** : `assertValidWeekChild` délègue à `App\Service\HolidayWorkweekRule::covers` (service pur, statique) pour chaque semaine du segment — 422 nommé sinon ; le filtre ne porte que sur `CalendarEntryPeriodType::HOLIDAY`. Le week-end ne compte pas, un jour hors saison compte comme couvert (tolérance clamp des vacances d'été). **Miroir déclaré côté front** : `frontend/src/features/cockpit/lib/holidayWorkweek.ts::holidayCoversWorkweek`, parité mécanique `holidayWorkweek.parity.json` gardée par `HolidayWorkweekMirrorParityTest` (groupe `contract`) — le front qualifie l'OFFRE (reprise vs fermeture/overlay), ce rail garde le POST. **Une semaine-enfant d'une mère FERMETURE doit être EXACTEMENT un segment début·milieu·fin** (les VACANCES gardent l'enveloppe libre, scinder/fusionner à la main) : `assertValidWeekChild` délègue à `App\Service\ClosureSegmentation::childWindowIsValidSegment` — le découpage lui-même vient de `App\Service\WeekSegmentationRule::segments` (statique pure, algorithme GÉOMÉTRIQUE aux ruptures seulement, aucune règle solveur redérivée : une semaine que la mère ne couvre pas entièrement lun→dim est un bout `start`/`end`, un run de semaines pleines contiguës est un `middle` — un trou de vacances ou une fenêtre déjà planifiée par un AUTRE plan coupe le run en deux) — 422 nommé sinon (« ni une semaine complète isolée, ni un milieu tronqué »). Tolère les semaines RÉVOLUES en tête (la géométrie PLEINE ou l'offre ROGNÉE par l'horloge sont TOUTES DEUX acceptées — une naissant d'une période passée/du seed, l'autre d'une création au cockpit). **Miroir déclaré côté front** : `cockpit/lib/weekSegmentation.ts::weekSegments`, parité mécanique `WeekSegmentationMirrorParityTest` (groupe `contract`) sur `weekSegmentation.parity.json`, module au registre `FrontRederivationRegistryTest`. Soumis à la même garde `PeriodWindowUniquenessGuard` que le POST `schedule_plans` : 409 `window_already_planned` si sa fenêtre recoupe déjà un AUTRE plan de période, sous le même verrou double club+saison puis entrée (club/saison lus sur la MÈRE). Déclarer une entrée RACINE (le FAIT — même une fermeture par-dessus une période déjà planifiée) reste, elle, toujours libre : la garde ne s'applique jamais à ce site. **`schoolHolidayId` auto-résolu** : une entrée `period`/`holiday` racine créée **sans lien explicite** reçoit celui de la vacance scolaire de la **zone du club** dont la fenêtre chevauche la sienne (`CalendarEntryStateProcessor::autoLinkedHolidayId`) — un lien explicite dans le body reste respecté tel quel ; sans zone club, sans chevauchement, ou hors kind holiday racine → `NULL`. Une seule vérité serveur : le radar dédoublonne sur ce champ (`RadarPanel`), le front n'arbitre rien. **Enums : absent → le défaut, PRÉSENT mais inconnu → 422** — `kind`, `periodType` et `status` sont facultatifs et ont un défaut documenté (ÉVÉNEMENT · pas de période · ACTIF), mais une valeur inconnue ne se replie plus en silence : elle lève un 422 nommé (`AbstractStateProcessor::unknownEnumValue`, maison unique du libellé, partagée avec `ConstraintStateProcessor`). Filet inatteignable tant que l'`Assert\Choice` du DTO tient — c'est le propre d'un filet ; le jour où il saute, une PÉRIODE ne devient plus un ÉVÉNEMENT sans un mot (ADR-0002). | Opération custom conflits (§3) |
| — | Competition | `/api/competitions` | Compétitions FFBB (championnat/coupe/brassage) — module matchs palier A. `entryDeadline` (date club, hors CRUD, écrite par le seul `POST /api/competitions/entry-deadlines` §3) + lecture additive résolue serveur `effectiveEntryDeadline`/`deadlineSource` (`club` prime, sinon le défaut communautaire `shared_competition_deadline` — joint par `ffbbCompetitionId`, une compétition non appariée n'a jamais de `community`). `fbiTeamLabel` perd son suffixe FFBB « (n) » à la lecture (`VenueLabelNormalizer::stripTeamNumberSuffix`) — sans quoi la clé de rapprochement division↔équipe changerait de forme sous les appariements existants | season-scoped |
| — | Fixture | `/api/fixtures` | Rencontres (HOME/AWAY, placement domicile, `externalRef` = n° FBI, `opponentLabel` sans suffixe FFBB « (n) »). Champ additif en lecture `unplacedReason` (`venue_lost`\|`null`) — pourquoi un match dépointé est retourné « à placer », distinct de la raison volatile d'auto-placement ; posé par `FixtureVenueLossMarker` (foyer unique), effacé par `setVenueId` dès qu'une salle non-null est reposée. Additifs `reviewState` (`NEW`\|`OUT_OF_SYNC`\|`REVIEWED`), `reviewedAt` et `pendingDeviations` (un par champ date/kickoff/venue encore ouvert) — le TRAITEMENT de l'espace « Importer », axe distinct du `status` de placement ; maison unique de la règle « placer = traiter » : `Fixture::setStatus` (§ module matchs, `/api/fixtures/review*` ci-dessous). Un extérieur, ou une rencontre passée/dans la semaine ISO en cours (fuseau club), naît déjà `REVIEWED` (`FbiFixtureImporter::treatOnArrival`) plutôt que `NEW` — jamais « à traiter » par le club. Deux champs additifs en lecture : `opponentOrganismeCode` (le code fédéral déjà stampé, dupliqué en lecture directe) et `opponentTeamKey` (le libellé adverse normalisé serveur, `VenueLabelNormalizer::normalize`). `awayTravel` (champ additif en lecture, `null` pour un domicile ou si rien n'est connu) : `{venueLabel, city, precision, oneWayMinutes, approximated, basis: linked\|most_frequent\|city}` — le trajet d'une rencontre EXTÉRIEURE, DÉRIVÉ de la rencontre elle-même (jamais du club), calculé EN BATCH par `FixtureStateProvider::decorateCollection` (`App\Service\OpponentTravelProjection::awayTravelByFixtureId`, zéro N+1, un domicile ne coûte rien). `AwayList.tsx`/`AwayTravelChip` lisent `fixture.awayTravel` directement | Ops custom conflits + import FBI + review (§3) |
| — | TeamLink | `/api/team_links` | Pont déclaré entre deux équipes — pas d'entité joueur, le gestionnaire déclare le lien (`teamAId < teamBId` normalisé par le processor) : `NOT_SIMULTANEOUS` (double projet, jamais en même temps) ou `BACK_TO_BACK` (enchaînées, implique `NOT_SIMULTANEOUS`), plus une `trainingIntensity` (`PREFERRED`\|`MANDATORY`) dédiée côté entraînement. Consommé par `MatchPlacementPayloadBuilder` (solveur de placement, préférence SOFT `NOT_SIMULTANEOUS` conservée) **et par le solveur d'entraînement** : `ScheduleConstraintBuilder::serializeTeamLinks` émet le bloc `teamLinks` du payload (roster-filtré), `MANDATORY` devient un anti-chevauchement DUR entre les deux équipes (`engine/app/solver/constraints/targeting.py::add_team_link_constraints`), `PREFERRED` un malus SOFT dérivé du tier dans l'objectif (`objective/terms.py::add_team_link_penalty`) — un verrou HARD contre un verrou HARD ne pose aucune contrainte (INFEASIBLE muet évité) mais est diagnostiqué post-solve (`result_builder._diagnose_team_links`). `MatchConflictDetector` ne le consomme plus — la famille `TEAM_LINK_OVERLAP` a quitté le radar de conflits (décision fermée → `etat-des-lieux.md` §2), asymétrie délibérée avec le solveur qui garde sa préférence souple, [`module-matchs.md`](../../specs/courantes/module-matchs.md) §2 | |
| — | TeamMatchHabit | `/api/team_match_habits` | Créneau de match habituel d'une équipe (un par jour de semaine, gymnase optionnel) — consommé par le module matchs (`MatchPlacementPayloadBuilder`, `AwayKickoffEstimator`) pour estimer les coups d'envoi à l'extérieur, **et par le solve hebdo** (`ScheduleConstraintBuilder::deriveMatchDay`) pour dériver le `matchDay` (jour de repos) de l'équipe — toute écriture marque les plannings `COMPLETED` du club+saison périmés (`ResourceChangeStaleScheduleListener`) | |
| — | MatchSlotRotation | `/api/match_slot_rotations` | Créneau de match PARTAGÉ (gymnase **NOT NULL** + jour ISO + heure, unicité `(club_id, season_id, venue_id, day_of_week, kickoff_time)`) occupé en alternance par 2..10 équipes ordonnées (`MatchSlotRotationTeam`, `position` purement FICTIF — aucun calendrier). Écriture par remplacement transactionnel des membres ; 409 sur course d'unicité de créneau. Toute écriture (rotation ou membre) marque les plannings `COMPLETED` du club+saison périmés (`ResourceChangeStaleScheduleListener`). Détail : [`module-matchs.md`](../../specs/courantes/module-matchs.md) § Rotation A/B. | |
| — | VenueTravelTime | `/api/venue_travel_times` (filtre `seasonId` exact) | Un barème de trajet ENTRE DEUX GYMNASES du club+saison : `drivingMinutes`/`walkingMinutes` (nullables, chacun indépendant) + `drivingSource`/`walkingSource` (`AUTO`\|`MANUAL`, ne portent une valeur que si la minute correspondante est renseignée). **Symétrique** : le processor normalise `venueAId < venueBId` (ordre lexical uuid) — un couple = une ligne (unique `club_id, season_id, venue_a_id, venue_b_id`). Toute valeur écrite par le CRUD est `MANUAL` — **jamais écrasée** par l'autofill (`POST /api/venue-travel-times/autofill`, §3 · `backend/docs/geo-api.md`). GetCollection/Get/Post/Put/Delete, écriture management (SEC-07), lecture ouverte au Membre. Cascade : suppr. d'un gymnase emporte ses barèmes (l'une ou l'autre colonne, patron `TeamLink`) ; purgée avec la saison ; suit la transition de saison (remap gymnase, minutes ET sources transcrites) ; STRUCTURE de club+saison — pas de `schedulePlanId`, la matrice nourrit tous les plans (patron `TeamLink`) ; toute écriture marque les plannings `COMPLETED` du club+saison périmés (`ResourceChangeStaleScheduleListener`). **Le solveur la lit** (bloc `venueTravelTimes`) — voir la ligne `VenueTravelRuleSetting` ci-dessous pour l'intensité. | |
| — | VenueTravelRuleSetting | `/api/venue_travel_rule_settings/{ruleKey}` (`ruleKey` FIXE `travelTime`) | Le levier d'intensité de la règle implicite `travelTime` — SINGLETON club+saison (`Entity/VenueTravelRuleSetting.php`, contrainte d'unicité `club_id`+`season_id`), vocabulaire **PREFERRED\|MANDATORY** des passerelles (`TeamLinkIntensity`), store DÉDIÉ plutôt qu'une 6ᵉ clé `ImplicitRuleSetting` (décision fermée : `etat-des-lieux.md` §2 — cette dernière est typée `ImplicitRuleIntensity` HARD/PREFERRED/OFF, vocabulaire bien-être). `Get`/`Put` seulement, une clé de chemin ≠ `travelTime` rend **404** des deux côtés (revue sécurité). GET résout (stocké ou défaut PREFERRED, `isDefault` servi), PUT upserte — écriture management (SEC-07), 409 saison archivée, 422 sur un vocabulaire bien-être. Recopie N+1 + purge suivent le patron de `VenueTravelTime`. Lu par `ScheduleConstraintBuilder::resolveTravelRuleIntensity`, émis dans `implicitRules.travelTime.intensity` du payload moteur. | |
...
confronte le contrat au ROUTEUR dans les deux sens et rougit sur tout écart. Cette déclaration
est éclatée **par domaine** :

- `CustomRoutesOpenApiFactory` n'est plus qu'un **composeur** : il instancie
  `OpenApiSchemas` puis appelle `contribute(Paths $paths)` sur chaque `CustomPathContributor`
  (liste exhaustive : `ls backend/src/OpenApi/PathContributor/`), dans un ORDRE fixe et
  significatif (`Paths::addPath()` est un append, `openapi-snapshot.json` fige l'ordre exact des
  chemins).
...
- **Certains contributeurs dépassent volontairement 300 lignes** (ex. `AccountSessionPaths`,
  `UncoveredCustomPaths`) et **le restent délibérément** : leurs routes s'entrelacent dans l'ordre exact du contrat,
...
| `/api/schedules/{id}/socle-deviation` | GET | `SocleDeviationController` | **Les écarts NOMMÉS vs le socle** — route de **LECTURE pure** (aucun `persist`, aucun `flush`) qui compare la version ciblée d'un plan de **PÉRIODE de type FERMETURE** à la version POINTÉE du plan SEASON (`SchedulePlanProvisioner::chosenOfSeasonPlan`) et rend deux listes, **`moved`** et **`unplaced`** — cible (VERSION pas plan), catégories, appariement déterministe et précédence de raison détaillés dans [ADR-0004](../../docs/architecture/adr-0004-period-plan-birth-as-socle-copy.md) §« Les écarts nommés vis-à-vis du socle ». Refus : **422** plan SEASON ou `periodType != CLOSURE` ; **409** version non `COMPLETED` ou socle non pointé (reste ATTEIGNABLE malgré `SocleGuard` — une période déjà commencée survit à la réouverture du socle) ; **404/403** tenant. **Pas de gate management** — lecture ouverte au Membre (patron `CalendarEntryConflictsController`). Zéro appel engine, aucune migration. NR bloquant `Security/SocleDeviationParityTest`. |

**Placement précédent émis au moteur (terme de stabilité ; aussi proximité de placement côté moteur, poids 9)** :
...
**Référence socle du comblement** : en mode **fill seulement**
...
`SCORE_FORMULA_VERSION` porte le suffixe **V13**. Gardé par `CrossStack/SocleReferencePayloadParityTest`
...
bloc) vérifiable, une écriture semi-faite ne serait ni « libre » ni « complète » (`ReservationGroupOccupancy`,
5 gardes : exclusivité, réciproque, plafond `commonSessions`, plafond membre, capacité).

Le corps ne connaît que `sharedTrainingBlockId` (aucun modèle groupe K — voir ressource
`SharedTrainingBlock` en §2). Gardé par les mêmes 5 règles, appliquées au bloc
...
**Constat sémantique (pas un trou)** : une réservation de bloc s'éclate en N verrous HARD sur une
case ; `add_room_at_most_one` (moteur) laisse une case toute-verrouillée non contrainte, et le
diagnostic de sur-capacité replie déjà les blocs PAR CASE (`result_builder/diagnostics.py`) —
aucun crédit manquant côté moteur pour cette réservation.

**Rail de DÉPLACEMENT groupé** : `POST /api/schedule-slots/move-group` déplace le bloc entier sous
UN verdict à N candidats (`candidates`/`references` en LISTES sur `/validate-assignments`) —
détail §« Édition manuelle » ci-dessous.
...
2. sinon un jour de **vacances scolaires** de la zone du club (`Club.schoolZone`, zone nulle =
   aucune neutralisation) → **0 h**, **même en Réalisé** — compter des heures « faites » sur une
   semaine où personne ne s'est entraîné serait faux (basculable en une seule ligne dans
   `resolveScheduleForDate`, commentée à l'endroit exact).
...
détaillées dans [`docs/geo-api.md`](geo-api.md). Le levier d'intensité est une ressource
...
| Route | Méthode | Contrôleur | Description |
|-------|---------|------------|-------------|
| `/api/clubs/{id}/import-teams/analyze` | POST | `ImportTeamsAnalyzeController` | **Dry-run** : mêmes body/gate que l'import, ZÉRO écriture. Délègue à `FfbbExcelImporter::analyze()`, qui rend `{rows:[{row, name, category, number, alreadyPresent}], errors, total}` — `row` = n° de ligne EXCEL (`rowIndex + 2`), `alreadyPresent` = le nom est déjà présent (club + saison courante, comparaison casse-insensible) **ou** déjà vu plus haut dans CE fichier (doublon interne). Mêmes refus métier que l'import : colonnes manquantes → 400, fichier d'un autre club (code Organisme ≠ code du club) → 422 dès l'analyse, rien créé. |
| `/api/clubs/{id}/import-teams` | POST | `ImportController` | Importe un fichier `.xlsx` (Excel) pour un club et une saison donnés. Body multipart : `file` (.xlsx), `seasonId`, et un champ optionnel **`rows`** — liste JSON de numéros de ligne Excel (`ImportController::parseRows`) ; **absent = tout importer**. Délègue à `FfbbExcelImporter::import()` (`?array $selectedRows` filtré en TÊTE de boucle — une ligne non cochée n'est ni comptée ni soumise au contrôle de code club). Retourne 200 avec `created`, `skipped`, `errors`. **SEC-22** : bornes d'upload et limiteur `xlsx_import` — §4 Access control. |
...
| `/api/venues/{id}/deletion-impact` · `/api/teams/{id}/deletion-impact` · `/api/coaches/{id}/deletion-impact` | GET | `DeletionImpactController` | Ce qu'une suppression VA détruire, calculé par le serveur : `{blocked, reason, lines[{key,count,one,many}], slotsInForce, declaredFixtures}`. Les lignes sont comptées en parcourant `App\Deletion\CascadePlan`, **la même liste** qu'`EntityCascadeDeleter` exécute (maison unique — ajouter une destruction sans son annonce est impossible, NR `DeletionImpactParityTest`). **Les libellés viennent du serveur** : gardés côté écran, une famille ajoutée à la cascade aurait disparu de la modale faute de traduction. `blocked` porte le refus du périmètre engagé (l'écran n'offre plus un geste qui rendrait 409) ; `slotsInForce` = séances touchées vivant dans une version POINTÉE (ADR-0002) ; `declaredFixtures` = les matchs `SUBMITTED`/`VALIDATED` qui perdront leur salle — annoncés, jamais bloquants ; le dépointage lui-même (`Fixture` visé par `venues/{id}/deletion-impact`) délègue à `FixtureVenueLossStep` → `FixtureVenueLossMarker`, le MÊME foyer que la gâchette de validation (`/api/schedules/{id}/validate-impact` ci-dessus) — même état final « à placer » + raison `venue_lost`. Lecture seule ; frontière tenant explicite en plus de RLS. Déclarées dans `CustomRoutesOpenApiFactory` et présentes au **snapshot OpenAPI** — `EveryCustomRouteIsDocumentedTest` l'exige de TOUTE route custom (« une route absente du contrat n'existe pour personne »). |
| `/api/fixtures/conflicts` | GET | `FixtureConflictsController` | Radar de conflits d'empreinte-temps coach/joueur entre rencontres et entraînements, recalculé À LA VOLÉE (rien n'est persisté) — algorithme, familles, sévérités et champs additifs par côté détaillés dans [`module-matchs.md`](../../specs/courantes/module-matchs.md) §2 « Détecteur de conflits ». Chaque item porte `fingerprint` (identité stable du conflit, `ConflictFingerprinter`) et `resolution: {status, note, updatedAt} \| null` (`null` = « à traiter », joint SERVEUR contre `conflict_resolution`). `TeamLink` n'est plus chargé (`ConflictRadarLoader`) : la famille `TEAM_LINK_OVERLAP` a quitté le radar (décision fermée → `etat-des-lieux.md` §2). |
| `/api/fixtures/conflicts/{fingerprint}/resolution` | PUT | `FixtureConflictsController` | Le gestionnaire pose (ou remplace) le statut de traitement d'un conflit (jamais un masquage : le radar continue de le rendre). SEC-07 (`assertManager()`) tiré **en premier**. Corps `{status, note?, fbiCorrection?}` — les huit statuts stockables, leurs règles par famille (`ConflictResolutionStatus::casesForFamily`) et le complément `fbiCorrection` (registre « à corriger dans FBI ») sont détaillés dans [`module-matchs.md`](../../specs/courantes/module-matchs.md) §6 « Résolution des conflits ». Note > 500 caractères → 422 ; empreinte hors flux courant du club+saison → 422 ; hors table de la famille → 422 ; `fbiCorrection` envoyé avec un AUTRE statut → 422. Upsert sur la clé unique (club, saison, empreinte) ; **pose concurrente rattrapée** : une violation d'unicité au flush est capturée, la ligne gagnante relue sur la connexion vivante (RLS) et rendue en **200 idempotent** au lieu d'une 500. Empreinte contrainte par la route (`[A-Z_]+:[0-9a-fA-F,:-]+`) — malformée → 404 de routing, jamais l'action. |
| `/api/fixtures/conflicts/{fingerprint}/resolution` | DELETE | `FixtureConflictsController` | Retour à « à traiter » (suppression de la ligne). SEC-07. **Idempotent** : 204 même sans ligne, y compris sur une empreinte disparue du flux. **N'appelle jamais `FbiCorrectionLedger`** — asymétrie volontaire, détail §6 : remettre un conflit « erreur FBI » à traiter ne ferme pas l'entrée du registre qu'il a pu ouvrir. Table `conflict_resolution` : TENANT (RLS `FORCE`, patron `opponent_venue_link`), unique `(club_id, season_id, fingerprint)`, jamais nettoyée à la volée sur un orphelin — purgée avec la saison (`SeasonDataPurger`) ou l'effacement RGPD du club (`ErasedClubPurger`). |
| `/api/matches/module-visit` | POST | `MatchModuleVisitController` | Le gardien à l'ouverture : stampe une référence de visite PAR (club, saison, utilisateur) et rend le delta « depuis ta dernière visite » (`newFixturesCount`, `newConflictFingerprints`, `planningChanged`) — détail [`module-matchs.md`](../../specs/courantes/module-matchs.md) §4. Un seul endpoint POST (pas de GET séparé, pour ne pas ouvrir de course F5 entre lire et tourner la référence) ; première visite muette ; fenêtre de grâce glissante de 30 min (rejoue les mêmes badges sans les éteindre) ; calcul du delta délégué à `MatchModuleDeltaComputer`, coupé de la rotation pour qu'un autre appelant puisse un jour lire sans stamper. Ouvert au Membre (aucune garde management), écrit même saison archivée (bookkeeping utilisateur, pas une mutation du planning). |
| `/api/venue_match_windows` · `/api/venue_unavailabilities` | CRUD | API Platform (5-fichiers) | **Capacité** : fenêtres d'accès match (jour+plage, `start<end` même jour) et indisponibilités toutes-circonstances (dates incluses + motif, écriture management-gated). `venueId` étranger invisible → 422. |
...
| `/api/fixtures/import/analyze` | POST | `ImportFixturesAnalyzeController` | **Dry-run** de l'export FBI global club : table des divisions résolue contre la correspondance persistée (`Competition`), zéro écriture. Rend aussi `deviations` — les écarts de réconciliation (date/heure/salle) des domiciles déjà **placés**, état app VS état fichier, à décider à l'import. **SEC-22** : bornes d'upload et limiteur `xlsx_import` — §4 Access control. |
| `/api/fixtures/import` | POST | `ImportFixturesController` | Import FBI **une passe** (fichier global + `mappings` JSON) : persiste les correspondances puis crée/**met à jour** par diff `(team, n° FBI)`. Rapport `created`/`updated`/`unchanged`/`exempted`/`warnings`/`unmappedDivisions`/`errors`. Remplace l'ancienne route par équipe `/api/teams/{id}/fixtures/import`. Accepte en plus `decisions` JSON (`{fixtureId, field: date\|kickoff\|venue, choice: keep_app\|take_file}`) — réconciliation par écart, workflow `NEW`/`OUT_OF_SYNC`/`REVIEWED` et attestation `VALIDATED` (D9) : [`module-matchs.md`](../../specs/courantes/module-matchs.md) §7 « Workflow de traitement ». Chaque dépôt écrit une `FbiIngestion` (club+saison, `source=FBI_XLSX`). La saison de l'ingestion est **celle de la requête** (`SeasonResolver::selectedOrCurrent`), jamais devinée depuis les lignes importées (revue sécurité). Voir aussi la commande `app:fixtures:catch-up-review` ([`commands.md`](commands.md)) pour un rattrapage sans re-dépôt. **SEC-22** : bornes d'upload et limiteur `xlsx_import` — §4 Access control. |
| `/api/fbi-ingestions/latest` | GET | `FbiIngestionFreshnessController` | Fraîcheur : le dernier dépôt xlsx (`source=FBI_XLSX`) du club+saison courants (`FbiIngestionRepository::latestXlsx`), `null` si aucun. Lecture ouverte au Membre (même patron que `LeagueMatchWindowsController`), tenant+saison résolus côté serveur. |
| `/api/fixtures/place` | POST | `PlaceMatchesController` | « Placer automatiquement » (ADR-0003). Rail **SYNCHRONE** — pas de Messenger/Mercure, verrou Redis dédié `MatchPlacementLock` (TTL 90 s, anti-double-clic). Ordre des gardes : SEC-07 (management) → saison inscriptible → `SocleGuard::assertSeasonPlanChosen` (409 si pas de socle en vigueur). Construit le payload (`MatchPlacementPayloadBuilder`, y compris `TeamLink`/`TeamMatchHabit`/`matchMinutes`/`warmupMinutes` par équipe injectés par `MatchDurationResolver`, catégorie sinon défaut de famille), appelle `POST /place-matches` sur l'engine (timeout 60 s, `BAD_GATEWAY` si l'appel échoue — rien n'est écrit avant l'application du résultat), applique les placements (`MatchPlacementResultApplier`). Un match non plaçable n'est **jamais une erreur** : il revient nommé dans `unplaced` avec sa raison. |
| `/api/fixtures/review` | POST | `ReviewFixturesController` | « Traiter » des rencontres, espace Importer. Corps `{fixtureIds}` (geste LIGNE : chaque rencontre traitée, écarts pendants vidés — « garder l'app » implicite) XOR `{teamId}` (geste MASSE : toutes les rencontres de l'équipe traitées SAUF celles à écart pendant, sautées et nommées dans `skipped[{fixtureId, reason}]` — jamais tranché en masse). Rend `{reviewed, skipped}`. SEC-07 + saison écrivable + `SocleGuard` ; 422 si zéro ou les deux gestes fournis. |
| `/api/fixtures/review/deviations` | POST | `ReviewFixtureDeviationController` | Trancher UN écart pendant. Corps `{fixtureId, field: date\|kickoff\|venue, choice: keep_app\|take_source}` — `keep_app` retire l'écart, `take_source` rejoue le moteur partagé (`FbiFixtureImporter::applyFieldTakeFile`) à partir de la valeur PERSISTÉE (jamais du client). Dernier écart retiré → `REVIEWED` + horodaté. `keep_app` (l'appli fait foi, FBI est en retard) ouvre en plus une entrée du registre « à corriger dans FBI » (`FbiCorrectionLedger::open`, tous champs — même foyer que le moteur de réconciliation partagé) ; `take_source` n'en ouvre jamais. SEC-07 + saison écrivable + `SocleGuard` ; 404 rencontre invisible (tenant) ; 422 corps malformé ou champ sans écart en attente. |
| `/api/fixtures/league-validation` | GET | `LeagueValidatedFixturesController` | Lecture détaillée **par championnat ÉCHU** (`{matured[], toTreat[], missingDeadline[], totalValidatable}`) pilotant la bascule en lot des domiciles déjà datés côté fédération — logique complète (échéance effective, prédicat, cas nommés) : [`module-matchs.md`](../../specs/courantes/module-matchs.md) §7 « Validé ligue en lot ». SEC-07 + saison écrivable + `SocleGuard` + **409 explicite si aucune saison ne se résout**. Route statique `priority: 10` pour gagner sur `/api/fixtures/{id}` d'API Platform. |
| `/api/fixtures/league-validation` | POST | `LeagueValidatedFixturesController` | **Sans corps** : recalcule les championnats échus AU MOMENT DE L'APPEL et bascule en lot chaque domicile éligible (même prédicat que le GET) — détail §7 ci-dessus. Rend `{confirmed}`. SEC-07 + saison écrivable + `SocleGuard` + **409 explicite si aucune saison ne se résout** ; **409 lisible en français sur une collision d'écriture simultanée** (verrou optimiste `Fixture`, écriture idempotente). |
| `/api/fixtures/fbi-corrections` | GET | `FbiCorrectionController` | Lecture des entrées OUVERTES du registre « à corriger dans FBI » (club+saison), ouverte à tout membre. Route statique `priority: 10` pour gagner sur `/api/fixtures/{id}` d'API Platform. |
| `/api/fixtures/fbi-corrections/{id}/close` | POST | `FbiCorrectionController` | Le gestionnaire coche « Corrigé dans FBI » (`closed_by=manual`). SEC-07 + saison écrivable ; entrée d'un autre club invisible (tenant) → 404 byte-identique, comme une entrée déjà fermée. |
| `/api/fixtures/fbi-corrections/{id}/reopen` | POST | `FbiCorrectionController` | Annule un « corrigé dans FBI » manuel de moins de `REOPEN_GRACE_HOURS = 24` h (409 sinon, ou si l'entrée est déjà ouverte, ou fermée par un dépôt). SEC-07 + saison écrivable. |
| `/api/ffbb/rencontres` | GET | `FfbbRencontresController` | Le canal API FFBB de réconciliation : récupère à la demande les rencontres publiées du club (`FfbbApiClient::searchRencontres`, filtre STRICT serveur sur le code club, index `ffbbserver_rencontres`), les croise avec l'app (`FfbbRencontreReconciler`, appariement 3 étages + tier-0 idempotence sur `Fixture.ffbbRencontreId`) et rend `{deviations[], creatable[], fetchedAt}` — `deviations` réutilise VERBATIM le moteur `FbiFixtureImporter` (même périmètre : domiciles déjà placés) ; `creatable` = les rencontres publiées sans fixture correspondante (mesuré : uniquement des amicaux), proposées à la création, jamais imposées. SEC-07 + `SocleGuard` + tenant ; 422 club sans code FFBB ; 502 FFBB injoignable. |
| `/api/ffbb/rencontres/apply` | POST | `FfbbRencontresController` | RE-FETCHE côté serveur (jamais les valeurs du client), applique les décisions par écart (mêmes `{fixtureId, field, choice}` que l'import xlsx) et crée les rencontres choisies (`{rencontreId, teamId}`, idempotent sur l'index unique partiel `uniq_fixture_ffbb_rencontre`). Partage le MÊME moteur de traitement que le xlsx (workflow `reviewState`/`pendingDeviations`, détail [`module-matchs.md`](../../specs/courantes/module-matchs.md) §7) ; seuls les warnings du moteur partagé ne sont pas remontés par ce canal. Écrit une `FbiIngestion` datée `source=FFBB_API` (compteurs seuls) — ne touche JAMAIS la fraîcheur xlsx (`fbi-ingestions/latest` ne lit que `FBI_XLSX`). SEC-07 + saison écrivable + `SocleGuard` + tenant ; 409 doublon concurrent (collision sur l'index unique). |
| `/api/venues/fbi-labels` | GET | `VenueExternalLabelController` | Inventaire agrégé « libellé de salle FBI/FFBB → gymnase » de la saison courante, par clé NORMALISÉE (`App\Service\Basketball\VenueLabelInventory::forSeason`, service pur) : `labelKey`/`displayLabel` (graphie brute la plus fréquente)/`venueId` (alias confirmé)/`suggestedVenueId` (gymnase unanime des domiciles placés, `null` si divergence ou déjà confirmé)/`homeCount`/`placedCount`/`unplacedCount`, trié par `displayLabel`. Lecture ouverte à **tout membre authentifié** (état, pas un geste) — pas de `ManagementAccessGuard`. `priority: 10` pour gagner sur la route item `/api/venues/{id}` d'API Platform. |
| `/api/venues/{id}/external-labels` | POST | `VenueExternalLabelController` | « Rattacher » un libellé de salle FBI/FFBB à un gymnase : `{label}` → `{venueId, label, attached}`. Ajoute l'alias (normalisé — `VenueLabelNormalizer` — idempotent) PUIS backfille les domiciles du club encore sans salle dont le libellé égale l'alias, sans jamais les placer (`attached` = nombre nouvellement rattaché, 0 sur un re-POST). Libellé déjà porté par un AUTRE gymnase du club → 422 nommé ; vide après normalisation → 422. SEC-07 + saison écrivable (409) + tenant (gymnase étranger invisible → 404). `{label, reassign: true}` bascule sur une ré-affectation — l'alias quitte son ancien porteur (`previousVenueId` dans la réponse) et se pose sur la cible ; les domiciles NON PLACÉS de la saison au même libellé sont re-pointés quel que soit leur gymnase actuel (`attached`), les domiciles PLACED/SUBMITTED/VALIDATED gardent leur salle et leur statut (comptés `kept`). Sans le drapeau, comportement byte-identique. |
| `/api/venues/{id}/external-labels/{label}` | DELETE | `VenueExternalLabelController` | « Retirer » un alias (idempotent, 204) — ne touche AUCUNE rencontre déjà rattachée (le lien reste posé, seul l'alias qui l'a produit part). Mêmes gardes que le POST. Retirer un alias ne dépointe jamais — seule une ré-affectation explicite (POST `reassign: true`) le fait, et seulement pour les domiciles non placés. |
| `/api/competitions/entry-deadlines` | POST | `CompetitionEntryDeadlinesController` | Saisie bulk `{competitionIds[], deadline}` : pose (ou efface, `deadline: null` **explicite**, clé absente → 422) UNE échéance sur un lot de compétitions du club+saison en une transaction ; un id inconnu/étranger → 422, rien écrit. Pour chaque compétition **appariée** (`ffbbCompetitionId` non null) et une date posée, upserte aussi le défaut communautaire `shared_competition_deadline` — détail [`module-matchs.md`](../../specs/courantes/module-matchs.md) §4 « Échéances de saisie ligue/comité ». SEC-07 (management) tire avant tout lookup ; saison archivée → 409. |
| `/api/matches/deadline-outlook` | GET | `EntryDeadlineOutlookController` | L'outlook cockpit, lecture seule, ouvert au Membre : échéances EFFECTIVES (club sinon défaut communautaire) sous J-7, plus le champ additif `fbiTodo {toEnter, toCorrect}` (compte GLOBAL « à faire dans FBI », détail §4 « Échéances de saisie ligue/comité »). Une fenêtre ouverte ET une référence de visite existante joignent le delta gardien (`MatchModuleDeltaComputer`) **sans stamper**. |
| `/api/opponents/resolve` | POST | `OpponentResolveController` | Le rattrapage de l'annuaire GLOBAL `opponent_directory` : localise les adversaires AWAY du club+saison encore absents/CITY-only (`OpponentLocationResolver`). SEC-07, cap dur `MAX_DISTINCT=200` avant tout appel réseau, rate-limit `opponent_resolve` par utilisateur. N'écrit QUE la table globale (aucune donnée club/saison). Route fine conservée (compat) — le front n'appelle plus que l'orchestrateur `/api/opponents/refresh` ci-dessous. |
| `/api/opponents/refresh` | POST | `OpponentRefreshController` | L'orchestrateur « Mettre à jour les adversaires » : UN appel enchaîne trois passes best-effort **indépendantes** — rattrapage des codes fédéraux, auto-localisation des gymnases depuis le libellé du fichier FBI, dispatch du calcul des trajets manquants — algorithme complet (égalité stricte, budget partagé, crédits partagés) : [`module-matchs.md`](../../specs/courantes/module-matchs.md) §1 « Table TENANT `OpponentVenueLink` ». SEC-07 d'abord, cap dur `MAX_DISTINCT=200` avant tout réseau, limiteur `opponent_refresh` 10/h. Budget de MUR partagé (`OpponentRefreshController::REFRESH_BUDGET_SECONDS = 45`) — au-delà, chaque passe rend `unresolved`/`skipped` plutôt que de dépasser la deadline. Réponse `{codes, autoLocated, travel}`, `travel: {queued, alreadyRunning, pending}`. Consommé par le bouton unique « Mettre à jour les adversaires » (`/matchs/adversaires`) — les routes fines (`/opponents/resolve`, `/opponents/travel/resolve`) restent en place (compat). |
| `/api/opponents/travel` | GET | `OpponentTravelController` | Groupée par CLUB adverse joué cette saison : `{clubId, seasonId, clubGeolocated, opponents: [{code, pairingKey, name, city, postalCode, precision, hasLogo, fixtureCount, venues: [{id, label, externalRef, latitude, longitude, source, travelMinutes, travelStatus, approximated, fixtureCount, fallbackVenueName}], unmatchedLabels: [{label, fixtureCount}]}]}`. `postalCode` (additif, `OpponentDirectoryEntry::getPostalCode`, fédéral, `null` si inconnu) préfiltre la recherche de gymnase de `LocateOpponentModal` côté front (§ module-matchs §9). `pairingKey` (additif, `App\Service\OpponentPairingKey::fromOpponent`) : le code fédéral quand il existe, sinon une clé SENTINELLE locale dérivée du libellé (adversaire sans code) — le front la réutilise TELLE QUELLE dans les routes d'écriture ci-dessous, il ne la redérive jamais. `venues[]` = un `OpponentVenueLink` du club : coordonnées fédérales (publiques, jamais le siège du club — repassées telles quelles au geste de fusion) ; `travelMinutes`/`travelStatus` (`done`\|`pending`\|`unavailable`, serveur, `pending` lit `TravelComputeLock::isHeld`) lus depuis `ClubTravelCache` ; `approximated` toujours `false` ici (un lien est un gymnase EXACT, le repli approché vit dans la projection par rencontre, pas ici) ; `fallbackVenueName` = le gymnase le plus fréquent RESTANT si celui-ci était retiré (calculé serveur). `unmatchedLabels[]` = les libellés de fichier SANS lien (« à apparier »). `hasLogo` dérivé de `OpponentDirectoryEntry.logoId`. Ouvert au Membre. |
| `/api/opponents/{code}/logo` | GET | `OpponentLogoController` | Sert le logo fédéral de l'adversaire `{code}` (re-hébergement paresseux depuis `logo_id` au premier GET, `LogoStorage`/`FfbbLogoFetcher`, même host `api.ffbb.com/assets/` que le logo club). Route MEMBRE (`IS_AUTHENTICATED_FULLY`) — jamais publique, contrairement au logo club institutionnel. 404 sans logo connu ou téléchargement en échec. `Cache-Control: private, max-age=86400`. |
| `/api/opponents/{code}/venues` | POST | `OpponentTravelController` | Ajoute un gymnase au club adverse `{code}` (ref fédérale ou coordonnées choisies dans `/api/ffbb/salles`), lien MANUAL. Le libellé de fichier est optionnel ; **jamais validé contre les rencontres de la saison** (à dessein — cas playoff). 422 si `{code}` n'est pas un adversaire AWAY de la saison, gymnase/libellé invalide, ou **plafond `OpponentVenueLinkManager::MAX_VENUES_PER_OPPONENT = 20`** dépassé par une NOUVELLE clé de libellé. SEC-07, SEC-19 (limiteur `opponent_travel_manual` 30/h, partagé par les trois gestes d'écriture ci-dessous). `{code}` accepte aussi la clé SENTINELLE d'un adversaire sans code fédéral — comptabilité du catalogue partagé et neutralisation de `ref` pour cette clé : maison unique `OpponentVenueLinkManager::writeGym`, détail [`module-matchs.md`](../../specs/courantes/module-matchs.md) §1, piège de test gardé dans `.claude/rules/backend.md`. |
| `/api/opponents/{code}/venue-links` | POST | `OpponentTravelController` | Apparie un libellé de fichier ORPHELIN (« à apparier ») à un gymnase choisi ; 422 si le libellé n'est celui d'aucune rencontre AWAY réelle de cet adversaire, ou plafond `MAX_VENUES_PER_OPPONENT` dépassé. Même limiteur/gardes/comptabilité que `/venues`. SEC-07. |
| `/api/opponents/venue-links/{id}` | PUT | `OpponentTravelController` | Ré-apparie/fusionne un lien EXISTANT vers un AUTRE gymnase (la clé de libellé est conservée, reconnue au prochain import) ; réponse `targetFixtureCount` = le compte RÉSULTANT du gymnase cible (« en portera N »). 404 byte-identique si le lien n'appartient pas au club (RLS). Comptabilité partagée idempotente : ne crédite le nouveau ref que si le club ne le porte pas déjà, ne débite l'ancien que si plus aucun AUTRE lien du club ne le porte (même neutralisation `ref` sous clé SENTINELLE que `/venues`, NR `OpponentVenueSuggestionShareTest`). SEC-07, SEC-19. |
| `/api/opponents/venue-links/{id}` | DELETE | `OpponentTravelController` | Retire l'appariement LOCAL (jamais le catalogue fédéral `opponent_venue_suggestion`) ; décrémente le partagé si le lien retiré était MANUAL, portait une ref fédérale, ET était le DERNIER lien du club sur ce ref (symétrie du crédit posé par `/venues`/`/venue-links`). 204 même 404 byte-identique cross-club. SEC-07. |
| `/api/opponents/travel/resolve` | POST | `OpponentTravelController` | (management) Ne recalcule pas TOUT : route seulement les paires siège→gymnase MANQUANTES du cache (une paire déjà résolue n'est jamais retouchée) — filtre et repli détaillés [`module-matchs.md`](../../specs/courantes/module-matchs.md) §1 « Cache de trajets et calcul asynchrone ». **Rail asynchrone** : dispatche `ComputeTravelTimesMessage(scope: OPPONENTS)` au worker et répond immédiatement `{queued: true, alreadyRunning: false}` (progression via `travelStatus`, Mercure `club:{clubId}:travel`) ; si un calcul tourne déjà, rien n'est dispatché — `{queued: false, alreadyRunning: true}`. Cap dur `OpponentTravelResolver::MAX_OPPONENTS` avant dispatch, rate-limit `opponent_travel_resolve` par utilisateur. SEC-07. |
| `/api/opponents/{code}/venue-suggestions` | GET | `OpponentTravelController` | Les suggestions PARTAGÉES de gymnases de l'adversaire `{code}` : table GLOBALE `opponent_venue_suggestion` (hors tenant, pas de `club_id`, pas de RLS, GRANT sans DELETE, **sans backfill**), deux sources `FFBB_API` (observé, sans ref) / `MANUAL` (choisi via `/api/ffbb/salles`, `numero` re-résolu côté serveur). `chosenByCount` compte des CHOIX (un par lien MANUAL club×libellé), jamais des clubs distincts (une ligne à 0 reste). Triées `FFBB_API` puis `MANUAL` par compte décroissant. 422 si `{code}` n'est pas un adversaire AWAY de la saison. management SEC-07. Gate NR bloquant : `OpponentVenueSuggestionShareTest`. |

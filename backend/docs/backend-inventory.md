...
Last verified @ 2026-09-02 (PR-0 lot overlay, `documentation-update`). Ligne `SharedTrainingBlock`
re-confrontée au code : la mention « aucune copie socle→période » est REMPLACÉE — un plan de
FERMETURE naît désormais avec une copie des blocs du socle (`SchedulePlanProvisioner::
copySocleSharedBlocks`, appelée du point de mint `ensurePeriodPlanId`, gardée par le type ;
HOLIDAY ne copie rien), NR au step bloquant `PeriodPlanBirthTest` ✓. Reste de l'inventaire non

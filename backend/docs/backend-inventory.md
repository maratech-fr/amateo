...
Last verified @ 2026-09-19 (`documentation-update`, PR H « onglet Adversaires »). §3 « Module
matchs » recalé contre le code lu cette passe : `GET /api/opponents/travel` recalée (+`travelStatus`/
`hasLogo` additifs, C5/C7) ; `POST /api/opponents/travel/resolve` recalée (dispatche au worker,
`{queued: true}`, ne route plus que les manques, C5/C6) ; nouvelle route `GET
/api/opponents/{code}/logo` (`OpponentLogoController`, C7) ajoutée. Reste confronté à la passe
précédente (2026-09-19, PR G « todo FBI unique ») : trois routes `FbiCorrectionController` (registre
« à corriger dans FBI »), `/api/fixtures/review/deviations`, `/api/matches/deadline-outlook`. Reste
du fichier non re-vérifié cette passe — historique des recalages précédents : `git log -p --follow`
ce fichier. Un stamp REMPLACE, l'historique vit dans git.
...
| `/api/opponents/travel/resolve` | POST | `OpponentTravelController` | (management) **C5/C6 (2026-09-19)** — ne recalcule plus TOUT : `OpponentTravelResolver::resolve` route seulement les MANQUES (club + équipe, un seul ensemble) ; une ligne déjà résolue n'est jamais retouchée (un trajet est une constante). **Quitte le rail synchrone** : dispatche `ComputeTravelTimesMessage(scope: OPPONENTS)` au worker et répond immédiatement `{queued: true}` (progression via `travelStatus`, Mercure `club:{clubId}:travel`). Cap dur `OpponentTravelResolver::MAX_OPPONENTS` avant dispatch, rate-limit `opponent_travel_resolve` par utilisateur. SEC-07. |

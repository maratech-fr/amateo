...
Last verified @ 2026-09-12 (P4-199 « règles d'import », backend seul — `documentation-update`).
Re-confronté au code, tout juste : `FbiFixtureImporter::treatOnArrival`/
`sourceIsAuthoritativeForWindow`/`stripTeamNumberSuffix` (`backend/src/Service/FbiFixtureImporter.php:614,628,658`)
✓ ; `VenueLabelNormalizer::stripTeamNumberSuffix` (`backend/src/Service/Basketball/VenueLabelNormalizer.php:47`)
✓ ; migration `Version20260912120000` (données seules, `fixture.opponent_label` +
`competition.fbi_team_label`) ✓ ; les deux canaux (`FbiFixtureImporter` xlsx et
`FfbbRencontreReconciler::newFixture`/`applyCreations`) appellent bien le même foyer ✓.

...
Last verified @ 2026-09-12 (P4-200 C1 « pont FBI → Engagements FFBB », backend seul —
`documentation-update`). Re-confronté au code, les deux lignes de route touchées : `GET
/api/ffbb/engagements` rend désormais `suggestionSource` (`FfbbEngagementsController.php`,
méthode `list`) ; `POST /api/ffbb/engagements/confirm` accepte un `competitionId` optionnel par
pairing (`resolveCompetition`) ; nouveau service `App\Service\Basketball\FbiDivisionSignature`
(stateless, dépend de `VenueLabelNormalizer`) ✓.

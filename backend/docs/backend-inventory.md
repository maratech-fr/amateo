...
Last verified @ 2026-09-17 (P2-54 « détail par côté d'un conflit », `documentation-update`). §3
« Module matchs » : ligne `/api/fixtures/conflicts` GET complétée contre le code lu —
`MatchConflictDetector::fixtureView` (`backend/src/Service/MatchConflictDetector.php`) émet
`estimatedKickoffTime`/`travelOneWayMinutes`/`matchDurationMinutes`/`opponentLabel` sur chaque côté
`left`/`right`/`fixture` des familles `MATCH_MATCH`/`MATCH_TRAINING` ; `FixtureConflictsController::decorateOpponentPlace`
décore `opponentPlace` en aval, côtés AWAY seulement, via le nouveau `App\Service\OpponentPlaceResolver`
(`backend/src/Service/OpponentPlaceResolver.php`, ordre override équipe → override club → annuaire
fédéral → salle FBI de la rencontre → `null`). Reste du fichier non re-vérifié cette passe —

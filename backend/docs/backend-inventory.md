...
Last verified @ 2026-08-25 (RMM-6 PR-1 — échéances ligue/comité : §2 ligne `Competition` (champ
`entryDeadline` hors CRUD + lecture `effectiveEntryDeadline`/`deadlineSource`) et §3 deux nouvelles
routes `POST /api/competitions/entry-deadlines`/`GET /api/matches/deadline-outlook`, lues contre
`CompetitionEntryDeadlinesController.php`/`EntryDeadlineOutlookController.php`/
`EntryDeadlineOutlook.php`/`SharedCompetitionDeadline.php` ; la nouvelle table hors-tenant
`shared_competition_deadline` confrontée à la migration `Version20260825140000.php` (GRANT
SELECT/INSERT/UPDATE, pas de DELETE) — première table communautaire hors tenant du dépôt, note
ajoutée §3. Vérification précédente toujours valable : RMM-5 PR-3 — `matchDay` du solve hebdo
DÉRIVÉ de l'image A/B, §2 lignes `TeamMatchHabit`/`MatchSlotRotation` ;
`ScheduleConstraintBuilder::deriveMatchDay` lu au code. RMM-5 PR-1 — ressource `MatchSlotRotation`,
§2, lue contre `MatchSlotRotationResource.php`/`MatchSlotRotationStateProcessor.php`/
`MatchSlotRotationStateProvider.php` ; cascades confrontées à `CascadePlan.php` +
`MatchSlotRotationTeamPruneStep.php`/`MatchSlotRotationVenuePruneStep.php`. RMM-4 PR-3, le canal API
FFBB — deux routes `GET /api/ffbb/rencontres` + `POST /api/ffbb/rencontres/apply` contre
`FfbbRencontresController.php` ; **RMM-4 est LIVRÉ EN ENTIER**. Les deux routes d'import FBI (PR-1)

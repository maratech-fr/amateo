...
Last verified @ 2026-08-24 (recalé par RMM-3 PR-1 : nouvelle route `POST /api/matches/module-visit` — `MatchModuleVisitController.php` lu en entier, endpoint POST unique confirmé, contrat de réponse confronté au corps JSON réel — et champ additif `fingerprint` sur `GET /api/fixtures/conflicts` confronté à `FixtureConflictsController.php:107-114`. PR-1 est backend seul : le RADAR frontal (badges « nouveau ») reste hors périmètre, RMM-3 n'entre pas encore comme module gradué)

...
Last verified @ 2026-09-19 (`documentation-update`, PR G « todo FBI unique »). §3 « Module matchs »
recalé contre le code lu : trois nouvelles routes `FbiCorrectionController` (registre « à corriger
dans FBI ») ajoutées ; ligne `/api/fixtures/review/deviations` recalée (`keep_app` ouvre désormais
une entrée du registre, tous champs) ; ligne `/api/matches/deadline-outlook` recalée (`fbiTodo`
additif). Reste du fichier non re-vérifié cette passe — historique des recalages précédents :
`git log -p --follow` ce fichier. Un stamp REMPLACE, l'historique vit dans git.

...
Last verified @ 2026-09-26 (`documentation-update`, passe « le présent » backend 1/4). Re-confronté
au code ce jour : titre/version OpenAPI (`config/packages/api_platform.yaml`), version API
Platform (`composer.json`), valeur courante d'`engine/CONTRACT_VERSION` /
`ScheduleConstraintBuilder::CONTRACT_VERSION`, absence de ressource `ClubUser` (membres servis par
`/api/memberships/*`), contenu de `src/DataFixtures/`, et le comportement des sections réécrites
cette passe (Import équipes, Réservation/SharedTrainingBlock, SchedulePlan/Schedule, CalendarEntry,
Module matchs). Reste du fichier non re-sondé en profondeur cette passe. Historique des passes :
...
| — | Competition | `/api/competitions` | Compétitions FFBB (championnat/coupe/brassage) — module matchs palier A. **RMM-6 (2026-08-25)** : `entryDeadline` (date club, hors CRUD, écrite par le seul `POST /api/competitions/entry-deadlines` §3) + lecture additive résolue serveur `effectiveEntryDeadline`/`deadlineSource` (`club` prime, sinon le défaut communautaire `shared_competition_deadline` — joint par `ffbbCompetitionId`, une compétition non appariée n'a jamais de `community`). **P4-199 (2026-09-12)** : `fbiTeamLabel` perd son suffixe FFBB « (n) » à la lecture (`VenueLabelNormalizer::stripTeamNumberSuffix`) ; migration `Version20260912120000` rejoue le retrait sur les libellés déjà en base — sans elle, la clé de rapprochement division↔équipe aurait changé de forme sous les appariements existants | season-scoped |
| — | Fixture | `/api/fixtures` | Rencontres (HOME/AWAY, placement domicile, `externalRef` = n° FBI, `opponentLabel` sans suffixe FFBB « (n) » depuis P4-199). Champ additif en lecture `unplacedReason` (`venue_lost`\|`null`, RMM-10) — pourquoi un match dépointé est retourné « à placer », distinct de la raison volatile d'auto-placement ; posé par `FixtureVenueLossMarker` (foyer unique), effacé par `setVenueId` dès qu'une salle non-null est reposée. **PR-3a (2026-09-08)** ajoute `reviewState` (`NEW`\|`OUT_OF_SYNC`\|`REVIEWED`), `reviewedAt` et `pendingDeviations` (un par champ date/kickoff/venue encore ouvert) — le TRAITEMENT de l'espace « Importer », axe distinct du `status` de placement ; maison unique de la règle « placer = traiter » : `Fixture::setStatus` (§ module matchs, `/api/fixtures/review*` ci-dessous). **P4-199 (2026-09-12)** : un extérieur, ou une rencontre passée/dans la semaine ISO en cours (fuseau club), naît déjà `REVIEWED` (`FbiFixtureImporter::treatOnArrival`) plutôt que `NEW` — jamais « à traiter » par le club. `opponentLabel`/`Competition.fbiTeamLabel` déjà en base sont nettoyés par la migration `Version20260912120000` (données seules, aucun changement de schéma). **P2-54 « adversaire multi-gymnases » PR-1 (2026-09-15)** ajoute deux champs additifs en lecture : `opponentOrganismeCode` (le code fédéral déjà stampé, dupliqué en lecture directe) et `opponentTeamKey` (le libellé adverse normalisé serveur, `VenueLabelNormalizer::normalize` — historique, plus consommé comme clé de jointure travel côté front depuis PR I ci-dessous). **PR I (2026-09-20, amendement « les gymnases adverses appartiennent au club, le trajet au gymnase »)** ajoute `awayTravel` (champ additif en lecture, `null` pour un domicile ou si rien n'est connu) : `{venueLabel, city, precision, oneWayMinutes, approximated, basis: linked\|most_frequent\|city}` — le trajet d'une rencontre EXTÉRIEURE, DÉRIVÉ de la rencontre elle-même (jamais du club), calculé EN BATCH par `FixtureStateProvider::decorateCollection` (`App\Service\OpponentTravelProjection::awayTravelByFixtureId`, zéro N+1, un domicile ne coûte rien). `AwayList.tsx`/`AwayTravelChip` lisent désormais `fixture.awayTravel` directement — l'ancienne jointure client par `(opponentOrganismeCode, opponentTeamKey)` contre `GET /api/opponents/travel` a disparu | Ops custom conflits + import FBI + review (§3) |
| — | TeamLink | `/api/team_links` | Pont déclaré entre deux équipes — pas d'entité joueur, le gestionnaire déclare le lien (`teamAId < teamBId` normalisé par le processor) : `NOT_SIMULTANEOUS` (double projet, jamais en même temps) ou `BACK_TO_BACK` (enchaînées, implique `NOT_SIMULTANEOUS`), plus une `trainingIntensity` (`PREFERRED`\|`MANDATORY`) dédiée côté entraînement. Consommé par `MatchPlacementPayloadBuilder` (solveur de placement, préférence SOFT `NOT_SIMULTANEOUS` conservée) **et par le solveur d'entraînement** depuis le lot passerelles PR-2 (#707) : `ScheduleConstraintBuilder::serializeTeamLinks` émet le bloc `teamLinks` du payload (roster-filtré), `MANDATORY` devient un anti-chevauchement DUR entre les deux équipes (`engine/app/solver/constraints/targeting.py::add_team_link_constraints`), `PREFERRED` un malus SOFT dérivé du tier dans l'objectif (`objective/terms.py::add_team_link_penalty`) — un verrou HARD contre un verrou HARD ne pose aucune contrainte (INFEASIBLE muet évité) mais est diagnostiqué post-solve (`result_builder._diagnose_team_links`). **Depuis le lot M (2026-09-21)** : `MatchConflictDetector` ne le consomme plus — la famille `TEAM_LINK_OVERLAP` a quitté le radar de conflits (décision fondateur), asymétrie délibérée avec le solveur qui garde sa préférence souple, [`module-matchs.md`](../../specs/courantes/module-matchs.md) §2 | |
...
**Placement précédent émis au moteur (P3-21 PR B, contrat 2.11 — terme de stabilité ; depuis P2-61 aussi proximité de placement côté moteur, poids 9, sans changement d'émission)** :
...
| `/api/venue-travel-times/autofill` | POST | `VenueTravelTimeAutofillController` | **C6 (2026-09-19)** — dispatche `ComputeTravelTimesMessage(scope: VENUE_MATRIX)` au worker, rend `{queued: true, alreadyRunning: false}` (progression/verdict par Mercure, `club:{clubId}:travel`). **Revue sécurité H** : si un calcul est déjà en cours pour le club (`TravelComputeLock::isHeld`), rien n'est dispatché — `{queued: false, alreadyRunning: true}`. Management + saison écrivable (409 archivée) + rate-limit dédié PAR UTILISATEUR (`venue_travel_time_autofill`, 10/h) ; 422 au-delà du cap de 120 paires. Une valeur `MANUAL` n'est jamais écrasée. |
...
| `/api/fixtures/conflicts` | GET | `FixtureConflictsController` | Radar : conflits d'empreinte-temps coach/joueur entre rencontres et entraînements. Chaque item porte un champ additif `fingerprint` (`ConflictFingerprinter`, maison unique) — l'identité stable du conflit, indépendante des champs gradués (severity, segment horaire, compteurs). Le radar teste la collision de gymnase (`VENUE_OVERLAP`, raison `MATCH_SLOT_WINDOW`) sur une fenêtre SALLE sans échauffement, saute le créneau d'entraînement de la PROPRE équipe du match (`MATCH_TRAINING`), et exclut du radar toute rencontre déjà jouée (`ClubDay::todayFor` du club, injecté). Chaque item porte aussi un champ additif `resolution: {status, note, updatedAt} \| null` (`null` = « à traiter », le défaut sans ligne), joint SERVEUR sur `fingerprint` contre `conflict_resolution` (club+saison courants). Chaque côté `left`/`right`/`fixture` porte quatre champs additifs émis par `MatchConflictDetector::fixtureView` (`estimatedKickoffTime`, `travelOneWayMinutes` — `null` = trajet non modélisé —, `matchDurationMinutes`, `opponentLabel`) — servis pour TOUTE famille qui partage cette vue de rencontre (`MATCH_MATCH`/`MATCH_TRAINING`, et `VENUE_OVERLAP`, qui porte aussi une mise en page dédiée côté front, `module-matchs.md` §2), plus `opponentPlace` décoré EN AVAL de la détection, sur les côtés AWAY seulement, par `App\Service\OpponentPlaceResolver` (batch, pointé sur le lien de la rencontre : `(code, libellé FBI normalisé)` → sa référence de salle fédérale → ville de la suggestion fédérale `opponent_venue_suggestion` ; sinon ville de l'annuaire fédéral `opponent_directory` ; sinon `null` — le libellé de gymnase n'est jamais servi, la VILLE seule). `MATCH_MATCH` et le côté match de `MATCH_TRAINING` testent le chevauchement sur `MatchFootprint::personConflictOccupancy`/`personConflictOccupancyAt` (occupation MOINS l'échauffement, trajet AWAY conservé) — une personne engagée deux fois n'est en conflit que si son arrivée dépasse le coup d'envoi du second engagement, **quel que soit le gymnase** (aucune exception « même gymnase », la règle générale SUBSUME cette ancienne exception). La famille `TEAM_LINK_OVERLAP` a quitté le radar (`ConflictRadarLoader` ne charge plus `TeamLink`) — détail [`module-matchs.md`](../../specs/courantes/module-matchs.md) §2 « Détecteur de conflits ». |
| `/api/fixtures/conflicts/{fingerprint}/resolution` | PUT | `FixtureConflictsController` | Le gestionnaire pose (ou remplace) le statut de traitement d'un conflit (jamais un masquage : le radar continue de le rendre). SEC-07 (`assertManager()`) tiré **en premier**, avant tout autre contrôle. Corps `{status, note?, fbiCorrection?}` — `status` l'un des **huit** cas stockables (`ConflictResolutionStatus`) : trois de BASE sur toute famille (`DEROGATION_REQUESTED`/`RESOLVED_INTERNALLY`/`NO_SOLUTION_YET`, « à traiter » n'en est pas un — c'est l'absence de ligne) ; deux réservés aux conflits de PERSONNE où un côté servi porte le rôle PLAYER (`COACHES_NOT_PLAYING`/`PLAYS_NOT_COACHING`, 422 sinon) ; trois propres à une famille — `IMPORT_MISSING_MATCHES` (`COMPETITION_INCOMPLETE`), `FBI_ERROR`/`MATCH_TO_MOVE` (`VENUE_OVERLAP`). `ConflictResolutionStatus::casesForFamily` donne, par famille, les cas autorisés — hors table → 422. `fbiCorrection: {fixtureId, field}` (`FBI_ERROR` seulement — envoyé avec un AUTRE statut → **422 explicite**) ouvre — atomiquement avec la résolution, via `FbiCorrectionLedger::declareFromConflict` — une entrée du registre « à corriger dans FBI » à valeur cible VIDE pour le côté du conflit désigné ; `fixtureId` doit être l'un des côtés du conflit courant sinon 422. **Une seule déclaration « erreur FBI » vivante PAR CONFLIT** : l'entrée porte l'empreinte du conflit (`FbiCorrection.conflictFingerprint`, nullable) ; re-déclarer sur ce même conflit (autre rencontre/champ) ferme la déclaration précédente (`closed_by=redeclared`, `FbiCorrectionCloseSource::REDECLARED`, définitif) avant d'ouvrir la nouvelle. Note > 500 caractères → 422 ; l'empreinte DOIT appartenir au flux COURANT du club+saison (recalculé comme le GET) sinon 422 (jamais un statut fantôme). Upsert sur la clé unique (club, saison, empreinte) ; **pose concurrente rattrapée** : une violation d'unicité au flush (deux `PUT` simultanés) est capturée, la ligne gagnante relue sur la connexion vivante (RLS) et rendue en **200 idempotent** au lieu d'une 500. Empreinte contrainte par la route (`[A-Z_]+:[0-9a-fA-F,:-]+`) — malformée → 404 de routing, jamais l'action. |
| `/api/fixtures/conflicts/{fingerprint}/resolution` | DELETE | `FixtureConflictsController` | Retour à « à traiter » (suppression de la ligne). SEC-07. **Idempotent** : 204 même sans ligne, y compris sur une empreinte disparue du flux. **N'appelle jamais `FbiCorrectionLedger`** — décision fondateur explicite, asymétrie volontaire : remettre un conflit « erreur FBI » à traiter ne ferme pas l'entrée du registre qu'il a pu ouvrir — aucun lien persisté entre les deux, pour ne jamais perdre le rappel d'une erreur FBI réelle déclassée par erreur ; seule une RE-déclaration sur ce conflit (ci-dessus) ou une fermeture depuis la liste referme l'entrée. Table `conflict_resolution` : TENANT (RLS `FORCE`, patron `opponent_venue_link`), unique `(club_id, season_id, fingerprint)`, jamais nettoyée à la volée sur un orphelin (empreinte disparue) — purgée avec la saison (`SeasonDataPurger`) ou l'effacement RGPD du club (`ErasedClubPurger`). |
| `/api/matches/module-visit` | POST | `MatchModuleVisitController` | Le gardien à l'ouverture : stampe une référence de visite PAR (club, saison, utilisateur) et rend le delta « depuis ta dernière visite » (`newFixturesCount`, `newConflictFingerprints`, `planningChanged`). Un seul endpoint POST (pas de GET séparé, pour ne pas ouvrir de course F5 entre lire et tourner la référence) ; première visite muette ; fenêtre de grâce glissante de 30 min (rejoue les mêmes badges sans les éteindre) ; calcul du delta délégué à `MatchModuleDeltaComputer`, coupé de la rotation pour que RMM-6 puisse un jour lire sans stamper. Ouvert au Membre (aucune garde management), écrit même saison archivée (bookkeeping utilisateur, pas une mutation du planning). |
...
| `/api/fixtures/import/analyze` | POST | `ImportFixturesAnalyzeController` | **Dry-run** de l'export FBI global club : table des divisions résolue contre la correspondance persistée (`Competition`), zéro écriture. Rend aussi `deviations` — les écarts de réconciliation (date/heure/salle) des domiciles déjà **placés**, état app VS état fichier, à décider à l'import. **SEC-22** : bornes d'upload posées par la maison unique `XlsxUploadGuard` — 2 Mo d'octets · 20 Mo cumulés une fois DÉCOMPRESSÉ · 5 000 lignes, refus **413** nommant le chiffre dépassé. La borne n'accorde aucune confiance aux en-têtes : elle inflate le zip avec un compteur borné et compte les `<row` du flux, jamais les tailles déclarées ni le `<dimension ref>` (qui peut mentir). Un fichier qui n'est pas un zip lisible n'est PAS refusé ici (rien à inflater) : il part au parseur en aval, dont le filet générique P4-5 rend 422 sans fuite. Limiteur `xlsx_import` (30/h par utilisateur) consommé APRÈS l'auth, pour qu'un refus d'accès ne dépense pas le budget d'import. |
| `/api/fixtures/import` | POST | `ImportFixturesController` | Import FBI **une passe** (fichier global + `mappings` JSON) : persiste les correspondances puis crée/**met à jour** par diff `(team, n° FBI)`. Rapport `created`/`updated`/`unchanged`/`exempted`/`warnings`/`unmappedDivisions`/`errors`. Remplace l'ancienne route par équipe `/api/teams/{id}/fixtures/import`. Accepte en plus `decisions` JSON (`{fixtureId, field: date\|kickoff\|venue, choice: keep_app\|take_file}`, `FbiFixtureImporter::import`) — un écart d'un domicile déjà placé **sans décision n'est plus écrasé en silence**, il reste intact et remonte dans `unresolvedDeviations` du rapport (avec `depositedAt`). `take_file` sur `date`/`venue` dé-place la rencontre (comme un HOME↔AWAY switch) ; sur `kickoff` la rencontre reste placée mais un `SUBMITTED`/`VALIDATED` retombe `PLACED`. Un domicile `PLACED`/`SUBMITTED` que le fichier renvoie identique sur date + heure + salle (les trois présents) passe `VALIDATED` — plus un geste, un effet de l'import (`FbiFixtureImporter::reconcileNoDivergence`/`sourceAttestsPlacement`). Chaque dépôt écrit une `FbiIngestion` (club+saison, `source=FBI_XLSX`, fraîcheur + compteurs seulement — **la trace des écarts vit sur la rencontre**, `Fixture.pendingDeviations`) ; salle comparée en **fuzzy containment** (`FbiFixtureImporter::venueMatches`), pas égalité stricte. La saison de l'ingestion est **celle de la requête** (`SeasonResolver::selectedOrCurrent`), jamais devinée depuis les lignes importées (revue sécurité). Les règles de naissance/fenêtre « source fait foi » sont détaillées dans [`module-matchs.md`](../../specs/courantes/module-matchs.md) §7 « Écran Importer ». Un existant matché resté `NEW` d'un dépôt antérieur est rattrapé (`REVIEWED` + horodaté) en tête d'`applyDiff`, foyer `FbiFixtureImporter::catchUpReview` — voir aussi la commande `app:fixtures:catch-up-review` ([`commands.md`](commands.md)) pour un rattrapage sans re-dépôt. **SEC-22** : bornes d'upload posées par la maison unique `XlsxUploadGuard` — 2 Mo d'octets · 20 Mo cumulés une fois DÉCOMPRESSÉ · 5 000 lignes, refus **413** nommant le chiffre dépassé. La borne n'accorde aucune confiance aux en-têtes : elle inflate le zip avec un compteur borné et compte les `<row` du flux, jamais les tailles déclarées ni le `<dimension ref>` (qui peut mentir). Un fichier qui n'est pas un zip lisible n'est PAS refusé ici (rien à inflater) : il part au parseur en aval, dont le filet générique P4-5 rend 422 sans fuite. Limiteur `xlsx_import` (30/h par utilisateur) consommé APRÈS l'auth, pour qu'un refus d'accès ne dépense pas le budget d'import. |
...
| `/api/fixtures/league-validation` | GET | `LeagueValidatedFixturesController` | Un club qui démarre l'application EN COURS de saison importe des domiciles déjà datés côté fédération (date + heure + gymnase déjà enregistrés). Rend une lecture détaillée **par championnat ÉCHU** (`{matured[]: {competitionId, name, deadline, deadlineSource, validatableCount}, toTreat[]: {fixtureId, teamId, competitionName, matchDate, opponentLabel, reason}, missingDeadline[]: {competitionId, name, validatableCount}, totalValidatable}`) — l'échéance EFFECTIVE du championnat (`CompetitionDeadlineResolver::resolve`, jour inclus) décide QUELS championnats sont proposés, un championnat à échéance future n'apparaît nulle part ; les domiciles échus qui échouent au prédicat (heure ET `venueId` présents, aucun écart en attente — **aucun contrôle des créneaux d'accès match, divergence ASSUMÉE** avec le geste unitaire, signalée par le radar `ACCESS_WINDOW_LOST` plutôt que bloquée) sont NOMMÉS avec leur raison, jamais tus. SEC-07 + saison écrivable + `SocleGuard` + **409 explicite si aucune saison ne se résout** (défense en profondeur, ne dépend plus de la seule activation du filtre Doctrine). Route statique `priority: 10` pour gagner sur `/api/fixtures/{id}` d'API Platform. |
| `/api/fixtures/league-validation` | POST | `LeagueValidatedFixturesController` | **Sans corps** : recalcule les championnats échus AU MOMENT DE L'APPEL (`maturedCompetitionIds`, horloge injectée + échéances du moment, jamais ce que le client aurait pu croire savoir), puis bascule en lot chaque domicile éligible d'un championnat échu (même prédicat que le GET) : `Fixture::setPlacementSource(MANUAL)` PUIS `Fixture::setStatus(VALIDATED, …)` (ancre requise par le cadenas de grille et par l'ancre `FIXED` du solveur de placement, §3). **Exception CONSENTIE** à « une rencontre naît avec son gymnase mais jamais placée d'office » — le chemin de création de l'import (`FbiFixtureImporter::attachConfirmedVenue`) N'EST PAS modifié, il continue de créer en `UNPLACED` ; c'est CE geste séparé, explicite et chiffré (jamais posé au chemin de création) qui valide. **On ne dévalide jamais** : le prédicat ne regarde que les domiciles `UNPLACED`, rejouable sans effet sur une rencontre déjà `VALIDATED` (par ce geste, l'ancienne règle ou la réconciliation D9). Rend `{confirmed}`. SEC-07 + saison écrivable + `SocleGuard` + **409 explicite si aucune saison ne se résout** ; **409 lisible en français sur une collision d'écriture simultanée** (verrou optimiste `Fixture`, aucun verrou Redis ajouté — écriture idempotente). Côté écran : `PlacementPanel` offre désormais « Corriger — repasser en Placé » sur toute rencontre `VALIDATED`, quel que soit le chemin qui l'a posée (réconciliation D9 ou ce geste) — une bascule en lot peut reposer sur un gymnase apparié automatiquement. |
...
| `/api/ffbb/rencontres/apply` | POST | `FfbbRencontresController` | RE-FETCHE côté serveur (jamais les valeurs du client), applique les décisions par écart (mêmes `{fixtureId, field, choice}` que l'import xlsx) et crée les rencontres choisies (`{rencontreId, teamId}`, idempotent sur l'index unique partiel `uniq_fixture_ffbb_rencontre`). Partage le MÊME moteur de traitement que le xlsx (`processPerimeterFields`/`reconcileNoDivergence`) — pose `reviewState`/`pendingDeviations` sur la rencontre et valide au même titre que le xlsx ; seuls les warnings du moteur partagé ne sont pas remontés par ce canal (non consommés par l'écran). Écrit une `FbiIngestion` datée `source=FFBB_API` (compteurs seuls, pas de `pendingDeviations`) — ne touche JAMAIS la fraîcheur xlsx (`fbi-ingestions/latest` ne lit que `FBI_XLSX`). SEC-07 + saison écrivable + `SocleGuard` + tenant ; 409 doublon concurrent (collision sur l'index unique). Même moteur de naissance/fenêtre que le xlsx (`FbiFixtureImporter::treatOnArrival`/`sourceIsAuthoritativeForWindow`, appelé par `FfbbRencontreReconciler`) — détail [`module-matchs.md`](../../specs/courantes/module-matchs.md) §7 « Écran Importer ». `FfbbRencontreReconciler` rattrape aussi un existant matché resté `NEW` (`catchUpReview`, avant le tri de périmètre) — même foyer, même effet que le canal xlsx. |
...
| `/api/matches/deadline-outlook` | GET | `EntryDeadlineOutlookController` | L'outlook cockpit, lecture seule, ouvert au Membre : pour chaque échéance EFFECTIVE (club sinon défaut communautaire) encore due, ses compétitions, le nombre de domiciles restant à saisir et si la fenêtre J-7 (`EntryDeadlineOutlook::REMINDER_WINDOW_DAYS`) est ouverte. Champ additif `fbiTodo {toEnter, toCorrect}` — le compte GLOBAL « à faire dans FBI » toutes semaines (domiciles PLACED · entrées ouvertes du registre « à corriger dans FBI »), pour que le cockpit et la barre de compteurs du Calendrier n'aient jamais à charger les fixtures. Une fenêtre ouverte ET une référence de visite existante joignent le delta gardien (`MatchModuleDeltaComputer`, réutilisé) **sans stamper** — maison unique du J-7, le front ne recalcule rien. |
...
| `/api/opponents/refresh` | POST | `OpponentRefreshController` | L'orchestrateur « Mettre à jour les adversaires » : UN appel enchaîne trois passes best-effort **indépendantes** (l'échec de l'une n'annule pas les autres) — (a) rattrapage des codes fédéraux (`OpponentLocationResolver::resolveObservations`) ; (b) auto-localisation des gymnases depuis le libellé du fichier FBI (`App\Service\Geo\OpponentVenueAutoLocator::locate` — par groupe AWAY `(code, libellé normalisé)`, égalité STRICTE contre les salles fédérales de l'annuaire adverse — CP sinon rayon 10 km —, hit UNIQUE → `OpponentVenueLink` `source=AUTO`, club-scoped SANS saison, 0/≥2 hits ou deux libellés → deux salles = rien, lien MANUAL jamais touché, n'écrit JAMAIS le partagé `opponent_venue_suggestion`) ; (c) dispatch du calcul des trajets manquants (`OpponentTravelResolver::resolve`, worker asynchrone, § ci-dessous). SEC-07 d'abord, cap dur `MAX_DISTINCT=200` (même borne que `/opponents/resolve`, avant tout réseau) puis limiteur `opponent_refresh` 10/h par utilisateur. Budget de MUR partagé entre les trois passes (`OpponentRefreshController::REFRESH_BUDGET_SECONDS = 45`, trois quarts du plafond amont 60 s), threadé à `OpponentLocationResolver::resolveObservations`, `OpponentVenueAutoLocator::locate` et `OpponentTravelResolver::resolve` — au-delà, chaque passe rend ce qu'elle n'a pas traité (`unresolved`/`skipped`) plutôt que de dépasser la deadline ; `OpponentTravelResolver::MAX_OPPONENTS=60` appliqué DANS `resolve()` (même cap que la route fine `/opponents/travel/resolve` ci-dessous, tenu aussi côté orchestrateur). Réponse `{codes, autoLocated, travel}`, `travel: {queued, alreadyRunning, pending}`. Si un calcul de trajets tourne déjà pour ce club, la passe (c) ne dispatche rien — `travel.alreadyRunning: true` — sans annuler (a)/(b), qui tournent toujours. Consommé depuis le front par le bouton unique « Mettre à jour les adversaires » (`/matchs/adversaires`) — les routes fines (`/opponents/resolve`, `/opponents/travel/resolve`) restent en place (compat). |
...
| `/api/opponents/{code}/venues` | POST | `OpponentTravelController` | Ajoute un gymnase au club adverse `{code}` (ref fédérale ou coordonnées choisies dans `/api/ffbb/salles`), lien MANUAL. Le libellé de fichier est optionnel (repli sur le libellé du gymnase choisi) ; **jamais validé contre les rencontres de la saison** (à dessein — ajouter un gymnase avant toute rencontre reste possible, cas playoff). 422 si `{code}` n'est pas un adversaire AWAY de la saison, gymnase/libellé invalide, ou **plafond `OpponentVenueLinkManager::MAX_VENUES_PER_OPPONENT = 20`** dépassé par une NOUVELLE clé de libellé (l'actualisation d'un libellé déjà apparié reste libre). SEC-07, SEC-19 (limiteur `opponent_travel_manual` 30/h, recablé sur les trois gestes d'écriture ci-dessous — chaque appel chauffe un itinéraire IGN et peut écrire au partagé). Comptabilité du catalogue partagé IDEMPOTENTE et SYMÉTRIQUE par `(club, code, ref)` (`OpponentVenueLinkRepository::countManualByRef`) : un `ref` n'est persisté sur le lien que s'il résout fédéralement (`OpponentTravelResolver::resolveFederalVenue`), un ref présent implique donc toujours un crédit passé. `{code}` accepte aussi la clé SENTINELLE d'un adversaire sans code fédéral (`App\Service\OpponentPairingKey`). ⚠ **La neutralisation de `ref` vit ENTIÈREMENT dans la MAISON UNIQUE `OpponentVenueLinkManager::writeGym`** (AVANT toute résolution fédérale, pour tous les appelants) — le contrôleur ne porte **aucun** filtre propre : un filtre contrôleur en défense en profondeur court-circuiterait le manager et rendrait sa garde intestable sur ce chemin (falsifié par `OpponentVenueSuggestionShareTest`, rouge sur la garde du manager désactivée seule — piège gardé dans `.claude/rules/backend.md`). Un sans-code ne peut jamais créditer le catalogue partagé. |
| `/api/opponents/{code}/venue-links` | POST | `OpponentTravelController` | Apparie un libellé de fichier ORPHELIN (« à apparier ») à un gymnase choisi ; 422 si le libellé n'est celui d'aucune rencontre AWAY réelle de cet adversaire (jamais un libellé arbitraire imposé au partagé par une requête forgée), ou plafond `MAX_VENUES_PER_OPPONENT` dépassé. Même limiteur/gardes/comptabilité que `/venues`, y compris la neutralisation de `ref` pour une clé SENTINELLE (voir `/venues` ci-dessus). SEC-07. |
| `/api/opponents/venue-links/{id}` | PUT | `OpponentTravelController` | Ré-apparie/fusionne un lien EXISTANT vers un AUTRE gymnase (la clé de libellé est conservée, reconnue au prochain import) ; réponse `targetFixtureCount` = le compte RÉSULTANT du gymnase cible (« en portera N », toutes les rencontres AWAY dont un libellé pointe désormais ce même gymnase). 404 byte-identique si le lien n'appartient pas au club (RLS). Comptabilité partagée idempotente : ne crédite le nouveau ref que si le club ne le porte pas déjà, ne débite l'ancien que si plus aucun AUTRE lien du club ne le porte. Pour un lien sous clé SENTINELLE (adversaire sans code), `OpponentVenueLinkManager::writeGym` neutralise `ref` AVANT résolution fédérale (empêche une ref fédérale brute client d'atteindre le catalogue partagé `opponent_venue_suggestion`, hors-tenant, sans GRANT DELETE, sous un code organisme inexistant) — même garde pour cette route et pour `/venues` (NR `OpponentVenueSuggestionShareTest`). SEC-07, SEC-19. |
| `/api/opponents/venue-links/{id}` | DELETE | `OpponentTravelController` | Remplace l'ancienne route `POST /api/opponents/travel/auto` — retire l'appariement LOCAL (jamais le catalogue fédéral `opponent_venue_suggestion`) ; décrémente le partagé si le lien retiré était MANUAL, portait une ref fédérale, ET était le DERNIER lien du club sur ce ref (symétrie du crédit posé par `/venues`/`/venue-links`). 204 même 404 byte-identique cross-club. SEC-07. |
| `/api/opponents/travel/resolve` | POST | `OpponentTravelController` | (management) Ne recalcule pas TOUT : `OpponentTravelResolver::resolve` route seulement les paires siège→gymnase MANQUANTES du cache (pas de notion d'équipe ni de ligne saison) ; une paire déjà résolue n'est jamais retouchée (un trajet est une constante). Le filtre amont (`pairsToRoute`) route les paires par code fédéral OU par clé SENTINELLE (`OpponentTravelResolver::awayPairingKeys`, un gymnase épinglé sur un adversaire SANS code fédéral entre aussi dans le recalcul), le repli VILLE restant réservé aux codes fédéraux (un sans-code n'a pas d'entrée d'annuaire). **Rail asynchrone** : dispatche `ComputeTravelTimesMessage(scope: OPPONENTS)` au worker et répond immédiatement `{queued: true, alreadyRunning: false}` (progression via `travelStatus`, Mercure `club:{clubId}:travel`) ; si un calcul tourne déjà pour ce club, rien n'est dispatché — `{queued: false, alreadyRunning: true}`. Cap dur `OpponentTravelResolver::MAX_OPPONENTS` avant dispatch, rate-limit `opponent_travel_resolve` par utilisateur. SEC-07. |
| `/api/opponents/{code}/venue-suggestions` | GET | `OpponentTravelController` | Les suggestions PARTAGÉES de gymnases de l'adversaire `{code}` : table GLOBALE `opponent_venue_suggestion` (hors tenant, patron `opponent_directory` — pas de `club_id`, pas de RLS, GRANT sans DELETE, **sans backfill** — les valeurs saisies par des clubs sont interdites au partagé), deux sources `FFBB_API` (observé, sans ref — le hit rencontre ne porte pas le `numero`) / `MANUAL` (choisi via `/api/ffbb/salles` : `Service/Basketball/FfbbSalleResolver` RE-RÉSOUT le `numero` côté serveur — `_geoRadius` + égalité stricte, jamais le libellé/coords du corps client ; ref inconnue/FFBB muet → tenant seul, rien au partagé), un `chosenByCount` **jamais un « qui »** — compte des CHOIX (un par lien MANUAL club×libellé qui pointe ce gymnase — un club avec deux libellés vers le même gymnase compte deux fois, pas de notion de saison/équipe), pas des clubs distincts (une ligne à 0 reste, jamais supprimée). `lastChosenAt` au JOUR seul. Triées `FFBB_API` puis `MANUAL` par compte décroissant. 422 si `{code}` n'est pas un adversaire AWAY de la saison. management SEC-07. Gate NR bloquant : `OpponentVenueSuggestionShareTest`. |
...
APRÈS l'auth : un 403/404/409 ne dépense pas le budget.

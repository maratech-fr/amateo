...
Last verified @ 2026-09-14 (D2 « rattrapage des statuts de traitement », backend seul —
`documentation-update`). Re-confronté au code, les deux lignes touchées : `POST /api/fixtures/import`
et `POST /api/ffbb/rencontres/apply` rattrapent désormais un existant matché resté `NEW`
(`FbiFixtureImporter::catchUpReview`, appelé en tête d'`applyDiff` et dans
`FfbbRencontreReconciler::reconcile` avant le tri de périmètre) ; nouvelle commande console
`app:fixtures:catch-up-review` (`backend/src/Command/CatchUpFixtureReviewCommand.php`) ✓.

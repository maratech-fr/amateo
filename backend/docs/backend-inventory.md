...
Last verified @ 2026-08-31 (P2-51 PR-1, `documentation-update` — nouvelle ligne `SharedTrainingBlock`
confrontée au code : entité + `SharedTrainingBlockTeam` (`backend/src/Entity/`), ressource
`SharedTrainingBlockResource` (`/api/shared_training_blocks`, GetCollection/Get/Post/Put/Delete),
`SharedTrainingBlockStateProcessor` (garde centrale Σ, multi-appartenance permise, ensemble
dupliqué, équipe inconnue/inactive), migration `Version20260831120000.php` (RLS FORCE + `admin_all`
sur les deux tables), `SharedTrainingBlockPruneStep` (mort entière), présence confirmée dans
`OverlayManager`/`SeasonDataPurger`/`ResourceChangeStaleScheduleListener` — tous vérifiés `file:line`
lors de cette passe. Reste antérieur (le reste de l'inventaire) non re-vérifié cette passe — un
stamp REMPLACE, l'historique vit dans git : `git log -p --follow backend/docs/backend-inventory.md`)
...
| — | SharedTrainingBlock | `/api/shared_training_blocks` | **P2-51 PR-1 (2026-08-31, modèle backend seul)** — le BLOC de mutualisation : un ensemble d'équipes (2..10) qui se comporte comme **UNE équipe à part entière**, avec son propre `commonSessions` — les séances lui APPARTIENNENT (le solveur les placera comme celles d'une équipe, PR-3), on ne déduit plus une séance commune de la co-présence (dissout le double-comptage du modèle groupe). Nouvelle paire d'entités, `SharedTrainingGroup` ci-dessus reste INTACT. GetCollection/Get/Post/Put/Delete. `schedulePlanId` nullable, MÊME patron que `SharedTrainingGroup` (NULL = socle saison, non-null = plan de période) — fait de PLAN, PAS de saison, aucune copie socle→période (décision fondateur 2026-08-31). **Multi-appartenance PERMISE** (pas d'unicité un-bloc-par-équipe — c'est la capacité qui manquait au groupe) : la garde porte sur `(block_id, team_id)`, jamais sur `team_id` seul. 422 métier (processor `SharedTrainingBlockStateProcessor`) : équipe inconnue du club+saison ou inactive, ensemble d'équipes déjà déclaré (identique) dans la même portée, **garde centrale Σ** — pour chaque équipe, la somme des `commonSessions` de ses blocs de MÊME portée (celui écrit compris) ne dépasse pas ses séances/semaine EFFECTIVES (`EffectiveTeamSessions`, override de période inclus). Un bloc meurt **ENTIER** quand une équipe membre est supprimée (`SharedTrainingBlockPruneStep` — patron du prune group, PAS le seuil de survie à 2 de la rotation match) ; purgé avec le plan (`OverlayManager`) et avec la saison (`SeasonDataPurger`) ; toute écriture marque les plannings `COMPLETED` du bon périmètre périmés (`ResourceChangeStaleScheduleListener`). Deux tables sous RLS FORCE (`shared_training_block`/`shared_training_block_team`, migration `Version20260831120000`). **Aucun payload backend⇄engine ni sémantique solveur à ce stade** (PR-2/PR-3) ; détail du modèle amendé : [`mutualisation-par-creneau.md`](../../specs/evolution/mutualisation-par-creneau.md) §0. | |

...
`documentation-update` — clôt le lot 2). §3 « Module matchs » : nouvelle ligne `/api/opponents/refresh`
(`OpponentRefreshController`, cap 200, limiteur `opponent_refresh` 10/h) et ligne
`/api/opponents/resolve` reconfirmée (`OpponentResolveController.php:39`, `MAX_DISTINCT=200`, cap
gardé par `OpponentResolveCapTest.php`). Reste du fichier non re-vérifié cette passe — historique
des recalages précédents : `git log -p --follow` ce fichier. Un stamp REMPLACE, l'historique vit
dans git.

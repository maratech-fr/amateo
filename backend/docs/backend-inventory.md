...
`documentation-update`). § route `POST /api/fixtures/place` recalée : `MatchPlacementPayloadBuilder`
injecte désormais `MatchDurationResolver` (`MatchPlacementPayloadBuilder.php` constructeur + assemblage
`teamRows`) et émet `matchMinutes`/`warmupMinutes` par équipe (catégorie sinon défaut de famille via
`MatchDurationResolver::resolve`) ✓. Passe précédente (E1, ré-affectation d'un libellé de salle) :
`GET /api/venues/fbi-labels` (`VenueExternalLabelController::inventory`, `priority: 10`) sert
`App\Service\Basketball\VenueLabelInventory::forSeason` ✓ ; le POST `/api/venues/{id}/external-labels`
accepte `reassign: true` (`VenueExternalLabelController::reassignHomeFixtures`) — retire l'alias de
l'ancien porteur, re-pointe les domiciles `FixtureStatus::UNPLACED` du même libellé quel que soit
leur gymnase, laisse intacts les PLACED/SUBMITTED/VALIDATED ✓ ; le DELETE reste sans effet sur les
rencontres déjà rattachées ✓.

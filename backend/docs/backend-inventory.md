...
Last verified @ 2026-09-02 (PR-3 lot overlay, `documentation-update`). Ligne `/api/schedules/{id}/fill`
re-confrontée au code : le handler ne saute plus `withPreviousAssignments` sans rien y substituer —
en fill il pose désormais `withSocleReferenceAssignments` (`GenerateScheduleHandler.php:262`,
`ScheduleConstraintBuilder.php:720`), et « contrat inchangé » était devenu FAUX (contrat 2.20 pour
ce bloc, `CONTRACT_VERSION` corrigée là où elle citait encore 2.19). Nouveau paragraphe dédié pour
`socleReferenceAssignments`, miroir de celui de `previousAssignments`. Reste de l'inventaire non

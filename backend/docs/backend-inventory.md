...
Last verified @ 2026-09-04 (D3 v1 PR-2 cockpit ; P4-172 concurrence des fenêtres de plan). Section
« Cockpit temporel » recalée : D3 v1 est SOLDÉE ENTIÈRE (PR-1 backend + PR-2 cockpit) — le geste
d'édition à l'écran n'est plus « ouvert », il pointe désormais `accueil-cockpit-temporel.md` §5bis.
Les trois entrées SchedulePlan/CalendarEntry/re-datage CLOSURE recalées pour le second verrou
`lockClubWindows` (grain club+saison, avant `lockPlanScope`) confronté à
`SchedulePlanProvisioner.php:410`, `SchedulePlanStateProcessor.php:100`,
`CalendarEntryStateProcessor.php:232,292`. ⚠ Vérification volontairement ÉTROITE au-delà de ces
points : le reste de l'inventaire n'a pas été reconfronté au code ce jour — historique des passes :

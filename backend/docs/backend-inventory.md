...
Last verified @ 2026-09-15 (P4-207 « résolution des conflits, backend seul », `documentation-update`).
§ route `GET /api/fixtures/conflicts` recalée : chaque item gagne le champ additif `resolution`,
joint SERVEUR par `ConflictResolutionRepository::mapByFingerprint` (`FixtureConflictsController.php`)
✓. Deux nouvelles routes ajoutées : `PUT`/`DELETE /api/fixtures/conflicts/{fingerprint}/resolution`
(même contrôleur, `assertManager()` en premier, empreinte contrainte par la route, upsert sur la clé
unique club+saison+empreinte, DELETE idempotent) ✓ — déclarées dans `SeasonAndFixturePaths.php` et
présentes au snapshot OpenAPI. Table `conflict_resolution` : RLS ACTIVE (`Version20260915120000`,
patron `opponent_travel`) ✓, purgée par `SeasonDataPurger::purge` (`ConflictResolution` dans la
liste `deleteByClubSeason`, `SeasonDataPurger.php:146`) ✓.

...
| `/api/opponents/{code}/venues` | POST | `OpponentTravelController` | **PR I (2026-09-20), remplace `POST /api/opponents/travel/manual`** — ajoute un gymnase au club adverse `{code}` (ref fédérale ou coordonnées choisies dans `/api/ffbb/salles`), lien MANUAL. Le libellé de fichier est optionnel (repli sur le libellé du gymnase choisi). 422 si `{code}` n'est pas un adversaire AWAY de la saison, ou gymnase/libellé invalide. SEC-07, SEC-19 (limiteur `opponent_travel_manual` 30/h, recablé sur les trois gestes d'écriture ci-dessous — chaque appel chauffe un itinéraire IGN et peut écrire au partagé). |
| `/api/opponents/{code}/venue-links` | POST | `OpponentTravelController` | **PR I (2026-09-20)** — apparie un libellé de fichier ORPHELIN (« à apparier ») à un gymnase choisi ; 422 si le libellé n'est celui d'aucune rencontre AWAY réelle de cet adversaire (jamais un libellé arbitraire imposé au partagé par une requête forgée). Même limiteur/gardes que `/venues`. SEC-07. |
...
| `/api/opponents/venue-links/{id}` | DELETE | `OpponentTravelController` | **PR I (2026-09-20), remplace `POST /api/opponents/travel/auto`** — retire l'appariement LOCAL (jamais le catalogue fédéral `opponent_venue_suggestion`) ; décrémente le partagé si le lien retiré était MANUAL avec une ref fédérale. 204 même 404 byte-identique cross-club. SEC-07. |

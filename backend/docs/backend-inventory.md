...
code postal de l'adversaire », `e60fbb1f`). Ligne `GET /api/opponents/travel` recalée cette passe :
`postalCode` (additif, `OpponentDirectoryEntry::getPostalCode`) ajouté au schéma servi par club
(`OpponentTravelController.php:395`). Reste confronté à la passe précédente, même jour
(`documentation-update`, PR I « les gymnases adverses appartiennent au club, le trajet au
gymnase » — amendement fondateur, `3993cbd3`…`3e203402`) : §3 « Module matchs »
recalé contre le code lu cette passe : `OpponentVenueLink` (tenant, club-scoped SANS saison) a
remplacé `OpponentTravel` (supprimée, migration `Version20260920140000`) — 4 routes neuves sur
...
| `/api/opponents/travel/resolve` | POST | `OpponentTravelController` | (management) **C5/C6 (2026-09-19), amendé PR I** — ne recalcule plus TOUT : `OpponentTravelResolver::resolve` route seulement les paires siège→gymnase MANQUANTES du cache (plus de notion d'équipe ni de ligne saison depuis PR I) ; une paire déjà résolue n'est jamais retouchée (un trajet est une constante). **Quitte le rail synchrone** : dispatche `ComputeTravelTimesMessage(scope: OPPONENTS)` au worker et répond immédiatement `{queued: true, alreadyRunning: false}` (progression via `travelStatus`, Mercure `club:{clubId}:travel`). **Revue sécurité H** : si un calcul tourne déjà pour ce club, rien n'est dispatché — `{queued: false, alreadyRunning: true}`. Cap dur `OpponentTravelResolver::MAX_OPPONENTS` avant dispatch, rate-limit `opponent_travel_resolve` par utilisateur. SEC-07. |

...
statuts de traitement », backend seul — `documentation-update`, rebase D1 sur D2). Re-confronté au
code : la ligne `/api/fixtures/conflicts` — le contrôleur injecte `App\Service\ClubDay`
(`FixtureConflictsController.php`, constructeur) pour poser `clubToday` sur
`MatchConflictDetector::detect()`, qui filtre le passé hors de toutes les familles sauf
`COMPETITION_INCOMPLETE` et l'index week-end de `FRIENDLY_ON_MATCH_SLOT` ; `MatchFootprint` porte deux
fenêtres (`occupancy`/`occupancyAt` = personne, `venueOccupancy`/`venueOccupancyAt` = salle seule) ;
`MatchModuleDeltaComputer` reçoit la même `ClubDay` ✓. Les lignes `POST /api/fixtures/import` et
`POST /api/ffbb/rencontres/apply` rattrapent un existant matché resté `NEW`
(`FbiFixtureImporter::catchUpReview`) ; commande `app:fixtures:catch-up-review`
(`backend/src/Command/CatchUpFixtureReviewCommand.php`) ✓.

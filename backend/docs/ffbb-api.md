...
(`Controller/Basketball/FfbbRencontresController.php:66,87` — **ligne recalée**, avaient dérivé à
60/81) ✓ · `PATCH /api/club/info` bien SUPPRIMÉ — zéro occurrence dans `src/` ✓. Non re-sondé cette
passe : fallback `FFBB_MEILISEARCH_TOKEN`, `FfbbClubPopulator::applyClub`, `FfbbEngagementsController`,

...
conflits, frontend » — fichier hors sujet). `CONTRACT_VERSION` avait dérivé à `2.21`
(`ScheduleConstraintBuilder.php:64` ⇄ `engine/CONTRACT_VERSION`) — corrigé aux 4 occurrences ✓ ·
TTL du verrou = `timeoutSeconds + LOCK_TTL_MARGIN_SECONDS` (`GenerateScheduleHandler.php:62,117`,
marge de 60 s) ✓ · `ScheduleStatus` à cinq états exactement (`backend/src/Enum/ScheduleStatus.php`) ✓.
...
7. **Métadonnées** : ajoute `version: "2.21"` (`ScheduleConstraintBuilder::CONTRACT_VERSION`, alignée sur `engine/CONTRACT_VERSION`), `clubId`, `seasonId`, `solverSeed` et `solverTimeoutSeconds`.
...
| **Payload invalide (422)** | Réponse engine sans clé `status` (corps d'erreur Pydantic) — improbable car le payload est construit par `ScheduleConstraintBuilder` | `FAILED` | `engine_failed` | Comparer le `snapshotData` au schéma engine (contrat 2.21, `engine/CONTRACT_VERSION`) |

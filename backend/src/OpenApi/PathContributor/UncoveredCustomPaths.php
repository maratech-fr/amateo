        ...
        return [
            ...
            '/api/reservations/group' => new PathItem(post: new Operation(
                ...
                responses: [
                    ...
                    '400' => new Response('Invalid JSON, or a missing/malformed field (no sharedTrainingBlockId given)'),
                    '401' => $unauthorized,
                    '403' => new Response('Not a management member'),
                    '404' => new Response('Unknown mutualisation block (or another club\'s — never an existence oracle)'),
                    '409' => new Response('Archived season (read-only)'),
                    '422' => new Response('Scope mismatch (block not declared for this plan), closed venue, occupied slot (exclusivity), shared-sessions ceiling reached, a member over its weekly sessions, or unknown schedule plan'),
                ],
                summary: 'Reserve a mutualisation block on one slot (atomic batch write of one HARD reservation per member)',
                requestBody: $this->schemas->jsonBody([
                    'type' => 'object',
                    'required' => ['sharedTrainingBlockId', 'venueId', 'dayOfWeek', 'startTime'],
                    'properties' => [
                        'sharedTrainingBlockId' => ['type' => 'string', 'format' => 'uuid', 'description' => 'The mutualisation block (a set of teams that behaves as one team)'],
                        'venueId' => ['type' => 'string', 'format' => 'uuid'],
                        'dayOfWeek' => ['type' => 'integer', 'minimum' => 1, 'maximum' => 7],
                        'startTime' => ['type' => 'string', 'description' => 'HH:MM'],
                        'durationMinutes' => ['type' => 'integer', 'default' => 90],
                        'schedulePlanId' => ['type' => 'string', 'format' => 'uuid', 'nullable' => true, 'description' => 'NULL = base (season) plan; set = a period overlay — must match the block\'s own scope'],
                    ],
                ]),
            )),

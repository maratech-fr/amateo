...
| **L9** | **La VUE DE SAISIE FBI** — la matérialisation de « sans qu'il oublie qqch » (mission §1). Précisée fondateur 2026-08-22. **Forme MINIMALE livrée en RMM-1 PR3** (`FbiEntryList.tsx` : les domiciles PLACÉS de la semaine + geste « Marquer saisi » de L4, non filtrable, non triée par équipe) ; la cible ci-contre reste ouverte pour PR4. | ~~La liste « À placer » s'arrête à PLACED ; rien ne présente ce qui RESTE À SAISIR dans FBI~~ (partiellement comblé, voir colonne 2) | La **liste à recopier**, organisée **par équipe / par semaine** (préconisation fondateur : coller à l'écran FBI pour que la saisie soit bête), filtrable équipe · date · semaine. Champs : **date + heure** (ce que FBI demande pour un domicile) + n° de rencontre et salle en repères. Cocher = le geste L4 ; **« tout marquer saisi » est acceptable mais ne coche QUE les lignes affichées à l'écran** (le filtre borne le lot — plus sûr) ; la checklist se vide ; l'échéance (RMM-6) affichée à côté. Bonus à l'étude : afficher si la ligue a posé officiel/arbitre (usage : solliciter des gens du club pour d'autres rencontres) — ⚠ suspendu à la vérification des colonnes du xlsx (§10). Zéro backend : une PROJECTION des fixtures existants. |
...
| **RMM-2** | Extraire le **rail d'étapes du wizard** dans `shared/` (`step-rail.tsx`), à rendu identique. Prérequis technique de RMM-1. **Resserrement fondateur 2026-08-23** : « exploiter `tabs` » et « mutualiser la grille temporelle » sont REPORTÉS à RMM-1 (on ne mutualise pas spéculativement une brique sans son second consommateur) — seule l'extraction du rail est livrée ici. | Front | — | P2-26 |

...
| **D-19** ✅ | Statuts/sources `admin_job_run` | CHECK SQL `Version20260716120000.php:19` vs 4 services **+ `PathContributor/AdminJobPaths.php:38-39`** (P4-138 a éclaté l'ex-monolithe `CustomRoutesOpenApiFactory` en contributeurs par domaine ; la copie OpenAPI vit désormais ici et dérive déjà de l'enum : `AdminJobStatus::values()`/`AdminJobSource::values()`, plus de liste manuscrite) | Le CHECK rattrape le code (bruyant), mais la copie **OpenAPI** dérive seule : la doc publique ment sur les valeurs | Créer `AdminJobStatus`/`AdminJobSource` — **fait** |
...
| **D-31** ✅ (omission résorbée, P4-147, 2026-08-29) | `IN_FLIGHT = [PENDING, GENERATING]` | `planning/queries.ts:13` vs `PlanningPage.tsx:36`, `lib/versions.ts:27`, `GenerateStep.tsx:22`, négation inline `scheduleStream.ts:52` | Un statut non terminal ajouté → certains écrans arrêtent de poller ou réactivent les boutons **en pleine génération**, d'autres non. ⚠ **La clôture ✅ ci-dessus était fausse dès son écriture le 2026-08-08** (sans qu'aucun test ne le voie) : `planning/lib/pickLandingSchedule.ts:3` portait déjà sa propre copie littérale (`const IN_FLIGHT = [...]`) depuis sa création le **2026-07-16** (`git log --follow`), donc **avant** cette consolidation — pas une rechute, une **sixième** copie que le balayage de l'inventaire (« 3 agents de balayage » — voir en-tête du fichier) n'a jamais trouvée. Résorbée par import de `IN_FLIGHT_STATUSES` (P4-147, trouvé en rotation de fraîcheur, pas par le balayage d'origine) | — |
...
| **D-34** ✅ (résiduel clos P4-148, 2026-08-29) | Unions de valeurs recopiées entre features | ~~`TeamLevel` : union stricte dans `wizard/api.ts:20`, **`level: string`** dans `matches/api.ts:191`~~ **Résorbé** : `Gender`/`TeamLevel` vivent désormais dans `shared/lib/teamIdentity.ts` (`wizard/api.ts` ré-exporte), `matches/api.ts` type `level: TeamLevel \| null`/`gender: Gender \| null` — trouvé et corrigé via `TsFieldsMatchOpenApiSchemaTest` (FRT-28/P4-148), pas par ce lot. `ScheduleStatus` et `SchedulePlanType` déclarés deux fois · `matches.Team` **n'a pas `isActive`** (restent ouverts) | Un niveau ajouté côté serveur passe **muet** côté matchs ; l'écran Matchs ne peut pas filtrer les équipes inactives et rien ne le lui rappelle | Mettre les **unions** dans `shared/api/types.ts`. ⚠ Les projections **structurelles** par feature restent légitimes (voir §3) |

...
> RMM-1 » et § « Filtres par équipe / coach / gymnase ». **Reste P4-197** (ci-dessous), relevé en
> réparant l'e2e du 2026-09-11 (P4-183, blocs de mutualisation imbriqués, est un lot distinct —
...
| P4-207 | **Résolution des conflits — statut persisté par empreinte (PR B2, frontend — suite de l'onglet Conflits)** | 🟡 | S | **Backend livré (PR B1, 2026-09-15)** : statut par `fingerprint` (empreinte STABLE, `ConflictFingerprinter.php`) — 3 cas persistés (**Dérogation demandée** · **Réglé en interne** · **Sans solution pour l'instant**), **À traiter** = défaut = absence de ligne ; table TENANT `conflict_resolution` (RLS, patron `opponent_travel`) + `GET /api/fixtures/conflicts` champ additif `resolution` + `PUT`/`DELETE /api/fixtures/conflicts/{fingerprint}/resolution` (gestionnaire seul) ; orphelin (empreinte disparue) jamais nettoyé à la volée, purgé avec la saison. Détail [`module-matchs.md`](../courantes/module-matchs.md) § « Résolution des conflits ». **Reste ouvert (PR B2, frontend)** : la chip de statut dans le slot `trailing` de `ConflictLine` (déjà extrait en PR A, `ConflictLine.tsx`) ; les COMPTEURS qui décrémentent sur un conflit traité (entrées du pivot Conflits + badge du radar `ConflictRadar`) ; l'interrupteur de pose/retrait du statut ; le badge de nav « Conflits · N à traiter » — absent depuis PR A faute de statut |

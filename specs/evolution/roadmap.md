...
| P4-207 | **Résolution des conflits — statut persisté par empreinte (PR B, suite de l'onglet Conflits)** | 🟡 | M | L'onglet Conflits (PR A, `/matchs/conflits`) liste tout mais ne permet de RIEN acquitter : un conflit revu reste identique au prochain chargement. Poser un statut par `fingerprint` (empreinte STABLE déjà posée par RMM-3, `ConflictFingerprinter.php`) : **À traiter** (défaut, sans ligne visible) · **Dérogation demandée** · **Réglé en interne** · **Sans solution pour l'instant** — + une note libre. Le conflit reste listé PARTOUT avec sa chip de statut (jamais masqué), mais les COMPTEURS décrémentent (entrées du pivot Conflits + badge du radar `ConflictRadar`) — c'est le statut qui allège la lecture, pas la disparition. Un conflit dont l'empreinte disparaît (résolu par le solveur/un geste) devient orphelin : masqué. Un changement de NATURE du conflit (même paire, type différent) repart à « À traiter ». Geste réservé au gestionnaire (rôle). Backend + endpoint API + nouvelle table TENANT (RLS) ; côté front, la chip de statut se pose dans le slot `trailing` de `ConflictLine` (déjà extrait en PR A, `ConflictLine.tsx`) ; le badge de nav « Conflits · N à traiter » — absent depuis PR A faute de statut — naît avec ce lot |

...
| Lot « overlay = la saison au moindre effort » — PR moteur (balayage bloc-aware + diagnostic honnête), PR comblement (référence = version pointée du socle, palier jour+heure/gymnase libre, poids échelonné par priorité, CONTRACT_VERSION 2.20), PR frontend (écarts au socle en ROUGE dans la grille — fermeture seulement — + compteur de carence demandé/dispo) | ⬜ plan validé 2026-09-02 ; **PR moteur prête** (les DEUX sites du kill corrigés — candidats `model.py` + balayage `structural.py` —, diagnostic bloc-aware, smoke overlay affirme la co-présence) |

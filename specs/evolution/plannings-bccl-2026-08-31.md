...
| Lot « overlay = la saison au moindre effort » — PR-1 moteur (balayage bloc-aware + diagnostic honnête), PR-3 comblement (référence = version pointée du socle, palier jour+heure/gymnase libre, poids échelonné par priorité, CONTRACT_VERSION 2.20), PR-4 frontend (écarts au socle en ROUGE dans la grille — fermeture seulement — + compteur de carence demandé/dispo) | ⬜ **PR-1 mergée** (#824, les DEUX sites du kill corrigés — candidats `model.py` + balayage `structural.py` —, diagnostic bloc-aware, smoke overlay affirme la co-présence) ; **PR-3 livrée** (branche `feat/pr3-socle-reference-fill`, en attente merge — `socleReferenceAssignments`, bonus de placement par tier, NR `SocleReferencePayloadParityTest`) ; reste **PR-4 frontend** |

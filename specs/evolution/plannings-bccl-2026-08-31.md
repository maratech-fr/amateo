...
| Durée d'indispo modifiable (D3) — v1 cadrée, PR-1 backend | ✅ 2026-09-04 (branche `feat/d3-pr1-redate-period-backend`) — v1 : racine `closure` d'un bloc (sans semaines-enfants) re-datable dans les deux sens par PUT, plan/contraintes datées/titre resynchronisés, version survit marquée à régénérer ; racines vacances/mères découpées/semaines-enfants restent gelées. **Reste ouvert : PR-2 frontend** (le geste d'édition, avec passe design). Voir D3 ci-dessus, trace `etat-des-lieux.md` §3 |

...
| Overlay Mateo — PR-1 : SEED du nouvel incident 31/08→16/10 (D2) | ✅ 2026-09-02 (PR en cours) — l'ancien incident « travaux » PURGÉ, remplacé par « Matéo indisponible (incident) » ; plan de fermeture SUR LA RACINE pointant une version COMPLETED transcrite (76 cases, 90 séances, 8 gymnases, Matéo absent) ; 13 blocs réels (8 hérités D10bis + 5 ajoutés), grille occupant-unique par ensemble exact, 50 TPO (Training Individuel désactivée, Basket Santé ×2), décochage « SM2 · au moins 1 à Matéo », ZÉRO réservation/genèse (viennent de l'exercice). e2e club-life réécrit au modèle overlay-validé. |
| Overlay Mateo — EXERCICE solveur (le dernier du programme) | ✅ 2026-09-02 — joué sur le RAIL PRODUIT (transcription → comblement, sandbox) : transcription = **47/90 séances déjà à leur place cible** (52 %), 16 gestes manuels (13 décrochages knock-on + 3 épingles orphelines, 422 assumé — décision fondateur) ; comblement produit = FAILED systématique (balayage occupant-unique AVEUGLE AUX BLOCS, `engine/app/solver/constraints/structural.py:91-124` : un partenaire de bloc épinglé tue le partenaire libre) ; comblement corrigé simulé = COMPLETED mais 14/44 séances libres gardent le jour+heure de saison vs **27/44 dans le réel** → il manque la préférence « garde jour+heure, gymnase libre » échelonnée par priorité d'équipe. 2 erreurs de données débusquées (Veterans · évite JDR ; Annexe mardi 19:30/60) — corrigées ligne suivante. Diagnostic « N séances pour M places » trompeur (ignore les blocs). Lot correctif cadré et validé fondateur (PR moteur balayage, PR comblement référencé socle + contrat 2.20, PR écarts en rouge dans la grille) |
| Overlay Mateo — PR-2 : corrections seed débusquées par l'exercice | ✅ 2026-09-02 (PR en cours) — « Veterans · évite JDR » SUPPRIMÉE (erreur du planning de saison, décision fondateur : l'overlay place Veterans à JDR ven 20:30 ; purge des bases seedées via `$staleNames`) ; case Annexe mardi recalée 19:00→20:30/90 min (erreur de saisie du fichier source, 19:30/60 violait « U15 · pas après 19:00 » — la règle reste) |
| Lot « overlay = la saison au moindre effort » — PR-1 moteur (balayage bloc-aware + diagnostic honnête), PR-3 comblement (référence = version pointée du socle, palier jour+heure/gymnase libre, poids échelonné par priorité, CONTRACT_VERSION 2.20), PR-4 frontend (écarts au socle marqués dans la grille — symbole ⇄, token `--diff` VIOLET, fermeture seulement — + panneau « Comparer avec la saison » cliquable + compteur de carence demandé/dispo) | ⬜ **PR-1 mergée** (#824, les DEUX sites du kill corrigés — candidats `model.py` + balayage `structural.py` —, diagnostic bloc-aware, smoke overlay affirme la co-présence) ; **PR-3 MERGÉE** (#825 — `socleReferenceAssignments`, bonus de placement par tier, NR `SocleReferencePayloadParityTest`) ; **PR-4 livrée, PR en cours** (branche `feat/pr4-socle-deviation-grid`, D6) — `WeekGrid` marque chaque carte déviée d'un symbole ⇄ (`ArrowLeftRight`) AVANT le nom d'équipe, jamais un mot à l'écran, token `--diff` dédié (violet, pas une couleur des 4 rôles existants), aucune légende/bascule, chaque ligne « déplacée » du panneau `SocleDeviationPanel` est un bouton qui vise sa carte dans la grille (D6-d, `entry.to.slotId` nouveau sur `SocleDeviationCalculator`/`SocleDeviationResult`), compteur de carence = phrase factuelle neutre sur `ValidateResult.capacity {demand, offer}` — clé additive bloc-aware, fermeture seulement, jamais sur une vacance ; a11y non-texte (WCAG 1.4.11, ≥3:1) gardée par `tests/e2e/a11y-contrast.spec.ts` |

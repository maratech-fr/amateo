...
Last verified @ 2026-09-02 (**rotation de fraîcheur** `documentation-update`, zone non touchée par
cette PR — re-vérifié contre le code, tout juste (une coquille de ligne corrigée) :
`Club.logoUrl`/`accentColorDark`/`accentPalette` dans l'entité (`backend/src/Entity/Club.php:138-154`)
✓, `PATCH /api/club/appearance` (`ClubAppearanceController.php:33`, pas 34) ✓, `useApplyClubTheme.ts`
lit `accentColor`/`accentColorDark`/`accentPalette` sur `me.club` ✓, et `GenerationWaiting.tsx`
n'affiche toujours aucun logo/initiale de club — sa seule occurrence de « logo » est le commentaire
qui acte la décision fondateur du 2026-08-17 ✓. L'historique vit dans git :

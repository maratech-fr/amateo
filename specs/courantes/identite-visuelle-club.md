...
>
> **Reste ⬜** : stockage prod réel (impl S3/objet à écrire quand la cible est connue) · usage plus poussé des 3 couleurs (teintes signature au-delà de `--accent-2`) · login marque produit (inchangé) · assets **par sport** pour cette scène (la scène livrée reste générique basketball codée en dur — le fond d'écran commun livré le 2026-09-25, `identite-visuelle-produit.md` § « Le fond d'écran », est lui aussi générique multi-sport, il ne solde pas ce point) : dépriorisé derrière la ligne « Multi-sport » de `roadmap.md` (🔴, attendre une vraie demande).
>

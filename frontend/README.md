...
- **Une feature = un dossier** `src/features/<x>/` avec `{api,queries,store}.ts` + ses composants. Features livrées :
  - **`auth`** — login / register (club ARA, statut pending/active) / `/me`. L'identité est un cookie httpOnly posé par le serveur (SEC-16) — `authStore` (Zustand) ne garde qu'un booléen `isAuthenticated`, aucun jeton ; `AuthGuard` redirige (pas de session → `/login`, onboarding non terminé → `/wizard`).
  ...
  - **`cockpit`** — **accueil temporel** (`/`) : bandeau planning socle (ouvrir/modifier/tous les plannings), calendrier mensuel des exceptions, radar d'overlays période/événement. Débloqué dès `me.seasonPlan.hasFinishedVersion` (le plan de saison porte ≥1 version terminée) — sinon `AuthGuard` redirige vers `/wizard`.
  - **`matches`** — **module matchs**, un layout unique `MatchesLayout` (`src/app/routes.tsx`, onglets défilables) portant toutes les routes `/matchs/*`. L'index (`MatchesLanding`) atterrit sur le **Calendrier** (`CalendarPage` : boucle Semaine/Mois/Phase, panneau de placement **permanent**, radar de conflits, barre `WeekCounters`) ou directement sur **Conflits** s'il y en a à traiter. Les autres onglets : **Importer** (dépôt FBI/API FFBB + file de traitement des rencontres), **Configuration**/**Semaine type**/**Adversaires** (réglages rares : gabarit A/B, créneaux partagés, localisation des adversaires). Détail métier complet : [`specs/courantes/module-matchs.md`](../specs/courantes/module-matchs.md).
...
- **Partagé** : `src/shared/api/client.ts` (client **ky**, `credentials: "include"` — l'identité est un cookie httpOnly, **aucun** header `Authorization` ni tenant, 401 → logout), `shared/api/collection.ts` (unwrap JSON-LD `{member:[…]}` + pagination), `shared/components/ui/` (primitives).
...
- **Onboarding guidé vs libre** : `AuthGuard` verrouille l'app au wizard tant que `me.seasonPlan.hasFinishedVersion === false` (le club n'a jamais généré) — le flag legacy `club.onboardingCompleted` n'est plus lu pour le routage.
...
| Doc | Contenu |
|-----|---------|
| [`AGENTS.md`](AGENTS.md) | Cheat-sheet agent : frontières, deux faits d'arborescence absents de `frontend-spec.md` (FRT-29, FRT-33), routing (3 filets lazy), data/state, gotchas — les primitives partagées pointent vers `frontend-components.md` §3, jamais recopiées ici. |

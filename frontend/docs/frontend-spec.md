...
Last verified @ 2026-08-25 (recalé par RMM-6 PR-2, l'éditeur des échéances de saisie et leur
affichage par ligne — RMM-6 reste OUVERT, PR-3 cockpit/login à venir. La ligne `/matchs` confrontée
au code : `EntryDeadlinesEditor.tsx` (nouvelle carte du SET-UP, multi-sélection + une date →
`POST /api/competitions/entry-deadlines` en lot, trois provenances CLUB/proposée/aucune distinguées
icône+texte, badge « partagée avec les autres clubs » sur les compétitions appariées, effacement
explicite), `FbiEntryList.tsx` (échéance EFFECTIVE servie par ligne — `lib/deadlineLabel.ts`, pur —
« avant le … (J-N) », dépassée en avertissement JAMAIS bloquant, absente sur un amical),
`ConfigurationPage.tsx` (+1 carte). Le reste du fichier (routes hors matchs, primitives, stack) non

...
Last verified @ 2026-09-15 (`documentation-update`, PR B2 « résolution des conflits, frontend »).
Recalé : la ligne `/matchs` — badge de nav Conflits (`Conflits · N`, N = à traiter, absent à 0,
depuis P4-207) remplace « Conflits sans badge » ; arbre des fichiers (§ arborescence) gagne
`ConflictResolutionControl`/`lib/conflictResolution.ts` et `shared/components/ui/filter-toggle`
dans la liste des primitives. Reste du fichier non re-sondé cette passe.
## 1. Stack Decided

...
Last verified @ 2026-09-15 (`documentation-update`, PR A « onglet Conflits »). Recalé : la ligne
`/matchs` — nav à 6 routes (Semaine · Consulter · Importer · Configuration · Conflits +
`/matchs/reconciliation`), nouvel onglet « Conflits » (`ConflictsPage.tsx`) décrit et pointé vers
`module-matchs.md` § « Onglet Conflits » ; correction « 9 `ConflictType` » → 10 (comptage confronté
à `lib/conflictLabels.ts` `CONFLICT_FAMILIES`) ; ligne du store matches (§9) gagne
`conflictsPivot`/`conflictsFamilies` ; arbre des fichiers (§ arborescence) gagne `ConflictsPage`.
Reste du fichier non re-sondé cette passe.
## 1. Stack Decided
...
│   │                           # ConflictsPage ET ConflictRadar)

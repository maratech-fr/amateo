...
Last verified @ 2026-08-24 (recalé par RMM-4 PR-2, réconciliation FBI — la ligne `/matchs` et le
tableau des endpoints confrontés au code : `router.tsx` (route `matchs/reconciliation` enfant de
`MatchesLayout`), `ReconciliationView.tsx`/`ReconciliationPanel.tsx`/`store.ts` (payload en mémoire,
renvoi propre sans payload), `ImportFbiDialog.tsx` (bascule « Examiner » sur `deviations` non vide,
flux inchangé sinon), `api.ts` (`fixtures/import/analyze`, `fixtures/import`,
`fbi-ingestions/latest`), `ConfigurationPage.tsx`/`MatchesPage.tsx` (carte + rappel de fraîcheur). Le
lot **reste OUVERT** (PR-3 à venir — `ReconciliationPanel.tsx:27`). Re-confirmé dans la même passe :
`MatchesLayout.tsx`/`queries.ts` (`useModuleVisit`, un POST par ouverture), `ModuleVisitBanner.tsx`
(muet en première visite). Le reste du fichier (routes hors matchs, primitives, stack) non
re-vérifié cette passe)
...
│   │                           # ReconciliationView/-Panel (RMM-4, écarts FBI par écart, lot ouvert)

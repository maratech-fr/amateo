...
Last verified @ 2026-09-09 (`documentation-update`, P4-187b geste Rattacher dans Importer). Recalé :
ligne de route `/matchs` — en-tête d'équipe de la file `ReviewQueue.tsx` gagne « N sans gymnase »
(`lib/reviewQueue.ts` `isUnattachedHome`/`unattachedCount`), `ReviewQueueRow.tsx` affiche pour un
domicile sans gymnase un sous-bloc « `<libellé>` · non rattaché » + **Rattacher** →
`VenueSelect` pré-sélectionné sur `Fixture.suggestedVenueId` + Confirmer (`POST
/api/venues/{id}/external-labels`, `useAttachVenueLabel`, invalide `fixtures`+`venues`) / Annuler ;
Consulter (`MatchRowsTable.tsx`) recalé sur le libellé réel « à rattacher dans Importer »
(était « non rattaché » — drift trouvé et corrigé cette passe) ; pas d'écran de retrait d'alias
(`DELETE /api/venues/{id}/external-labels/{label}` sans consommateur `frontend/src`, roadmap
P4-196) — confronté au code (`ReviewQueueRow.tsx`, `ReviewQueue.tsx`, `ImportPage.tsx`,
`lib/reviewQueue.ts`, `api.ts`, `queries.ts`, `MatchRowsTable.tsx`). Reste du fichier non re-sondé
cette passe.
## 1. Stack Decided
...
│   │                           # six AccordionSection contrôlées ancrées ?section=, P4-185 —

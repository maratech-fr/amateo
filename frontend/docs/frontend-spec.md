...
Last verified @ 2026-08-24 (recalé par RMM-3, le « gardien » à l'ouverture — dernière PR du lot
module matchs : la ligne `/matchs` confrontée au code — `MatchesLayout.tsx`/`queries.ts`
(`useModuleVisit`, un POST par ouverture, `enabled` sous la garde socle), `ModuleVisitBanner.tsx`
(segments non nuls, muet en première visite), `ConflictRadar.tsx` (chip « Nouveau » ornementale).
Re-confirmé dans la même passe : `MatchesPage.tsx` (`panelSlot` permanent avec état vide,
`swapCandidateIds` sur `WeekendGrid.tsx`, listener Échap), `FbiEntryList.tsx` (groupage équipe,
filtres, lot borné à l'affiché), `store.ts` (`unplacedReasons` purgé au changement de semaine). Le
reste du fichier (routes, primitives, stack) non re-vérifié cette passe)
...
│   ├── matches/                # MatchesLayout (deux routes) → MatchesPage (boucle : rail 5 étapes

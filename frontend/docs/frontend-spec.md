...
Last verified @ 2026-09-08 (`documentation-update`, PR-2a onglet Consulter du module matchs). Recalé : ligne de
route `/matchs` (onglet Consulter, `ConsultPage.tsx`, `lib/consultFilter.ts`, `lib/conflictLabels.ts`, extension
`lib/urlState.ts`) et ligne du store `matches` (`consultKinds`/`consultFamilies`/`consultTypicalWeek`),
reconfrontées au code ; la ligne PR-1 (même jour) reste exacte. Reste du fichier non re-sondé cette passe.
---

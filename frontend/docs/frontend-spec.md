...
Last verified @ 2026-08-25 (recalé par RMM-5 PR-4, le SET-UP A/B et le signal — **RMM-5 est LIVRÉ
EN ENTIER (4 PR), clôt P2-49**. La ligne `/matchs` confrontée au code : `MatchSlotRotationsEditor.tsx`
(créneau + membres ORDONNÉS, flèches ↑/↓, badge A/B/C, phrase « l'ordre ne commande aucun
calendrier »), `TypicalWeekendGrid.tsx`/`lib/typicalWeekend.ts` (`weekCountOf`, segmenté `Tabs`
invisible sans rotation ≥ 2 membres, grille identique à avant par défaut `rotations=[]`, rotation
hors week-end listée à part), `lib/loopSteps.ts` (`isOffModel` étendu aux membres de rotation —
la rotation du jour prime sur l'habitude ; `sameWeekendRotationCount`, nouveau signal), `api.ts`/
`queries.ts` (premier client du CRUD `/api/match_slot_rotations` livré PR-1),
`ConfigurationPage.tsx`/`MatchesPage.tsx` (câblage des deux nouveaux hooks + badge). Zéro fichier
backend/engine dans le diff. Le reste du fichier (routes hors matchs, primitives, stack) non

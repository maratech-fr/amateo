...
Last verified @ 2026-09-08 (`documentation-update`, PR-3b espace Importer du module matchs). Recalé : ligne de
route `/matchs` (nav Semaine · Consulter · Importer · Configuration, badge de compte, `ImportPage.tsx`,
`ReviewQueue.tsx`/`ReviewQueueRow.tsx`, `lib/reviewQueue.ts`, `ReconciliationView.tsx` réduite au canal API
créable, `ReconciliationPanel` disparu, `ConfigurationPage.tsx` allégée par P4-186), le tableau des endpoints
`/matchs` et l'arborescence `features/matches/`, confrontés au code (`MatchesLayout.tsx`, `routes.tsx`,
`api.ts`, `store.ts`). Reste du fichier non re-sondé cette passe.
## 1. Stack Decided

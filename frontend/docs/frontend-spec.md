...
Last verified @ 2026-09-12 (`documentation-update`, mesure terrain fondateur — dépôt FBI en
onglets par famille). Recalé : `ImportFbiDialog.tsx` range les divisions du dépôt FBI en onglets
par famille (`lib/divisionFamily.ts`, `Tabs`/`TabPanel`), confronté au code — voir l'entrée
« Espace Importer » ci-dessous et le détail complet dans `specs/courantes/module-matchs.md` §
« Onglets par famille — dépôt FBI ». Reste du fichier non re-sondé cette passe.
## 1. Stack Decided

...
Last verified @ 2026-09-16 (`documentation-update`, PR 2b « auto-localisation des adversaires +
endpoint orchestrateur » — clôt le lot 2). Recalé, contre `api.ts`/`queries.ts`/
`OpponentTravelCard.tsx` : le bouton unique « Mettre à jour les adversaires » appelle désormais
`POST /api/opponents/refresh` seul (trois passes serveur), plus les deux appels front enchaînés de
PR 2a. Reste du fichier (dont la ligne `/matchs` recalée PR 2a le même jour — ordre de nav,
septième route `/matchs/semaine-type`, Configuration à cinq `AccordionSection`, « Accès match »)
non re-sondé cette passe.
## 1. Stack Decided
...
| matches `store` | `src/features/matches/store.ts` | état UI du module matchs — `railStep` (vue de la boucle choisie ; `null` = auto = premier trou non fait, `defaultLoopStep`/`lib/loopSteps.ts` — SAUF si ce trou est `fbiEntry` **vide**, auquel cas `homeSlots` est rendu à la place, P4-192), `unplacedReasons` (raisons du dernier auto-placement, par fixtureId) : les deux resettent au changement de semaine (`setSelectedWeekend`, RMM-1) ; `filterMode` (`equipe`/`coach`/`gymnase`) + `filterIds` (PR-1 filtres 2026-09-08) : changer d'axe vide la sélection, tout changement de filtre remet `railStep` à null, changer de semaine ne purge PAS le filtre ; miroir dans l'URL `?vue=&filtre=` ; `consultKinds`/`consultFamilies` (`null` = tout) + `consultTypicalWeek` (PR-2a Consulter, 2026-09-08) + `consultTemporality`/`consultMonth`/`consultPhaseId` (PR-2b), miroir URL `type=`/`conflits=`/`type_semaine=`/`temps=`/`mois=`/`phase=` ; `conflictsPivot`/`conflictsFamilies` (PR A Conflits, 2026-09-15) — SÉPARÉS de `consultFamilies` (le compte de l'onglet Conflits est SAISON, celui de Consulter la semaine affichée), miroir URL `pivot=`/`conflits=` (`decodeConflictsParams`/`applyConflictsToParams`) | Non persisté |
...
│   ├── matches/                # MatchesLayout (six routes) → MatchesPage (boucle : rail 5 étapes
│   │                           # dérivées lib/loopSteps.ts, grille week-end, radar conflits,
│   │                           # FbiEntryList) + ConsultPage (lecture seule) + ImportPage (données
...
│   │                           # sept AccordionSection contrôlées ancrées ?section=, P4-185 —
│   │                           # lib/configSummaries.ts pour les résumés d'en-tête ;
│   │                           # VenueLabelsSection, 7ᵉ section, écran d'appariement des libellés
│   │                           # FFBB, E2/P4-205 ; UnpairedVenueLabelsBanner, signal partagé) +
│   │                           # ReconciliationView (canal API seul, rencontres créables — le
│   │                           # ReconciliationPanel des écarts par écart a disparu, PR-3b) +

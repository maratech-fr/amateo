...
Last verified @ 2026-08-25 (recalé par RMM-6 PR-3, **DERNIÈRE du lot — RMM-6 est LIVRÉ EN ENTIER**.
La ligne `/` confrontée au code : `CockpitPage.tsx` gagne `FbiDeadlineCard` — première incursion du
module matchs au cockpit, pleine largeur sous le bandeau socle, mute hors fenêtre J-7 ouverte
(`useDeadlineOutlook`, `GET /api/matches/deadline-outlook`), escalade « dès le login », résumé du
gardien fusionné dans la carte. La ligne `/matchs` reste celle recalée à la passe précédente
(`EntryDeadlinesEditor.tsx`, `FbiEntryList.tsx`, `ConfigurationPage.tsx`) — non re-vérifiée cette
passe. Le reste du fichier (routes hors matchs/cockpit, primitives, stack) non re-vérifié)

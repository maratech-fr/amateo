...
Last verified @ 2026-09-12 (`documentation-update`, P4-199 règles d'import). Recalé : `ReviewQueueRow.tsx`
porte désormais un bouton « Pris en compte » (label `validateLabel`, même endpoint que « Valider »)
sur une `REVIEWED` à alerte `autoApplied`, et le bandeau nomme la source + ancien→nouveau (bug
d'affichage corrigé — l'ancienne version montrait `appValue` comme destination) ; le caveat
« Masquer les extérieurs » (2026-09-12, entrée « Espace Importer » ci-dessous) est soldé. Reste du
fichier non re-sondé cette passe.
## 1. Stack Decided

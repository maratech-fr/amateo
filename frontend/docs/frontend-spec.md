...
Last verified @ 2026-09-19 (`documentation-update`, PR H « onglet Adversaires »). Re-confronté au
code cette passe : nav `MatchesLayout` à **6** onglets **Conflits · Calendrier · Importer ·
Configuration · Adversaires · Semaine type** dans cet ordre (l'onglet Adversaires inséré entre
Configuration et Semaine type) ; les ancres de `/matchs/configuration` tombent à **quatre**
`?section=echeances|durees|reglages|libelles` — l'ancienne `adversaires` a migré vers son propre
écran (`/matchs/adversaires`, ancien deep-link `?section=adversaires` désormais un `<Navigate>`
complet, pas une simple absence de section ouverte) ; `ConfigurationPage.tsx` gate désormais
**quatre** `AccordionSection` (pas cinq). Reste confronté aux passes précédentes du même jour (PR G
« todo FBI unique », PR F « ClubSiegeSubsection ») et à la passe d'avant (2026-09-18, UXC-23/
UXS-08) : citations recalées à l'époque, non re-sondées cette passe.
## 1. Stack Decided

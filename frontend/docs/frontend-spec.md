...
Last verified @ 2026-09-19 (`documentation-update`, PR G « todo FBI unique »). Re-confronté au
code cette passe : la description du Calendrier (`/matchs`) était **PÉRIMÉE** sur trois points —
`FbiDeadlineCard` n'est plus rendue SEULEMENT en fenêtre J-7 (gagne un régime hors-fenêtre sur
`fbiTodo`) ; `WeekCounters` ne porte plus trois compteurs symétriques (le FBI est devenu GLOBAL,
hors du groupe « Semaine affichée ») ; la modale FBI a changé de nom et de forme (« FBI — à
faire », deux sections, filtre équipe seul, deep-link `?fbi=1`) — les trois recalées contre
`FbiDeadlineCard.tsx`/`WeekCounters.tsx`/`FbiEntryList.tsx`/`CalendarPage.tsx`/`lib/urlState.ts`.
Reste confronté à la passe précédente (2026-09-19, PR F) : §6.6 ter était **PÉRIMÉE** (décrivait
encore le formulaire `PATCH /api/club/info` — comité/correspondant/président/salle principale —
supprimé le 2026-08-04, `ClubPage.tsx` est lecture seule FFBB depuis cette date) — réécrite pour
l'état réel (`ClubInfoSection`/`ClubSiegeSubsection`, `ClubPage.tsx`) et pour la nouvelle section
« Siège du club » (`AddressGeocodeField`, `PATCH /api/club/siege`). Reste confronté à la passe
d'avant (2026-09-18, lot correctif frontend de l'audit 2026-09-18, UXC-23/UXS-08 — la ligne
`/matchs` de ce fichier cite ces deux items) :
...
- Un planning `COMPLETED` peut être **validé** (bouton « Valider » de la toolbar, visible
  ...
  détail métier : `specs/courantes/module-matchs.md` §10 « Le périmètre engagé ».
...
│   ├── matches/                # MatchesLayout (sept routes, 5 onglets) → CalendarPage (route
...
│   │                           # cinq AccordionSection contrôlées ancrées ?section=, P4-185 —
│   │                           # lib/configSummaries.ts pour les résumés d'en-tête ;
│   │                           # VenueLabelsSection, 5ᵉ section, écran d'appariement des libellés

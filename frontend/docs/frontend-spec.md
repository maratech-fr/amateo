...
Last verified @ 2026-09-08 (`documentation-update`, PR-2b Mois · Phase du module matchs). Recalé : ligne de
route `/matchs` (temporalités, `MatchRowsTable.tsx`, `lib/monthView.ts`, `lib/phaseView.ts`, primitive
`shared/components/ui/table.tsx`) et ligne du store `matches` (`consultTemporality`/`consultMonth`/`consultPhaseId`),
reconfrontées au code ; lignes PR-1 et PR-2a du même jour re-confrontées. Reste du fichier non re-sondé cette passe.
## 1. Stack Decided

...
| Drag & drop | @dnd-kit | core ^6 · sortable ^10 · utilities ^3 | Tri des équipes (inter-tier), accessible DnD |
...
| `/` | **Cockpit temporel** (`CockpitPage`) : bandeau planning principal (Ouvrir/**Modifier** = reopen · Tous les plannings) · calendrier mensuel des exceptions · radar (à traiter). **Débloqué dès `me.seasonPlan.hasFinishedVersion`** (le plan de saison porte ≥1 version terminée — dérivé, indépendant du pointeur : rouvrir ne re-verrouille pas) ; sinon redirige vers `/wizard`. **Palier B** : CTAs radar « Adapter » actifs (→ wizard mode période) ; « Voir le plan » (overlay généré → consultation) ; « Modifier » le socle avec overlays → **confirmation proportionnée** (409 `overlays_exist` → dialog « supprimera N secondaires »). Overlays exclus du sélecteur de plannings (badge « Période »). **Première incursion du module matchs** (RMM-6 PR-3, 2026-08-25 — étendue au registre « à corriger dans FBI » le 2026-09-19) : `FbiDeadlineCard`, pleine largeur sous le bandeau socle, servie par le même endpoint (`GET /api/matches/deadline-outlook`) : en fenêtre J-7 → ton accent/warning avec les échéances ; **hors fenêtre mais `fbiTodo` (à saisir + à corriger) > 0 → rendue quand même, ton NEUTRE, la seule ligne globale** (« N FBI à faire, dont M à corriger ») ; rien à faire ET aucune fenêtre → muette (le cockpit ne reste plus muet dès qu'il y a du FBI en attente). Escalade « dès le login » (décision fondateur : le placement de match est une urgence), résumé du gardien fusionné dans la même carte, lien unique vers `/matchs?fbi=1`, détail : `specs/courantes/module-matchs.md` §4 « Le gardien à l'ouverture — delta de visite » | Required | `AppLayout` |
...
| `/matchs` | **Module matchs — l'écran unique** (`MatchesLayout` → `Outlet`, 6 onglets, RMM-1 **livré en entier**, fusionné en un seul écran par PR 3b) : **le Calendrier** (`/matchs`, servi par la route d'atterrissage conditionnelle `MatchesLanding.tsx` en INDEX (UXS-07, 2026-09-24 — l'index n'est plus INCONDITIONNELLEMENT le Calendrier : renvoie sur `/matchs/conflits` s'il y a des conflits à traiter, sinon rend `CalendarPage.tsx` ; décision figée une fois par SESSION `useMatchesStore.landingDecided` non persisté, attente du compte sous `FullPageSpinner` — jamais le Calendrier avant de savoir —, `useConflicts`/`openConflictCount` = même cache que le badge, échec ⇒ Calendrier « fail-open », URL avec paramètres ⇒ décision sautée ; la nav des six onglets reste inchangée), `CalendarPage.tsx`, PR 3b 2026-09-16 — remplace l'ancien onglet Semaine `MatchesPage.tsx` + l'ancien onglet Consulter `ConsultPage.tsx`, TOUS DEUX SUPPRIMÉS ; `/matchs/consulter` redirige en permanence vers `/matchs`, query conservée) fusionne la boucle de placement et la lecture Semaine · Mois · Phase sur le MÊME écran ; barre de filtres PR-1 partagée entre les trois temporalités (même `filterMode/filterIds`, même URL `?vue=&filtre=`) ; chaîne pure `applyMatchFilter → applyKindFilter → scopeConflictsToWeek → countByFamily → applyFamilyFilter` (`lib/consultFilter.ts`) ; chips **type de compétition** (amical = `competitionId` null ; championnat/coupe/brassage = `Competition.competitionType`, multi, **défauts revus le 2026-09-16/17** : championnat+coupe+brassage cochés, amical décoché — `DEFAULT_KINDS`, `lib/consultFilter.ts`), interrupteur **« Semaine type »** (Semaine seule, grille sans habitudes = `buildWeekendGrid(…, [])`, **MASQUÉE par défaut** depuis le 2026-09-16/17), interrupteur **« Extérieurs »** (les trois temporalités, **MASQUÉS par défaut**, jamais les compteurs/conflits/radar — détail `specs/courantes/module-matchs.md` §5 « Écran Calendrier »), chips **familles de conflits** avec compteur sur la temporalité affichée (conflits sans date toujours comptés, affichés en bandeau global vers l'onglet Conflits), libellés de famille dans `lib/conflictLabels.ts` (table des 9 `ConflictType` — `TEAM_LINK_OVERLAP` en a été retiré, lot N, 2026-09-21) ; **temporalité Semaine = l'établi** (`WeekWorkbench.tsx`, extrait trait pour trait de l'ex-`MatchesPage.tsx`) — grille `WeekendGrid`/`AwayList` avec panneau de placement PERMANENT (`PlacementPanel`, état vide « Sélectionnez un match »), échange visible sur la grille, radar `ConflictRadar` rendu EN DERNIER (sous la grille) ; barre `WeekCounters` REMPLACE l'ancien rail à 5 étapes : « N à placer » et « N conflits → » (lien `/matchs/conflits`) BORNÉS à la semaine affichée dans un `role="group"` « Semaine affichée » (`deriveWeekCounters`, `lib/loopSteps.ts`), et **« N FBI à faire »** GLOBAL (toutes semaines, `fbiTodo` servi par le backend — domiciles PLACED + entrées ouvertes du registre) HORS de ce groupe, ouvrant l'écran **« FBI — à faire »** ; URL fusionnée `vue=`/`filtre=`/`type=`/`conflits=`/`type_semaine=`/`exterieurs=`/`temps=`/`mois=`/`phase=`/`semaine=YYYY-MM-DD`/**`fbi=1`** (deep-link de la liste FBI depuis le cockpit, `decodeFbiParam`/`applyFbiToParams`, `lib/urlState.ts`) ; **temporalités Mois · Phase (PR-2b, 2026-09-08)** — Mois = table partagée (`MonthTable.tsx`, `shared/components/ui/table.tsx`, primitive née ici) groupée par jour, navigateur ‹ mois ›, `lib/monthView.ts` (`listMonths`, `resolveActiveMonth`, `groupByDay`, `scopeConflictsToMonth`) ; Phase = `<select>` natif des compétitions appariées (`PhaseTable.tsx`), en-tête « importées / attendues » (`COMPETITION_INCOMPLETE` sinon `count/expectedMatchdays` — **`expected` peut valoir `null`** depuis P4-195 sur une `CUP`, l'en-tête bascule alors sur « N journée(s) importée(s) » sans dénominateur), journées par week-end, `lib/phaseView.ts` ; ligne de match `MatchRowsTable.tsx` (date/heure ou « heure non publiée », équipe + rôle, dom./ext., adversaire, gymnase résolu sinon `fbiVenueLabel` « à rattacher dans Importer », statut, pastilles de familles), clic → bascule en Semaine sur le week-end du match, MÊME écran ; barre d'actions : « Placer automatiquement » seul bouton primaire, « Nouveau match » secondaire. Nav `MatchesLayout` (6 onglets, défilable horizontalement à l'étroit, `scrollIntoView` ramène l'onglet actif en vue) : **Conflits · Calendrier · Importer · Configuration · Adversaires · Semaine type** (badge « Importer · N » quand N > 0, `lib/reviewQueue.ts` `pendingReviewCount`, dérivé du même cache que la boucle ; badge « Conflits · N » — N = conflits À TRAITER, `openConflictCount`, absent jamais « · 0 » — depuis P4-207 PR B2) — **onglet « Conflits » (`/matchs/conflits`, `ConflictsPage.tsx`, PR A 2026-09-15, statut de traitement P4-207 PR B2)** : lecture seule (sauf le traitement d'un conflit lui-même, réservé au gestionnaire), TOUS les conflits de la saison (même flux `useConflicts` que le Calendrier, sans le filtre PR-1), pivotés coach/équipe/gymnase/journée (`lib/conflictPivot.ts` `pivotConflicts`), une entrée par accordéon (`ConflictSeverityGroups`/`ConflictLine`, maison partagée avec `ConflictRadar`), chaque conflit composé par `ConflictResolutionControl.tsx` (pastille `StatusPill`/menu APG de traitement, note inline, `lib/conflictResolution.ts` maison unique des compteurs « à traiter »), **puces « Traitement »** (remplace l'ex-interrupteur « Masquer les traités » depuis le 2026-09-16/17, legacy `?traites=masques` toujours lu) et `FilterToggle` « Seulement avec un match à domicile » (`?domicile=1`), bouton « Voir la semaine » (conflit daté) → pose la semaine et navigue vers `/matchs` (le Calendrier) ; détail complet : `specs/courantes/module-matchs.md` §6 « Écran Conflits » ; **barre de filtres au-dessus du navigateur de semaine (PR-1 filtres, 2026-09-08)** — segmenté « Par équipe · Par coach · Par gymnase » (`MatchesFilterBar.tsx`, patron `VIEWS` de `/planning`) + la puce `ResourceFilter` de `features/planning` importée telle quelle (3ᵉ consommateur cross-feature) ; vue coach = équipes coachées (principal/assistant) + équipes jouées (badge « joueur » sur les listes, `lib/matchFilter.ts` `expandCoachTeams`) ; filtre appliqué EN AMONT (`applyMatchFilter`, pass-through byte-identique sans filtre) à la grille, aux listes et au radar ; un match est dans un filtre gymnase ssi posé là (les extérieurs en sortent) ; un conflit est retenu dès qu'un acteur (fixture/équipe/coach/gymnase) est dans le filtre ; navigateur ‹ › sur les semaines de la personne filtrée, atterrissage `resolveActiveWeekend` (première semaine ≥ courante, sinon dernière) ; état porté par l'URL `?vue=&filtre=` (lecture au montage, écriture `replace`, `lib/urlState.ts`) ; **la liste « À placer » couvre depuis P4-197 TOUTES les semaines filtrées** (pas seulement l'affichée — avant, un week-end 100 % déplacements la laissait vide et sans repli) ; le mode échange se voit SUR la grille (`WeekendGrid` candidates en anneau, Échap sort), **les extérieurs vivent DANS la grille depuis lot 3 PR-3a (2026-09-16)** — colonne « Extérieur » en fin de groupe de date (icône `Bus`, 8rem), bande « sans heure » en tête pour les AWAY sans heure ni habitude, clic ouvre `FixtureFormDialog` (jamais le panneau de placement), inerte en mode échange ; détail `specs/courantes/module-matchs.md` §5 « Écran Calendrier », `UnplacedList` — la colonne « À placer », qui porte depuis RMM-10 (P2-52) un badge calme « Le gymnase n'est plus affilié au club » (`unplacedReasonLabel.ts`) quand `Fixture.unplacedReason=venue_lost`, ton muted distinct du warning d'auto-placement, radar de conflits, `FbiEntryList` (ouverte via la modale **« FBI — à faire »**, plus une étape de rail) — l'écran GLOBAL (toutes semaines, plus un filtre Date) en deux sections empilées : **« À corriger dans FBI »** en premier (une ligne par match, gras = valeur Amateo à taper, FBI en sourdine jamais barré, « vu dans FBI le … », bouton « Corrigé dans FBI »/« Annuler ») puis **« À saisir »** (domiciles PLACED, mention « FBI affiche … » depuis `Fixture.fbiEcho`, « tout marquer saisi » borné aux lignes affichées, **chaque ligne portant son échéance de saisie EFFECTIVE servie par le backend** — RMM-6 PR-2, « avant le … (J-N) », dépassée en avertissement JAMAIS bloquant, absente sur un amical) — familles vides masquées, les deux vides → « Rien à faire dans FBI. », filtre Équipe conservé). **Suivi après placement (P4-197)** : une fois un match placé, `WeekWorkbench` recadre la semaine dessus et lui rend le focus. **Le « gardien » à l'ouverture (RMM-3, livré)** : `MatchesLayout` POSTe une visite au montage (`useModuleVisit`, une fois par ouverture, `enabled` sous la même garde socle) — `ModuleVisitBanner` résume ce qui a changé depuis la dernière visite (matchs arrivés, nouveaux conflits, planning de saison changé), muet en première visite/delta vide, et `ConflictRadar` porte des chips « Nouveau » par empreinte de conflit (ornement pur). L'axe de navigation est la **semaine calendaire** (`weekLabel`), le n° de rencontre FBI (`externalRef`) affiché en repère (grille + listes). Au-dessus de la grille de `WeekWorkbench`, un écart au modèle (`offModelBadge`) et un signal « même week-end » (`sameWeekendBadge`, pilule neutre — deux membres d'une même rotation reçus le même week-end, RMM-5 PR-4) s'affichent l'un sous l'autre. **Depuis PR 2a « Configuration & navigation » (2026-09-16)**, le gabarit A/B et les créneaux partagés ont quitté la Configuration pour la page sœur **« Semaine type »** (`/matchs/semaine-type`, `TypicalWeekPage.tsx`, deep-linkable, nav propre — ci-dessus) : le gabarit A/B (`TypicalWeekendGrid`) en vedette, **Créneaux partagés (alternance)** (`MatchSlotRotationsEditor`) sous un séparateur, et le bouton **« Habitudes & passerelles »** (`HabitsLinksDialog`, modale) — les anciens deep-links `?section=gabarit\|creneaux` de `/matchs/configuration` y redirigent (`<Navigate replace>` sur lecture du param brut). `MatchSlotRotationsEditor` rend une **liste compacte** (une ligne-bouton par rotation, ex. « Samedi 20:30 · Armand · U9F2 ⇄ U9M1 », une seule dépliée à la fois, bouton Supprimer FRÈRE du bouton de dépliage — jamais imbriqué). **Depuis C8 (2026-09-19)**, les adversaires ont, eux aussi, quitté la
Configuration pour leur propre page sœur **« Adversaires »** (`/matchs/adversaires`,
`OpponentsPage.tsx`, `OpponentTravelCard` absorbée, **refondue au grain GYMNASE par PR I,
2026-09-20**) : un `<tbody>` par club adverse (ligne club « Ajouter un gymnase »/« Aucun gymnase
connu », une ligne par gymnase apparié, les libellés « à apparier » du même club), menu APG
« Fusionner »/« Retirer » par gymnase, filtre segmenté (Sans gymnase/À apparier/Tous, `?filtre=`),
progression dérivée de `travelStatus` (Mercure via `useTravelStream`, `shared/lib/travelStream.ts`),
« Réessayer les manquants » sur échec partiel — le trajet lui-même est désormais lu par le
Calendrier depuis `fixture.awayTravel` (dérivé de la rencontre, plus un appel séparé à cet écran)
— détail complet : `specs/courantes/module-matchs.md` § Écran Adversaires. Les **quatre réglages rares restants** — **Échéances de saisie** (`EntryDeadlinesEditor`, RMM-6 PR-2 : multi-sélection de compétitions + une date appliquée en lot, trois provenances CLUB/proposée/aucune distinguées icône+texte, badge « partagée avec les autres clubs » sur les compétitions appariées), **Durée des matchs** (`MatchDurationsEditor`, tableau partagé par groupe de défaut servi, `shared/components/ui/table.tsx`, légende = la phrase de défaut), **Accès match** (PR 2a — ex-« Réglages de saison », clé URL `reglages` conservée : la liste des gymnases qui ONT au moins une fenêtre d'accès match, triés alpha, puis les gymnases SANS accès repliés à part (`ChevronDown`, disclosure native) ; chaque ligne « Modifier » ouvre une **modale par gymnase** — `MatchWindowsEditor`, même éditeur que l'étape Gymnases du wizard, plus de sélecteur de gymnase dans la carte elle-même — le focus revient explicitement au bouton « Modifier » de la ligne à la fermeture, qui a pu changer de liste), et **Libellés FFBB des gymnases** (`VenueLabelsSection.tsx`, devenue l'ÉCRAN D'APPARIEMENT — E2/P4-205, 2026-09-14, sur le backend d'E1 : un tableau, une ligne par libellé de l'inventaire agrégé `GET /api/venues/fbi-labels`, compteurs domiciles/placés/non placés, `VenueSelect` valant le gymnase confirmé sinon la suggestion « d'après les rencontres » pré-sélectionnée (`StatusPill` neutre), **Confirmer** (sans alias, POST additif sans confirmation) ou **Réaffecter** (un alias existe déjà et on change de gymnase, `ConfirmDialog` nommant « M non placés basculeront, N placés conservent leur gymnase », `reassign: true`), **Retirer** (ghost, `ConfirmDialog` inchangé) — état vide « Aucun libellé de salle importé pour l'instant ») — vivent sous `/matchs/configuration` (`ConfigurationPage`, réglages de saison seuls depuis P4-186 — le dépôt FBI/canal API/Engagements FFBB ont déménagé dans l'onglet Importer) en **quatre `AccordionSection` CONTRÔLÉES, une seule ouverte à la fois** (P4-185, « une section = un écran », amendé PR 2a puis C8 — les adversaires ont quitté cette page pour `/matchs/adversaires`), ancrées `?section=echeances\|durees\|reglages\|libelles` (**défaut = tout replié depuis PR 2a** — absent, `aucune`, une valeur inconnue, ou les clés déplacées `gabarit`/`creneaux` ⇒ aucune section ouverte ; `gabarit`/`creneaux` redirigent en plus vers `/matchs/semaine-type`, `decodeSectionParam`/`applySectionToParams` dans `lib/urlState.ts`) ; un résumé discret dans l'en-tête de chaque bouton (`lib/configSummaries.ts`, ex. « 2 sur 5 compétitions renseignées », « 4 personnalisées », « 1 à localiser sur 6 équipes adverses », « 4 gymnases » (`accessSummary`, PR 2a), « N libellé(s) · N non apparié(s) » (`labelsSummary`, depuis l'inventaire agrégé, E2) — muet en chargement/échec, jamais un compte inventé côté front) ; repli = démontage de la section (un brouillon en cours — nouveau créneau, cases d'échéances non sauvées — se perd au changement de section, assumé). `MatchSlotRotationsEditor` rend une **liste compacte** (une ligne-bouton par rotation, ex. « Samedi 20:30 · Armand · U9F2 ⇄ U9M1 », une seule dépliée à la fois, bouton Supprimer FRÈRE du bouton de dépliage — jamais imbriqué) et `MatchDurationsEditor` un **tableau partagé par groupe de défaut servi** (`shared/components/ui/table.tsx`, légende = la phrase de défaut) — huit routes d'un même layout (`/matchs` index = le Calendrier, `/matchs/consulter` = redirection permanente vers `/matchs`, `/matchs/importer`, `/matchs/configuration`, `/matchs/adversaires`, `/matchs/semaine-type`, `/matchs/reconciliation`, `/matchs/conflits`), garde socle commun. **Rotation A/B (RMM-5, LIVRÉ EN ENTIER — 4 PR, clôt P2-49)** : `MatchSlotRotationsEditor` déclare des créneaux de match partagés (gymnase+jour+heure, N équipes ORDONNÉES en alternance, flèches ↑/↓, jamais de drag — « l'ordre ne commande aucun calendrier ») ; `TypicalWeekendGrid` gagne un segmenté « Semaine A/B/… » (`weekCountOf`, invisible sans rotation déclarée — la grille reste EXACTEMENT celle d'avant), date-less (aucun ancrage calendaire réel), rotation hors week-end listée à part. **Espace Importer (PR-3a backend + PR-3b frontend, LIVRÉ EN ENTIER, 2026-09-08 — clôt RMM-4/P2-48 et P4-186)** : `/matchs/importer` (`ImportPage.tsx`) réunit les ENTRÉES de données de match — carte « Données de match » (Importer FBI, Vérifier via l'API FFBB, **Engagements FFBB** — modale `FfbbEngagementsDialog` refondue P4-200 C2 (2026-09-13) : détail de l'écran (compteur, chip de suggestion, `size="xl"`) dans `specs/courantes/module-matchs.md` §7 « Écran Importer » —, fraîcheur du dernier dépôt `useLatestFbiIngestion`) — et la FILE de traitement (`ReviewQueue.tsx`) : un accordéon par équipe (`AccordionSection` en mode contrôlé, ouverture reflétée dans `?equipe=`), en-tête « SM1 · N à valider · N écart(s) · N sans gymnase » (`unattachedCount`, P4-187b), « Tout valider » (rencontres à écart sautées et nommées en toast), interrupteur « Afficher les traitées » (`?traitees=1`, masquées par défaut) et, **depuis la lisibilité de ligne (2026-09-12)**, interrupteur « Masquer les extérieurs » (`?exterieurs=masques`, même patron, filtre AVANT `buildReviewQueue` — en-têtes recomptés, une équipe 100 % extérieur disparaît naturellement ; le badge de l'onglet Importer garde le compte réel, masquer ≠ traiter ; caveat soldé par P4-199 (2026-09-12) : un extérieur naît désormais `REVIEWED` d'office, plus rien à « valider » en masse sur ces lignes) ; chaque rencontre (`ReviewQueueRow.tsx`) affiche une icône `House`/`Plane` (aria-hidden, le mot « Domicile »/« Extérieur » porte le sens), puis après l'adversaire l'heure (« HH:MM » tabulaire ou « heure non publiée ») et la salle (gymnase résolu via `venueId`, sinon le libellé FBI brut `fbiVenueLabel`, sinon rien), se valide en un clic — « Valider » pour une NEW, **« Pris en compte » pour une REVIEWED qui porte encore une alerte `autoApplied`** (même endpoint, P4-199) — ou s'arbitre écart par écart (Amateo / FBI ou API FFBB, boutons « Garder Amateo »/« Prendre … », conséquence explicitée par `lib/deviationConsequence.ts`), bandeau dédié pour un écart `autoApplied` nommant la source et l'ANCIENNE valeur → la NOUVELLE (« {source} a déplacé ce match (champ) : ancien → nouveau », bug d'affichage corrigé au passage), bouton **« Replacer »** (domicile SEULEMENT, absent sur un extérieur) qui pose le filtre équipe + la semaine + la rencontre sélectionnée avant de naviguer vers `/matchs` (le Calendrier, panneau permanent — plus de rail depuis PR 3b), pour ouvrir directement le panneau de placement (au lieu d'atterrir sur la grille nue). **Rattacher un domicile importé sans gymnase (P4-187b)** : un domicile qui porte un `fbiVenueLabel` mais aucun `venueId` (`lib/reviewQueue.ts` `isUnattachedHome`, ouvertes ET traitées) affiche un sous-bloc neutre « `<libellé>` · non rattaché » + bouton **Rattacher** ; déplié, un `VenueSelect` pré-sélectionné sur `Fixture.suggestedVenueId` (s'il figure dans la liste des gymnases actifs, sinon placeholder) + **Confirmer**/**Annuler** — Confirmer envoie le libellé BRUT (`POST /api/venues/{id}/external-labels`, `useAttachVenueLabel`, invalide `fixtures` + `venues`) et affiche un toast « N domicile(s) rattaché(s) à `<gymnase>` (toutes équipes) » ou « Libellé confirmé — aucun nouveau domicile à rattacher » si l'alias existait déjà sans domicile à réattacher ; un 422 serveur s'affiche tel quel, le bloc reste ouvert. **Réaffecter ou retirer un alias mal posé** vit dans l'AUTRE écran, la 5ᵉ section de `/matchs/configuration` — l'écran d'appariement E2 décrit ci-dessus (`?section=libelles`) : `DELETE /api/venues/{id}/external-labels/{label}` (`detachVenueLabel`, `ConfirmDialog` inchangé — « les matchs déjà rattachés gardent ce gymnase, seuls les prochains imports ne le seront plus ») pour un simple retrait, ou `reassign: true` sur le même POST que l'attache pour corriger un mauvais rattachement **en un geste** (E1 backend + E2 écran, remplace les deux gestes séparés d'avant le 2026-09-14). **Le signal partagé** (`UnpairedVenueLabelsBanner.tsx`, décision : l'appariement a UNE maison, les autres écrans y RENVOIENT par un bouton « Apparier les salles », jamais de modale sur modale) — « N libellé(s) de salle non apparié(s) — M domiciles n'apparaissent pas sur la grille » (`role="status"`, ton warning), rendu au-dessus de la grille du Calendrier et sur la carte « Données de match » d'Importer ; sous la grille, `HiddenHomesWeekNotice` fait l'écho tranquille « N domiciles de ce week-end sans gymnase, non affichés » (ton neutre, dérivé du cache `useFixtures` de la semaine affichée) ; le rapport de `ImportFbiDialog` propose le même bouton après un import qui laisse des libellés sans gymnase. `lib/reviewQueue.ts` (`buildReviewQueue`, `pendingReviewCount`) dérive tout côté client depuis le MÊME cache `useFixtures` que la boucle — aucun endpoint agrégé. `ImportFbiDialog` a perdu son détour « Examiner… » : l'import envoie toujours `{file, mappings}` sans décisions, les écarts sont CONSIGNÉS sur les rencontres (`OUT_OF_SYNC`) et le rapport propose « Ouvrir la file ». **Dépôt en onglets par famille (2026-09-12, mesure terrain 50 divisions/291 lignes)** : la table de correspondances se range en onglets (`Tabs`/`TabPanel`) par famille de division (`lib/divisionFamily.ts` — Départemental, Régional, Brassage, Coupes CRM, Coupe ARA, Amicaux, Autres), le libellé de chaque onglet portant un compteur d'appariement (« Départemental (4/23) »), un onglet vide n'étant pas rendu ; le fichier choisi est lu en mémoire UNE fois (snapshot figé envoyé à l'analyse ET à l'import — un fichier relu au disque entre les deux appels faisait échouer l'import en silence côté navigateur) ; le bouton Importer reste actif, une confirmation NOMME les divisions restées sans équipe avant d'importer quand même ; modale passée en `size="xl"`, un seul défilement (la modale). Détail complet (règles de classification, exemples réels) : `specs/courantes/module-matchs.md` §7 « Écran Importer ». La vue `/matchs/reconciliation` (`ReconciliationView.tsx`, `ReconciliationPanel` SUPPRIMÉ) ne sert plus qu'au canal API pour les rencontres FFBB **créables** (absentes de l'app) — plus aucun écart n'y transite ; `useMatchesStore().reconciliation` est réduit au seul variant `{channel: "api", creatable, fetchedAt}`. `checkViaApi` (dans `ImportPage`) route en trois issues : créables → la vue de réconciliation ; pas de créable mais des écarts → `apply` direct (toast « N écart(s) consigné(s) dans la file ») ; rien → toast « Tout est en phase… ». Vocabulaire D9 : `VALIDATED` se dit désormais « Attesté FBI » (`lib/fixtureStatusLabel.ts`), plus « Validé ligue » ; `UNPLACED` se dit « Sans créneau » (UXC-24, 2026-09-24), plus « Importé » — « À placer » (liste `WeekWorkbench`) et « à confirmer » (légende `WeekendGridLegend`) restent, trois mots pour trois métiers. **Lot L (2026-09-21, amendé par la revue `d60b3fc0` le même jour)** : bandeau de rattrapage (`LeagueValidationBanner`, muet à 0) sur Importer + section du rapport de fin d'import (`LeagueValidationReportEntry`, `ImportFbiDialog`) confirment en LOT, d'un geste chiffré partagé (`LeagueValidationConfirmDialog`), les domiciles importés déjà datés côté fédération (heure + gymnase) — utile au club qui démarre en cours de saison ; la pastille de statut ne change pas, seule cette confirmation dit « validé ligue » ; renvoi permanent vers les Échéances de saisie (`EntryDeadlinesLink`, deep-link `?section=echeances`). **`VALIDATED` n'est plus un cul-de-sac** : `PlacementPanel.tsx` offre désormais « Corriger — repasser en Placé » sur TOUTE rencontre `VALIDATED` (réconciliation D9 ET bascule en lot) — décision fondateur, une bascule en lot peut reposer sur un gymnase apparié AUTOMATIQUEMENT ; le texte sous la pastille, générique aux deux chemins, ne parle plus d'une attestation par le seul fichier fédéral (« Ce match est ancré sur les date, heure et salle enregistrées côté ligue. Corrigez-le si l'une d'elles doit changer… »). Détail métier : `specs/courantes/module-matchs.md` §7 « Écran Importer » | Required | `AppLayout` |
...
│   ├── matches/                # MatchesLayout (huit routes, 6 onglets) → CalendarPage (route
│   │                           # index « /matchs » — l'écran unique, PR 3b 2026-09-16 : fusion de
│   │                           # l'ex-Semaine et de l'ex-Consulter, MatchesPage.tsx/ConsultPage.tsx
│   │                           # SUPPRIMÉS ; WeekWorkbench (établi de la temporalité Semaine —
│   │                           # panneau de placement permanent, grille week-end, radar EN DERNIER,
│   │                           # FbiEntryList en modale « FBI — à faire ») ; WeekCounters (2
│   │                           # compteurs bornés à la semaine + 1 global FBI, lib/loopSteps.ts) ;
│   │                           # CalendarControls
│   │                           # (chips, segmenté Semaine·Mois·Phase, navigateur) ; MonthTable/
│   │                           # PhaseTable (temporalités Mois/Phase, lecture) + ImportPage (données
│   │                           # de match + ReviewQueue/-Row, file de traitement par équipe,
│   │                           # PR-3b, geste Rattacher un domicile sans gymnase P4-187b) +
│   │                           # ConfigurationPage (réglages de saison seuls, P4-186 ;
│   │                           # quatre AccordionSection contrôlées ancrées ?section=, P4-185 —
│   │                           # lib/configSummaries.ts pour les résumés d'en-tête ;
│   │                           # VenueLabelsSection, 4ᵉ section, écran d'appariement des libellés
│   │                           # FFBB, E2/P4-205 ; UnpairedVenueLabelsBanner, signal partagé) +
│   │                           # OpponentsPage (page sœur « Adversaires », onglet dédié C8
│   │                           # 2026-09-19, ex-OpponentTravelCard absorbée) +
│   │                           # TypicalWeekPage (page sœur « Semaine type », gabarit A/B +
│   │                           # créneaux partagés, PR 2a) + ReconciliationView (canal API seul,
│   │                           # rencontres créables — le ReconciliationPanel des écarts par écart
│   │                           # a disparu, PR-3b) +
│   │                           # ConflictsPage (lecture seule, tous les conflits de la saison
│   │                           # pivotés coach/équipe/gymnase/journée, lib/conflictPivot.ts,
│   │                           # PR A 2026-09-15) — ConflictLine/ConflictSeverityGroups
│   │                           # (maison unique de la ligne de conflit, consommée par
│   │                           # ConflictsPage ET ConflictRadar) + ConflictResolutionControl
│   │                           # (statut de traitement par empreinte, P4-207 PR B2,
│   │                           # lib/conflictResolution.ts maison unique des compteurs)
...
│   │                           # modal, menu (APG menu-button, restoreFocusOnSelect/triggerRef,
│   │                           # P4-207), accordion, toaster, empty-hint, filter-toggle (case à
│   │                           # cocher partagée d'un filtre d'affichage, P4-207)

...
Last verified @ 2026-08-24 (recalé par RMM-4 PR-3, le canal API FFBB — **RMM-4 est LIVRÉ EN
ENTIER**. La ligne `/matchs` et le tableau des endpoints confrontés au code :
`ReconciliationView.tsx` (union discriminée du store `channel: "xlsx"|"api"`, bandeau d'honnêteté
`role="status"` sur le canal API, section créations avec `TeamSelect` par ligne — non sélectionné =
non créé), `store.ts` (`ReconciliationPayload` porte désormais les deux formes), `api.ts`
(`getFfbbRencontres`/`applyFfbbRencontres`, `ffbb/rencontres` + `.../apply`), `queries.ts`
(`useFfbbRencontres`/`useApplyFfbbRencontres`), `ConfigurationPage.tsx` (bouton « Vérifier via
l'API FFBB », à la demande — `enabled: false` par défaut). Re-confirmé dans la même passe :
`router.tsx` (route `matchs/reconciliation` enfant de `MatchesLayout`), `ImportFbiDialog.tsx`
(bascule « Examiner » sur `deviations` non vide, flux inchangé sinon), `MatchesLayout.tsx`
(`useModuleVisit`, un POST par ouverture). Le reste du fichier (routes hors matchs, primitives,
stack) non re-vérifié cette passe)

...
Last verified @ 2026-09-11 (`documentation-update`, P4-192 + P4-184 — dernières lignes ouvertes du
lot module matchs, frontend seul). Recalé : (1) l'atterrissage du rail (`railStep`, entrée `matches
store` § 8) — `defaultLoopStep` (`lib/loopSteps.ts:152-158`, confronté au code) bascule sur
`homeSlots` quand le premier trou est `fbiEntry` **vide** (champ `LoopStep.empty`, posé
uniquement sur cette étape) ; tout autre trou, ou un choix de rail fait par l'utilisateur, prime
toujours ; (2) la puce `ResourceFilter` (§ 6.2, `ResourceFilter.tsx:46-64`, confronté au code) —
`Échap` la ferme et rend le focus au bouton déclencheur, via un listener `keydown` natif sur le
wrapper (`stopPropagation`) ; le voile de clic-hors est inchangé. Détail métier des deux :
`specs/courantes/module-matchs.md` § « Refonte UX — RMM-1 » et § « Filtres par équipe / coach /
gymnase ». Reste du fichier non re-sondé cette passe.
## 1. Stack Decided

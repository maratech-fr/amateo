...
Last verified @ 2026-09-11 (`documentation-update`, P4-196 écran de retrait des libellés FFBB).
Recalé : ligne de route `/matchs` — 7ᵉ `AccordionSection` de `/matchs/configuration`, **« Libellés
FFBB des gymnases »** (dernière position, ancrée `?section=libelles`), `VenueLabelsSection.tsx` :
un gymnase par ligne (ceux qui portent ≥ 1 alias), alias en pastilles affichées VERBATIM (forme
normalisée — minuscules, sans accents, phrase d'aide dédiée) + bouton **Retirer** par pastille,
`ConfirmDialog` (« Les matchs déjà rattachés gardent ce gymnase ; seuls les prochains imports ne
le seront plus. »), état vide qui renvoie vers le geste **Rattacher** d'Importer ; résumé d'en-tête
`labelsSummary` (`lib/configSummaries.ts`) compte les GYMNASES à ≥ 1 alias, jamais les alias
(« N gymnase(s) nommé(s) par la FFBB » / « aucun libellé rattaché » / muet en chargement) ; retrait
= `DELETE /api/venues/{id}/external-labels/{label}` (`detachVenueLabel`, libellé encodé),
`useDetachVenueLabel` invalide `["venues"]` **et** `["fixtures"]` en `onSettled` (le
`suggestedVenueId` d'Importer est dérivé des alias à la lecture). **Corriger une erreur = deux
gestes** : retirer ici, puis Rattacher depuis Importer ; les rencontres déjà rattachées au mauvais
gymnase avant le retrait se corrigent une par une (hors scope assumé) — confronté au code
(`VenueLabelsSection.tsx`, `ConfigurationPage.tsx`, `lib/urlState.ts`, `lib/configSummaries.ts`,
`api.ts`, `queries.ts`). **Recalé en suivant (P4-194/195, même date)** : la cellule `/matchs` §
Consulter · Phase disait encore « importées / attendues (`COMPETITION_INCOMPLETE` sinon
`count/expectedMatchdays`) » sans le cas `null` — `phaseView.ts::phaseCompleteness` rend
`expected: number | null` depuis P4-195 (une `CUP` n'a pas `expectedMatchdays`), l'en-tête bascule
alors sur « N journée(s) importée(s) » sans dénominateur (`ConsultPage.tsx`, confronté au code).

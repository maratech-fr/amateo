...
Last verified @ 2026-09-16 (`documentation-update`, PR 2a « Configuration & navigation »).
Recalé, contre `MatchesLayout.tsx`/`routes.tsx`/`TypicalWeekPage.tsx`/`ConfigurationPage.tsx`
lus à cette passe : la ligne `/matchs` — ordre de nav (Conflits · Consulter · Importer ·
Configuration · **Semaine type** · Semaine, défilable horizontalement), septième route
`/matchs/semaine-type` (gabarit A/B + créneaux partagés + « Habitudes & passerelles », sortis de
la Configuration), Configuration retombée à **cinq** `AccordionSection` (défaut tout replié, plus
« gabarit ouvert par défaut »), section « Réglages de saison » renommée **« Accès match »** (liste
par gymnase + modale par gymnase), `OpponentTravelCard` (recherche, bouton unique « Mettre à jour
les adversaires »). Reste du fichier non re-sondé cette passe.
## 1. Stack Decided

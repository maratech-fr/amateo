...
Last verified @ 2026-09-14 (`documentation-update`, E2/P4-205 « écran d'appariement des libellés
de salle »). Recalé : la ligne `/matchs` — la 7ᵉ section « Libellés FFBB des gymnases » décrit
désormais l'écran d'appariement (tableau, `VenueSelect`, Confirmer/Réaffecter/Retirer) et le
signal partagé `UnpairedVenueLabelsBanner`/`HiddenHomesWeekNotice`, confrontés au code
(`VenueLabelsSection.tsx`, `UnpairedVenueLabelsBanner.tsx`) — le détail complet reste dans
`module-matchs.md` § « Gymnase depuis le libellé », ce fichier ne redécrit que la carte des
boutons. Reste du fichier non re-sondé cette passe.
## 1. Stack Decided
...
│   ├── matches/                # MatchesLayout (cinq routes) → MatchesPage (boucle : rail 5 étapes

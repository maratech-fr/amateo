...
Last verified @ 2026-09-09 (`documentation-update`, P4-185 accordéons de la Configuration). Recalé :
ligne de route `/matchs` (les six sections de `/matchs/configuration` en `AccordionSection`
CONTRÔLÉES, une seule ouverte, ancrées `?section=` — `ConfigurationPage.tsx`,
`lib/urlState.ts`/`decodeSectionParam`/`applySectionToParams`, `lib/configSummaries.ts` pour les
résumés d'en-tête ; `MatchSlotRotationsEditor.tsx` en liste compacte dépliable ;
`MatchDurationsEditor.tsx` en `Table` partagée par groupe de défaut ; corrigé au passage : la ligne
listait encore « Engagements FFBB » sous Configuration alors que P4-186 l'a déplacé vers Importer,
et omettait « Durée des matchs »/« Adversaires à localiser » des six sections) et l'arborescence
`features/matches/`, confrontés au code (`ConfigurationPage.tsx`, `MatchSlotRotationsEditor.tsx`,
`MatchDurationsEditor.tsx`, `lib/urlState.ts`, `lib/configSummaries.ts`). Reste du fichier non
...
│   │                           # PR-3b) + ConfigurationPage (réglages de saison seuls, P4-186 ;

...
Last verified @ 2026-09-02 (**rotation de fraîcheur** `documentation-update`, zone non touchée par cette PR. Re-confronté au code : `MutualisationPanel.tsx` toujours absent (supprimé) ; `SharedTrainingBlockPanel.tsx` existe (`frontend/src/features/wizard/steps/`) ; `sharedTraining.ts::mutualisedTeammateLabel` toujours la source unique (`wizard/lib/sharedTraining.ts:61`) ; `SlotReservationModal.tsx` (`wizard/steps/`) garde le préfixe `block:` (`BLOCK_VALUE_PREFIX`, ligne 22) sans repli `group:` ; `GroupReservationController.php` lit toujours `sharedTrainingBlockId` seul, `sharedTrainingGroupId` cité seulement en commentaire comme retiré. Tout juste. Reste du fichier non re-contrôlé ligne à ligne cette passe — un stamp REMPLACE, l'historique des passes précédentes vit dans git)

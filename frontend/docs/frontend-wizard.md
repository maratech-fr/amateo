...
Last verified @ 2026-09-25 (`documentation-update`, P3-7 PR-B frontend — import FBI des équipes,
`9cc27466`). Re-confronté au code : le bouton « Importer depuis FBI » vit dans `TeamsEditor`
(`features/wizard/steps/TeamsStep.tsx:742-746`), absent par construction en mode période
(`TeamsStep` bascule sur `PeriodTeams`, test négatif dans `TeamsStep.test.tsx`) ; `TeamsImportModal.tsx`
appelle `POST /clubs/{id}/import-teams/analyze` puis `POST /clubs/{id}/import-teams` (`wizard/api.ts`) ;
`shared/lib/fileSnapshot.ts` existe et est aussi consommé par `ImportFbiDialog.tsx` ✓. Reste du
fichier non re-contrôlé ligne à ligne cette passe — un stamp REMPLACE, l'historique vit dans git.

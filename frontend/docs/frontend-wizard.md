...
Last verified @ 2026-08-26 (P2-53 RMM-8 PR-4 — le levier Préféré/Obligatoire de la règle de trajet,
DERNIÈRE PR du lot). Confronté au code : `useWizardFooter` porte bien le bouton « Trajets entre
gymnases » (`VenuesStep.tsx`, `venues.length >= 2`) ✓ ; `VenueGeocodeField` ne réécrit jamais un
gymnase déjà géolocalisé sans clic explicite sur « Modifier l'adresse » ✓ ; `TravelMatrixModal`
groupe bien « Depuis {gymnase} » et ne lance l'autofill que sur clic (falsifié par
`TravelMatrixModal.test.tsx`) ✓ ; `Coach.isVehicled` écrit via `payload()` de `CoachesStep.tsx` ✓ ;
`TravelRuleNotice` (`ImplicitRulesPanel.tsx`) n'apparaît que si `useVenueTravelTimes` sert ≥1
ligne, et porte désormais un VRAI sélecteur Préféré/Obligatoire (`useTravelRuleSetting`/
`useUpdateTravelRuleSetting`), lecture seule uniquement sur saison archivée, falsifié dans les
deux sens par `TravelRuleNotice.test.tsx` ✓. Reste du fichier non re-contrôlé ligne à ligne

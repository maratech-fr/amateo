...
Last verified @ 2026-08-22 (édition lot P2-45 — les passerelles & la mutualisation RENTRENT À LA MAISON, l'étape Équipes : l'onglet « Mutualisation » quitte `ConstraintsStep` (§4) au profit d'une modale « Liens de {équipe} » par ligne, sur les DEUX hôtes de l'étape (saison `TeamsStep`, période `PeriodTeamsPanel`). Re-vérifié contre le code : `ConstraintsStep.tsx` ne monte plus `MutualisationPanel` ni l'onglet « Mutualisation » (mode `"mutualise"` supprimé) ✓ ; `TeamsStep.tsx` et `PeriodStructure.tsx` rendent l'affordance `Link2` « Liens de {équipe} » ouvrant `TeamLinksModal` (`features/wizard/steps/TeamLinksModal.tsx`) ✓ ; la section passerelles est `TeamLinksSection` extraite de `HabitsLinksDialog` (`features/matches/`), en LECTURE SEULE en période/vacances et éditable en saison ✓ ; `MutualisationPanel` composé avec `initialTeamId` (pré-cochage) + `hideLinksManager` (masque « Gérer les passerelles ») ✓ ; la sous-ligne des équipes porte « Passerelle avec X (Préféré) », intensité SERVIE par `useTeamLinks` ✓. Le reste du fichier non re-vérifié cette passe — un stamp REMPLACE, l'historique des passes précédentes vit dans git)

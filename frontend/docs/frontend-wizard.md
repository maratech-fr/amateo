...
Last verified @ 2026-09-03 (lot « un seul chemin de remplissage », `documentation-update`, zone frontend non touchée par la PR. Recalé : le catalogue `CategoryCatalog` n'est plus seedé par `BasketballInit` (fixture supprimée) mais par `BcclSeeder` (`backend/src/Seed/BcclSeeder.php:195`, `CategoryCatalog::categories()`) — et toujours par `AuthController::seedNewClub` (`:984`) pour un club inscrit. Re-confronté aussi : `SharedTrainingBlockPanel.tsx` existe (`frontend/src/features/wizard/steps/`), `MutualisationPanel.tsx` toujours absent. Reste du fichier non re-contrôlé ligne à ligne cette passe — un stamp REMPLACE, l'historique vit dans git)

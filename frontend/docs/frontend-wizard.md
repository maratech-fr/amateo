...
Last verified @ 2026-09-06 (`documentation-update`, PR P4-164 PR-1 — le picker de l'onglet
Réserver et le classement `TeamSelect` cités en item 4/§1 sont le SUJET de la PR). Recalé sur le
code : `TeamSelect` (item 1 et item 4) est désormais construit sur la primitive `listbox.tsx`
(groupes par rang, plus des optgroups natives) ; le picker `assignableTeams` de `SlotReservationModal`
n'EXCLUT plus une équipe au résidu solo épuisé — elle reste visible, désactivée, avec son motif
(`SlotReservationModal.tsx`, `optionMetaFor`) — l'ancien `optionLabel`/suffixe texte a disparu.
Reste du fichier non re-contrôlé ligne à ligne cette passe — un stamp REMPLACE, l'historique vit
dans git.

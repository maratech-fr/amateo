...
Last verified @ 2026-08-31 (P2-51 PR-7, `documentation-update` — le modèle groupe {équipes, K} est RETIRÉ, le bloc (`SharedTrainingBlock`) devient la SEULE notion de mutualisation ; confronté au code : `MutualisationPanel.tsx` n'existe plus (supprimé), `SharedTrainingBlockPanel.tsx` (liste plate + recherche, aucun tri « équipes liées » par passerelle), `sharedTraining.ts::mutualisedTeammateLabel` (une seule source, les blocs), `SlotReservationModal.tsx` (picker `block:` seul, plus de préfixe `group:`), `GroupReservationController.php` (`sharedTrainingBlockId` seul, plus de repli). Les sections « Sous-ligne des liens », « Liens de {équipe} » et « Entraînements mutualisés » recalées. Reste du fichier non re-contrôlé ligne à ligne cette passe — un stamp REMPLACE, l'historique des passes précédentes vit dans git)

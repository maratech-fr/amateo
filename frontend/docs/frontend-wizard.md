...
Last verified @ 2026-08-28 (BCK-22 côté écran — `reasonLabel` (`TravelMatrixModal.tsx`) confronté au
code : `AutofillUnresolvedReason` (`wizard/api.ts`) porte bien les 3 codes
`missing_geo`/`routing_failed`/`budget_exceeded`, et `REASON_LABELS` est une table `Record`
EXHAUSTIVE — plus de ternaire à repli muet — falsifié par `TravelMatrixModal.test.tsx` (une paire
`budget_exceeded` affiche « calcul interrompu, relancez », jamais « calcul impossible ») ✓. Reste
du fichier non re-contrôlé ligne à ligne cette passe — un stamp REMPLACE, l'historique des passes

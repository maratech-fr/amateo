...
Last verified @ 2026-08-28 (P4-142 — renommage infra : rôle owner `amateo_owner`, conteneurs `amateo-*`, fonction RLS `enable_rls_for_existing_amateo_tables`, DSN admin recalés. Re-confronté au CLUSTER réel : 44 policies `admin_all` → `amateo_owner`, 0 vers l'ancien rôle ; `clubscheduler` n'existe plus ; `app_user` sans superuser ni BYPASSRLS ; 44/44 tables RLS en FORCE. Passe précédente : édition = ajout du renvoi `testing/blocking-tests.md` sur la ligne « Tests & guardrails » — le fichier existe (liste canonique sortie de `CLAUDE.md` §4 ce jour, gardée par `BlockingTestsListMatchesCiTest`) ✓ ; passe du 2026-08-26 : renvoi `backend/docs/geo-api.md` (P2-53 PR-1) ; re-vérifié alors : `engine/CONTRACT_VERSION` porte **2.16** (le bump P2-53 PR-2 est fait : blocs `venueTravelTimes`/`isVehicled`/règle `travelTime`), et la liste des features `frontend/src/features/` est exacte à l'unité près — `admin`, `auth`, `club`, `coach-wishes`, `cockpit`, `feedback`, `legal`, `matches`, `planning`, `profile`, `release-notes`, `season-transition`, `wizard`. ⚠ Vérification volontairement ÉTROITE (renvoi + contrat + liste features) : le reste de la carte n'a pas été reconfronté au code ce jour)

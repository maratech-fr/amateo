...
Last verified @ 2026-08-25 (recalé au bump de contrat **2.14 → 2.15** — RMM-5 rotation A/B PR-2, bloc `slotRotations`. Passe précédente 2026-08-24, fusionnée au rebase : **ENG-32** — le monolithe `constraints.py` est devenu le paquet `constraints/`, les références pointent désormais fichier+fonction, stables au refactor ; **rotation de fraîcheur** — `engine/CONTRACT_VERSION` porte **2.15** (bump RMM-5 rotation A/B PR-2, bloc `slotRotations`), et la liste des features `frontend/src/features/` est exacte à l'unité près — `admin`, `auth`, `club`, `coach-wishes`, `cockpit`, `feedback`, `legal`, `matches`, `planning`, `profile`, `release-notes`, `season-transition`, `wizard`. ⚠ Vérifications volontairement ÉTROITES (références solveur + contrat + liste features) : le reste de la carte n'a pas été reconfronté au code ce jour)

...
Last verified @ 2026-08-26 (édition = ajout du renvoi `backend/docs/geo-api.md` (P2-53 PR-1 — clients sortants BAN+IGN) ; re-vérifié dans la même passe : `engine/CONTRACT_VERSION` porte **2.16** (le bump P2-53 PR-2 est fait : blocs `venueTravelTimes`/`isVehicled`/règle `travelTime`), et la liste des features `frontend/src/features/` est exacte à l'unité près — `admin`, `auth`, `club`, `coach-wishes`, `cockpit`, `feedback`, `legal`, `matches`, `planning`, `profile`, `release-notes`, `season-transition`, `wizard`. ⚠ Vérification volontairement ÉTROITE (renvoi + contrat + liste features) : le reste de la carte n'a pas été reconfronté au code ce jour)

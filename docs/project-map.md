...
Last verified @ 2026-08-24 (deux passes le même jour, fusionnées au rebase : **ENG-32** — le monolithe `constraints.py` est devenu le paquet `constraints/`, les références pointent désormais fichier+fonction, stables au refactor ; **rotation de fraîcheur** — `engine/CONTRACT_VERSION` porte toujours **2.14**, et la liste des features `frontend/src/features/` est exacte à l'unité près — `admin`, `auth`, `club`, `coach-wishes`, `cockpit`, `feedback`, `legal`, `matches`, `planning`, `profile`, `release-notes`, `season-transition`, `wizard`. ⚠ Vérifications volontairement ÉTROITES (références solveur + contrat + liste features) : le reste de la carte n'a pas été reconfronté au code ce jour)

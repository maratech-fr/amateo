...
Last verified @ 2026-08-30 (rotation documentation-update, sans rapport avec le sujet de la PR (P4-149). Re-confronté au code : `ls frontend/src/features/` toujours exact à l'unité près — `admin`, `auth`, `club`, `coach-wishes`, `cockpit`, `feedback`, `legal`, `matches`, `planning`, `profile`, `release-notes`, `season-transition`, `wizard` ✓ · `engine/CONTRACT_VERSION` = `2.16` ✓ · `TenantFilterListener::getSubscribedEvents()` porte bien le commentaire priorité 7 (`backend/src/EventListener/TenantFilterListener.php:50`) ✓. ⚠ Vérification volontairement ÉTROITE : le reste de la carte n'a pas été reconfronté au code ce jour)

...
Last verified @ 2026-09-09 (P4-187a « gymnase depuis le libellé », `documentation-update`) : ligne Behat nouvelle `un-domicile-importe-retrouve-son-gymnase.feature` ajoutée sous § Matchs (`ls backend/features/`), libellé « attestée FBI » de la feature PR-3a recalé, mention `tests/e2e/matches-importer.spec.ts` re-vérifiée (`ls frontend/tests/e2e/`).

...
conflits de PERSONNE ») : confronté au code cette passe — `engine/tests/semantic/
test_match_placement_semantics.py` existe (nouveau fichier, échauffement seul non pénalisé / vrai
recouvrement pénalisé) ✓ ; `backend/features/les-conflits-d-un-match-disent-la-verite.feature`
porte le scénario « deux gymnases » inversé + le contre-exemple à recouvrement réel ✓. Reste des
lignes non touchées cette passe — historique complet :

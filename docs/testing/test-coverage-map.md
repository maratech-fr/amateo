...
Last verified @ 2026-09-09 (P4-188 + P4-189 + P4-191, `documentation-update`) : ligne Behat nouvelle `les-conflits-d-un-match-disent-la-verite.feature` ajoutée sous § Matchs et § « Cycle de vie des plans (ADR-0002) » (`ls backend/features/` → 19 fichiers), le décompte « 11 suivantes » de la section Behat remplacé par un renvoi à `ls` (comptage déjà stale avant cette passe — 13 « suivantes » avant l'ajout, pas 11).
...
| `les-conflits-d-un-match-disent-la-verite.feature` (P4-188/189/191, 2026-09-09) | une fermeture racine découpée en milieu/fin dont l'enfant « milieu » pointe un entraînement remonte ce conflit `MATCH_TRAINING` (la période la plus ÉTROITE gagne, jamais un repli sur la racine sans plan) — la borne de début du conflit est l'heure murale du club, sans décalage horaire |

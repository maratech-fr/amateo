...
Last verified @ 2026-08-23 (recalé par la livraison P4-122 : le §5 cesse d'annoncer « aucun e2e ne construit les 3 plannings » — `club-life.spec.ts` est actif, les deux branches d'idempotence prouvées en local. Re-vérifié en écrivant : le spec vit hors `test.fixme`, vise la carte radar par son texte et la puce de couverture par sa fenêtre. Historique des passes : `git log -p --follow docs/testing/testing-strategy.md` — un stamp REMPLACE, il ne s'empile pas (DOC-33).)

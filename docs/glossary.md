...
| **Passerelle** (`TeamLink`) | Lien DÉCLARÉ entre deux équipes (aucune entité joueur n'existe) : `NOT_SIMULTANEOUS` (joueurs partagés — leurs matchs ne doivent pas se chevaucher) ou `BACK_TO_BACK` (enchaînement souhaité — préférence solveur, pas une règle). **Depuis le lot M (2026-09-21)** : le radar de conflits ne signale plus cette famille (l'ancien finding `TEAM_LINK_OVERLAP` a disparu du détecteur, décision fondateur) ; le solveur de placement, lui, garde `NOT_SIMULTANEOUS` comme préférence SOFT — asymétrie délibérée, `module-matchs.md` §2. Symétrique, nom neutre : servira aussi à l'entraînement |
...
| **Segment** (P2-41) | La fenêtre d'un enfant de période (`CalendarEntry.parentEntryId`) : un bloc de semaines calendaires **pleines et contiguës** (lundi→dimanche, clamp saison admis aux deux bords), inclus dans les semaines qui couvrent la mère. **La semaine simple est le segment de taille 1** (rétro-compatible, aucun nouveau concept). Un segment naît avec SON plan comme n'importe quel enfant (rail 1 entrée = 1 plan) et reste soumis aux mêmes garde-fous — anti-chevauchement entre segments frères, garde d'unicité de fenêtre 409 `window_already_planned` (P2-38). Livré des deux côtés : API (`CalendarEntryStateProcessor::assertValidWeekChild`) et écran — le picker (`WeekPickerDialog`) propose des segments précochés aux ruptures géométriques de l'offre. **Pour une VACANCE, ces segments restent scindables/fusionnables à la main** (liberté P2-41 intacte) ; **pour une FERMETURE (`closure`), leur nombre et leurs bornes sont désormais IMPOSÉS** — v. **Début / Milieu / Fin** ci-dessous. Détail : [ADR-0002](architecture/adr-0002-pattern-plan.md) (lot P2-41, amendé 2026-09-05). |
| **Début / Milieu / Fin** (découpage d'une indisponibilité, décision fondateur 2026-09-05) | Le découpage IMPOSÉ des segments d'une mère **FERMETURE** (`closure`) — remplace le scindage/fusionnement libre pour ce type seul, les VACANCES n'étant pas concernées. **Début** : la semaine entamée de tête (l'événement commence après le lundi). **Milieu** : un run de semaines PLEINES lun→dim contiguës — UN SEUL plan, même sur plusieurs semaines ; un trou de vacances (lun→ven couvert) ou une fenêtre qu'un AUTRE plan de période gouverne déjà coupe le milieu en **deux runs**, chacun son propre plan. **Fin** : la semaine entamée de queue. Au plus trois segments, jamais une semaine complète isolée. Algorithme PUR aux ruptures GÉOMÉTRIQUES seulement (semaines offertes + fenêtre de l'événement, aucune règle solveur redérivée) : `App\Service\WeekSegmentationRule::segments` (backend, garde le POST d'une semaine-enfant et le geste « Adapter d'un bloc » — 422 si la fenêtre visée compte plus d'un segment) ⇄ `cockpit/lib/weekSegmentation.ts::weekSegments` (front, miroir déclaré, qualifie l'offre du picker), parité mécanique `WeekSegmentationMirrorParityTest`. `App\Service\ClosureSegmentation` compose l'offre côté serveur (semaines pleines clampées saison, moins les vacances, moins les fenêtres déjà planifiées) ; tolère les semaines révolues en tête (l'offre ACTIONNABLE du picker peut être plus étroite que la géométrie PLEINE, les deux formes sont acceptées à la naissance d'un enfant). |
...
## Payload backend↔engine (contrat `CONTRACT_VERSION`, actuel 2.23)
...
| **RLS** | Row-Level Security PostgreSQL, policies `FORCE` sur `club_id`, clé = GUC `app.club_id`. Runtime `amateo_app` (restreint) / migrations-ops `amateo_owner` (connexion `admin`, **bypasse RLS**). CLI sans GUC → 0 ligne (attendu). |
